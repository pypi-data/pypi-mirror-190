            ...
            if len(all_prediction_df_columns) > 3:
                ...
                if task =='clsf':
                    with Pool(cpu_num) as p:
                        results_pool.append(p.map(self.get_score_clsf, all_combinations[1:]))
                elif task=='regr':
                    with Pool(cpu_num) as p:
                        results_pool.append(p.map(self.get_score_regr, all_combinations[1:]))

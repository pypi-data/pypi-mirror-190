...
class QuickDB:
    ...
    def getFS(self, account, date, tickers=None):
        ...
        for ticker, yq in self.getYQ(date).dropna().iteritems():
            ...
            result[ticker] = load_pkl(os.path.join(self.path, f'fs/{yq}.data'))[account].get(ticker)
        if tickers is not None:
            return pd.Series(result)[tickers]
        else:
            return pd.Series(result)

    def getTradables(self, start, end=None):
        """
        ...
        for ticker, yq in YQ.iteritems():
            try:
                res[ticker] = pd.Series(
                    ...
                    load_pkl(os.path.join(self.path, f'fs/{_yq}.data'))[account].get(ticker) for _yq in yq
                )
            except:
                pass

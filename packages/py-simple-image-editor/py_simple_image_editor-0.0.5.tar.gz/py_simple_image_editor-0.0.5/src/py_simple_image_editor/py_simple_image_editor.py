...
class Editor(ttk.Frame):
    def __init__(self, parent:tk.Frame | ttk.Frame, file:str=None, size:bool|tuple =None):
        ...
        if self.file:
            ...
            try:
                ...
                animated = True
            size = size if not size is None else img.size
        size = size if not size is None else (SIZE, SIZE)

        self.width, self.height = size
        self.tool_controller = tool_controller
        self.project = PixelProject(*size)
        ...
        (gif_view := LayerViewer(self.project, self.project.width, self.project.height, left_column_panes)).pack(fill = "both", expand = True)
        ...
        if self.file:
            for f in self.project.frames.copy():
                del f
            self.project.frames = []
            if animated:
                self.project.import_gif(self.file)
            else:
                self.project.import_image(self.file)
...
class App(sttk.App):
    def __init__(self, file:str=None):
        ...
        Editor(self.window, file).pack(fill="both", expand=True)

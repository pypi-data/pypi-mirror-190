...
class MeasureSearch(Task):
    ...
    def __call__(
        ...
        """
        print(f"Using subsample size {subsample_subset_size} for " f"measure search")
        trial_ = namedtuple(
            "trial_",
            [
                "fingerprint_type",
        ...
        for similarity_measure in all_similarity_measures:
            ...
            for fingerprint_type in all_fingerprint_types:
                ...
                (
                    nearest_corr,
                    nearest_p_val,
                ) = self.prop_var_w_similarity.get_property_correlations_in_most_similar(
                    molecule_set
                )
                (
                    furthest_corr,
                    furthest_p_val,
                ) = self.prop_var_w_similarity.get_property_correlations_in_most_dissimilar(
                    molecule_set
                )
                if optim_algo == "max_min":
                    ...
                    raise InvalidConfigurationError(f"{optim_algo} " f"not implemented")
                all_scores.append(
                    trial_(
                        fingerprint_type=fingerprint_type,
                        similarity_measure=similarity_measure,
        ...
        if show_top > 0:
            ...
            plot_multiple_barchart(
                x=[_ for _ in range(len(top_performers))],
                heights=bar_heights,
                legend_labels=[
                    "Overall scores",
                    "Nearest neighbor property " "correlation",
                    "Furthest neighbor property " "correlations",
                ],
                colors=colors,
    ...
    def __str__(self):
        return (
            "Task: determine appropriate fingerprint type and "

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        self.stdout.write("push notifications emitter started", ending='')
        ...
        while True:
            ...
            try:
                with transaction.atomic():
                    ...
                    self.stdout.write(
                        "proccessing {} events".format(events_len), ending=''
                    )
    ...
    async def emit(self, session: aiohttp.ClientSession, event: PushNotificationEvent):
        self.stdout.write(
            "emit event {}".format(event.event_id), ending='')
        ...
        try:
            ...
            self.stdout.write(
                "event {} has been delivered to {}".format(
                    event.event_id, res.url), ending=''
            )
        except Exception as e:
            self.stderr.write(
                "error while delivering the event {}, retry {}: {}".format(
                    event.event_id, event.retry, str(e)), ending=''
            )
        finally:

...
def test_standby(
    ...
    settings = standby_instance._settings
    standby_ctx = Context(settings=settings)
    pgbackrest_settings = pgbackrest.get_settings(settings)
    ...
    with postgresql.running(standby_ctx, standby_instance):
        if pgbackrest_repo_host:
            ...
            with monkeypatch.context() as m:
                ...
                rbck = repo_path.backup(
                    standby_ctx, standby_instance, pgbackrest_settings
                )
    info = pgbackrest.backup_info(
        ctx, service, pgbackrest.get_settings(instance._settings)
    )
    standby_info = pgbackrest.backup_info(
        standby_ctx, standby_service, pgbackrest_settings
    )
...
def test_backup_restore(
    ...
    backup1 = next(pgbackrest.iter_backups(ctx, instance, pgbackrest_settings))

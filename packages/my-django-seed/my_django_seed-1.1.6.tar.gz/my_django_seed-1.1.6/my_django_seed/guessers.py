...
def _timezone_format(value):
    ...
    """
    try:
        return timezone.make_aware(value, timezone.get_current_timezone(), is_dst=False) if\
            getattr(settings, 'USE_TZ', False) else value
    except NonExistentTimeError:
        return timezone.make_aware(value, pytz.UTC) if getattr(settings, 'USE_TZ', False) else value

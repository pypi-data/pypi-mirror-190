...
def _timezone_format(value):
    ...
    """
    try:
        return timezone.make_aware(value, timezone.get_current_timezone()) if getattr(settings, 'USE_TZ', False) else\
            value
    except NonExistentTimeError:
        return timezone.make_aware(value, pytz.UTC) if getattr(settings, 'USE_TZ', False) else \
            value
...
class FieldTypeGuesser(object):
    ...
    def guess_format(self, field, formatters=None):
        """
        Returns the correct faker function based on the field type
        :param field:
        :param formatters: optional dict with field as key and callable as value
        """
        faker = self.faker
        provider = self.provider

        if formatters and field.name in formatters:
            return formatters[field.name]

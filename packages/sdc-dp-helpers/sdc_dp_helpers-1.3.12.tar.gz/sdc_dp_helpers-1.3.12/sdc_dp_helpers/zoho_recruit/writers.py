...
class ZohoRecruitWriter(BaseWriter):
    ...
    def verify_data(
        ...
        """
        if not {"data", "end_point", "module", "start_page"}.issubset(
            set(payload.keys())
        ):
            raise KeyError("invalid payload expecting: data,end_point, option")
        ...
        end_point: str = payload["end_point"]
        start_page: int = payload["start_page"]
        date: str = payload["date"]

        write_path: str = f"{self.folder_path}/{end_point}/{module}/{date}_{start_page}"

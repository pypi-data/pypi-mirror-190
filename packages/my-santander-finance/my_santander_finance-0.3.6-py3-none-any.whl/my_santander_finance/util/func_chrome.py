...
def extract_version_registry(output):
    try:
        google_version = ""
        for letter in output[output.rindex("DisplayVersion    REG_SZ") + 24 :]:  # noqa E203
            if letter != "\n":
    ...
    try:
        if platform == "linux" or platform == "linux2":
            ...
            try:
                # Try registry key.
                stream = os.popen(
                    'reg query "HKLM\\SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Google Chrome"'  # noqa E501
                )
                output = stream.read()
...
if __name__ == "__main__":
    # la version esta definida con los 3 primeros caracteres
    # ejemplo: 103.0.5060.114 => version 103

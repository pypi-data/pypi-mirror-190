...
class RedshiftOperator:
    ...
    def get_current_structure(self) -> List:
        ...
        dict_datatype = {'character varying' : 'String',
                          'double precision': 'FLOAT'}
        lst_current_structure = [(row[2], eval(dict_datatype[row[3]]) if row[3] in dict_datatype else eval(row[3].upper()) )
    ...
    def check_structure_consistency(self) -> Set:
        ...
        lst_new_structure = [(column_name, aws_type)
                             for column_name, aws_type in self._aws_type.items()]
    ...
    def update_structure(self, iter_new_structure: Set) -> None:
        ...
        for column, new_aws_type in iter_new_structure:
            try:
                ...
                if str(ex).startswith('(psycopg2.errors.InternalError_)') or str(ex).startswith('(psycopg2.errors.CannotCoerce)'):
                    raise Exception(f"❌ You cannot convert the column {column} as {tmp_column.type}. Please, overwrite this table or rename the column with a different name")
                else:
                    raise Exception(ex)

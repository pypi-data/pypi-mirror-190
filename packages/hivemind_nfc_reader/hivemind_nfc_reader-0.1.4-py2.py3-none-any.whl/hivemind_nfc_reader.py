...
def on_button_edge_detect(channel):
    if GPIO.input(channel) == BUTTON_PRESSED:
        ...
        if state["last_button_up"].get(channel, datetime.min) > datetime.now() - timedelta(seconds=1):
            return
        if state["last_button_down"].get(channel, datetime.max) < datetime.now() - timedelta(seconds=1):
            return
    ...
    while True:
        try:
            ...
            for button, button_down_time in state["last_button_down"].items():
                ...
                logger.info("Button {} held for 1 sec".format(button))

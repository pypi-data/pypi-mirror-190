...
REQUIREMENTS = {
    "Cycle": {
        "inputs": [{
            ...
            "optional": {
                "properties": [
                    {"@type": "Property", "value": "", "term.@id": ["ndfContent", "energyContentHigherHeatingValue"]}
                ]
            }
        }]
    }
...
LOOKUPS = {
    "liveAnimal-ipcc2019Tier2Ch4": ["Digestibility", "Ym", "SD"],
    "crop-property": ["ndfContent", "energyContentHigherHeatingValue"]
}
RETURNS = {
    "Emission": [{
        "value": "",
...
def _should_run(cycle: dict):
    ...
    NDF = _get_crop_property_average(cycle, inputs, 'ndfContent')

    enteric_factor = safe_parse_float(_get_liveAnimal_lookup_value(lookup, term_id, 'Ym', DE, NDF, ionophore)) / 100
    enteric_sd = safe_parse_float(_get_liveAnimal_lookup_value(lookup, term_id, 'SD', DE, NDF, ionophore), None)

...
class SenClu():
    ...
    def getEncodedSentences(self,docs,device):
        ...
        for i,d in enumerate(docs_segmented):
            if len(d)==0:
                if self.verbose and nFailed<4: print(" Removing empty sentence embedded doc; Original doc ID",i," Original length:",len(docs[i]), " Content enclosed in '--' (e.g., it might contain only 'return characters'):  --"+docs[i]+"--")
                nFailed+=1

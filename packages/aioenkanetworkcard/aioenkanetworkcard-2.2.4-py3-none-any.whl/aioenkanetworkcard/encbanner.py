...
async def upload():
    async with EnkaNetworkAPI(agent= "ENC Library: 2.2.3") as ena:
...
class ENC:
    ...
    def __init__(self,lang = "ru", characterImgs = None,
            ...
            randomImg = False, hide = False, save = False, nameCards = False, splashArt = False, miniInfo = True, agent = "Library: 2.2.3") :
    ...
    async def enc(self,uids = None):
        result = {}
        uids = uidCreat(uids)
        async with EnkaNetworkAPI(agent = self.USER_AGENT, lang=self.lang) as client:
            for uid in uids:

...
class FeatureService(unified_common.BaseTectonObject, internal_fco.Fco):
    ...
    def _validate(self) -> bool:
        ...
        validations_api.print_validating_dependencies(self, len(self._feature_definitions))
        dependent_specs = []
        for feature_definition in self._feature_definitions:
            validation_passed = feature_definition._validate()
        ...
        if validation_response.success:
            supplement = specs.FeatureServiceSpecArgsSupplement(
                ids_to_feature_views={
                    spec.id: spec for spec in dependent_specs if isinstance(spec, specs.FeatureViewSpec)
                }
            )

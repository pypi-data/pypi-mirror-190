...
class FeatureView(unified_common.BaseTectonObject, declarative_base.BaseFeatureDefinition):
    ...
    def _get_dependent_objects(self) -> List[unified_common.BaseTectonObject]:
        raise NotImplementedError

    def _validate_dependencies(self) -> bool:
        dependent_objects = self._get_dependent_objects()
        validations_api.print_validating_dependencies(self, len(dependent_objects))
        for dependent_object in dependent_objects:
            validation_passed = dependent_object._validate()
    ...
    def _validate(self) -> bool:
        ...
        all_dependencies_valid = self._validate_dependencies()
        if not all_dependencies_valid:
            return False
        ...
        validations_api.print_validation_success(self)

        # TODO(jake): Return the real validation result.
        return True
...
class MaterializedFeatureView(FeatureView):
    ...
    def _check_can_query_from_source(self, from_source: Optional[bool]):
        if self._feature_definition.is_incremental_backfill:
            ...
            if self._feature_definition.materialization_enabled:
            ...
            if not self._feature_definition.materialization_enabled:
                raise errors.FD_GET_MATERIALIZED_FEATURES_FROM_DEVELOPMENT_WORKSPACE_GFD(self.name)
...
class OnDemandFeatureView(FeatureView):
    ...
    def _get_dependent_objects(self) -> List[unified_common.BaseTectonObject]:
        return self._get_dependent_feature_views() + list(self.transformations)
    ...
    def get_online_features(
        ...
        required_request_context_keys = list(request_context)

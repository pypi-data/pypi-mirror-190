...
class FeatureSetItemSpec:
    ...
    def from_args_proto(
        ...
        feature_view_spec = supplement.ids_to_feature_views[feature_view_id]

        override_mapping = {
            column_pair.feature_column: column_pair.spine_column for column_pair in proto.override_join_keys
        }
        join_key_mappings = []
        for join_key in feature_view_spec.join_keys:
        ...
        if len(proto.features) > 0:
            ...
            # TODO(TEC-12599): This will not work for aggregate features.
            # TODO(jake): Make sure that feature service validation verifies that the selected features exist in the
            #  feature view schema.
            features = tuple(feature_view_spec.view_schema.column_names())

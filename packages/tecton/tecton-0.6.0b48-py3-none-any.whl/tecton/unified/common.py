...
class BaseTectonObject:
    ...
    def _validate(self) -> bool:
        """Internal validation method. Will not print validation status updates for objects that are already valid."""
        raise NotImplementedError
    ...
    def validate(self) -> bool:
        ...
        (e.g. `my_ds = BatchSource(name="my_ds", batch_config=FileConfig(...))`) must be validated before many of their
        methods can be called, e.g. `get_historical_features()`.

        :returns True if validation succeeed. False if validation failed.
        """
        if self._is_valid:
            print(f"{self.__class__.__name__} '{self.name}' has been successfully validated. (Cached)")
            return True
        else:

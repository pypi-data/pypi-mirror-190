...
MAX_SIZE_TEXT = 0
cache_text = OrderedDict()  


def fetch_text(id, text):
    try:  # 캐시에서 마지막 채팅 시간
        text2 = cache_text[id] + text
    except:  # 없으면 현재 시간
        text2 = text
    return text2
...
def save_text(id, text):
    if id in cache_text:
        cache_text.move_to_end(id)  # 가장 최근에 사용된 아이템은 캐쉬의 맨 뒤로 이동

    if len(text) >= MAX_SIZE_TEXT:
        cache_text.popitem(last=False)  # 캐시 용량이 꽉 찾을 때는 캐쉬에 맨 앞에 있는 아이템 삭제

    cache_text[id] = fetch_text(id, text)

...
class UnsupervisedGUI(object):
    def __init__(self,
        ...
        self.scaling_dropdown = DropDownMenu(settings_frm, 'SCALING:', self.normalization_options, '12')
        ...
        self.feature_file_selected.grid(row=0, column=0, sticky=NW)
    ...
    def show_algo_hyperparameters(self):
        ...
        if self.choose_algo_dropdown.getChoices() == 'UMAP':
            ...
            run_frm = LabelFrame(self.dim_reduction_frm, text='RUN', pady=5, padx=5, font=Formats.LABELFRAME_HEADER_FORMAT.value, fg='black')
            run_btn = Button(run_frm, text='RUN', fg='blue', command= lambda: self.run_umap())
            run_frm.grid(row=5, column=0, sticky=NW)
    ...
    def run_umap(self):
        min_distances = [float(x) for x in self.min_distance_listb.get(0, END)]
        n_neighbours = [float(x) for x in self.n_neighbors_estimators_listb.get(0, END)]
        dimensions = [float(x) for x in self.n_neighbors_estimators_listb.get(0, END)]

        if len(min_distances) == 0 or len(n_neighbours) == 0 or len(dimensions) == 0:
            print('ERROR, GIVE VALUES')
        else:
            umap_embedder()

...
class UnsupervisedGUI(object):
    ...
    def show_algo_hyperparameters(self):
        ...
        if self.choose_algo_dropdown.getChoices() == 'UMAP':
            ...
            self.n_dimensions_listb = Listbox(self.hyperparameters_frm, bg='grey', fg='black', height=5, width=10)
            self.min_distance_listb = Listbox(self.hyperparameters_frm, bg='grey', fg='black', height=5, width=10)
            self.hyperparameters_frm.grid(row=3, column=0, sticky=NW)
            n_neighbors_estimators_listb.grid(row=0, column=0, sticky=NW)
            n_dimensions_listb.grid(row=0, column=1, sticky=NW)
            min_distance_listb.grid(row=0, column=2, sticky=NW)

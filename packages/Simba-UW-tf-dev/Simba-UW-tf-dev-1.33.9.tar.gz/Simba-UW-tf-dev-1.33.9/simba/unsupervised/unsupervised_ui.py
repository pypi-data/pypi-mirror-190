...
class UnsupervisedGUI(object):
    ...
    def show_algo_hyperparameters(self):
        ...
        if self.choose_algo_dropdown.getChoices() == 'UMAP':
            ...
            self.value_entry_box = Entry_Box(self.value_frm, 'VALUE: ', '12', validation='numeric')
            ...
            add_min_distance_btn = self.create_btn = Button(self.hyperparameters_frm, text='ADD', fg='blue', command= lambda x: self.add_to_listbox(list_box=self.min_distance_listb))
            ...
            self.n_neighbors_estimators_listb.grid(row=3, column=0, sticky=NW)
            self.n_dimensions_listb.grid(row=3, column=1, sticky=NW)
            self.min_distance_listb.grid(row=3, column=2, sticky=NW)

...
class UnsupervisedGUI(object):
    ...
    def show_algo_hyperparameters(self):
        ...
        if self.choose_algo_dropdown.getChoices() == 'UMAP':
            ...
            add_min_distance_btn = self.create_btn = Button(self.hyperparameters_frm, text='ADD', fg='blue', command= lambda: None)
            add_dimensions_btn = self.create_btn = Button(self.hyperparameters_frm, text='ADD', fg='blue', command=lambda: None)
            add_neighbours_btn = self.create_btn = Button(self.hyperparameters_frm, text='ADD', fg='blue', command=lambda: None)
            self.n_neighbors_estimators_listb = Listbox(self.hyperparameters_frm, bg='grey', fg='black', height=5, width=10, activestyle='dotbox')
            self.n_dimensions_listb = Listbox(self.hyperparameters_frm, bg='grey', fg='black', height=5, width=15, activestyle='dotbox')
            self.min_distance_listb = Listbox(self.hyperparameters_frm, bg='grey', fg='black', height=5, width=15, activestyle='dotbox')
            self.hyperparameters_frm.grid(row=3, column=0, sticky=NW)
            n_neighbors_estimators_lbl.grid(row=0, column=0)
            min_distance_lbl.grid(row=0, column=1)
            dimensions_lbl.grid(row=0, column=2)
            add_neighbours_btn.grid(row=1, column=0)
            add_dimensions_btn.grid(row=1, column=1)
            add_min_distance_btn.grid(row=1, column=2)
            self.n_neighbors_estimators_listb.grid(row=1, column=0, sticky=NW)
            self.n_dimensions_listb.grid(row=1, column=1, sticky=NW)
            self.min_distance_listb.grid(row=1, column=2, sticky=NW)

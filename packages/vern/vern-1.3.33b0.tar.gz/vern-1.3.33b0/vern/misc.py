            ...
            if self.linear_regression:
                ...
                if self.subplots:
                    if self.reference:
                        ...
                        reference_df.plot(kind="line", x=keys[0], ax=self.ax, subplots=[[reference_df.keys()[1+i], reference_df.keys()[1+(len(keys)-2)+i]] for i in range(len(keys)-2)], layout=(self.subplot_x,self.subplot_y), sharex=False, sharey=True, xlabel=keys[0].split("\t")[0], ylabel=keys[0].split("\t")[1], style=["orange"]*2+["b"]*(len(keys)-2))
                    else:

...
def read_files(input_path, csv_path, plot_path, interactive=False, linear_regression=False, wide=False, subplots=False, reference=False):
    df = pd_read_csv( 
        filename=input_path, encoding="utf-8", sep=",", header_count=0,
        rename_columns=False,
    )
    df.to_csv(csv_path, index=False)
    p = Plot(df, interactive=interactive, linear_regression=linear_regression, wide=wide, subplots=subplots, reference=reference)
    p.plot(plot_path)
...
def parse_tabular(**kwargs):
    ...
    read_files(kwargs["input_path"], kwargs["csv_path"], kwargs["plot_path"], interactive=interactive, linear_regression=linear_regression, wide=wide, subplots=subplots, reference=reference)

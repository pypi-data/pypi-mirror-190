...
class Client():
    ...
    async def Signin(self, username:str=None, password:str=None, ping:bool=True):
        ...
        if(password== None or password == ""):
            request.data.Pack(base_pb2.signin(jwt=username, ping=ping, agent="pyclient"))
        else:
            request.data.Pack(base_pb2.signin(username=username, password=password, ping=ping, agent="pyclient"))
        result:base_pb2.signinreply = await protowrap.RPC(self, request)
        self.jwt = result.jwt
    ...
    async def GetElement(self, xpath:str):
        ...
        result:base_pb2.getelement = await protowrap.RPC(self, request)
        return result.xpath
    async def RegisterQueue(self, queuename:str, callback):
        ...
        result:queues_pb2.registerqueuereply = await protowrap.RPC(self, request)
    ...
    async def QueueMessage(self, queuename:str, payload, correlationId=None, striptoken=True, rpc=False):
        ...
        reply.data.Pack(queues_pb2.queuemessage(queuename=queuename, data=res, striptoken=striptoken, correlationId=correlationId, replyto=replyto))
        self.grpcqueue.put(reply)
        if(future!=None):
            msg:queues_pb2.queuemessagereply = await future
    ...
    async def PushWorkitem(self, wiq:str, name:str, payload:dict, files: any = None, wiqid:str = None, nextrun: datetime = None, priority: int = 2, compressed: bool = False):
        ...
        request.data.Pack(q)
        result:workitems_pb2.pushworkitemreply = await protowrap.RPC(self, request)
    ...
    async def PopWorkitem(self, wiq:str,includefiles:bool=False,compressed:bool=False):
        ...
        result:workitems_pb2.popworkitemreply = await protowrap.RPC(self, request)
    ...
    async def UpdateWorkitem(self, workitem, files: any = None, compressed:bool=False):
        ...
        result:workitems_pb2.updateworkitemreply = await protowrap.RPC(self, request)
        if(result == None): return None
        return result.workitem;
    async def Query(self, collectionname:str="entities", query:dict={}, projection:dict={},top:int=100,skip:int=0, orderby=None, queryas:str=None):
        ...
        request.data.Pack(querys_pb2.query(
        ...
        result:querys_pb2.queryreply = await protowrap.RPC(self, request)
    ...
    async def ListCollections(self, includehist:bool=False):
        ...
        request.data.Pack(querys_pb2.listcollections(includehist=includehist))
        result:querys_pb2.listcollectionsreply = await protowrap.RPC(self, request)
        if(result == None or result.results == None): return None
        return json.loads(result.results)
    async def DropCollection(self, collectionname:str):
        request = base_pb2.envelope(command="dropcollection")
        request.data.Pack(querys_pb2.dropcollection(collectionname=collectionname))
    ...
    async def GetDocumentVersion(self, id:str, collectionname:str, version:int = 0, decrypt:bool=True):
        ...
        request.data.Pack(querys_pb2.getdocumentversion(collectionname=collectionname, id=id, version=version, decrypt=decrypt))
        result:querys_pb2.getdocumentversionreply = await protowrap.RPC(self, request)
    ...
    async def Count(self, query:dict={}, collectionname:str="entities", queryas:str=None):
        ...
        request.data.Pack(querys_pb2.count(
            collectionname=collectionname, query=json.dumps(query), queryas=queryas))
        result:querys_pb2.countreply = await protowrap.RPC(self, request)
    ...
    async def InsertMany(self, items:list(), collectionname:str="entities", skipresults:bool=False):
        ...
        request.data.Pack(querys_pb2.insertmany(
            collectionname=collectionname, items=json.dumps(items), skipresults=skipresults))
        result:querys_pb2.insertmanyreply = await protowrap.RPC(self, request)
        if(result == None or result.results == None): return None
        return json.loads(result.results)
    async def UpdateOne(self, item:dict, collectionname:str="entities"):
        request = base_pb2.envelope(command="updateone")
        request.data.Pack(querys_pb2.updateone(
            collectionname=collectionname, item=json.dumps(item)))
        result:querys_pb2.updateonereply = await protowrap.RPC(self, request)
        if(result == None or result.result == None): return None
        return json.loads(result.result)
    async def UpdateDocument(self, query:dict, document:dict, collectionname:str="entities"):
        ...
        request.data.Pack(querys_pb2.updatedocument(
            collectionname=collectionname, query=json.dumps(query), document=json.dumps(document)))
        result:querys_pb2.updatedocumentreply = await protowrap.RPC(self, request)
    ...
    async def InsertOrUpdateOne(self, item:dict, collectionname:str="entities", uniqeness:str="_id"):
        ...
        request.data.Pack(querys_pb2.insertorupdateone(
            collectionname=collectionname, uniqeness=uniqeness, item=json.dumps(item)))
        result:querys_pb2.insertorupdateonereply = await protowrap.RPC(self, request)
        if(result == None or result.result == None): return None
        return json.loads(result.result)
    async def InsertOrUpdateMany(self, items:dict, collectionname:str="entities", uniqeness:str="_id", skipresults:bool=False):
        ...
        request.data.Pack(querys_pb2.insertorupdatemany(
            collectionname=collectionname, uniqeness=uniqeness, items=json.dumps(items), skipresults=skipresults))
        result:querys_pb2.insertorupdatemanyreply = await protowrap.RPC(self, request)
        if(result == None or result.results == None): return None
        return json.loads(result.results)
    async def DeleteOne(self, id:str, collectionname:str="entities", recursive:bool=False):
        ...
        request.data.Pack(querys_pb2.deleteone(
            collectionname=collectionname, id=id, recursive=recursive))
        result:querys_pb2.deleteonereply = await protowrap.RPC(self, request)
        if(result == None): return 0
    ...
    async def DeleteMany(self, query:dict, collectionname:str="entities", recursive:bool=False):
        ...
        request.data.Pack(querys_pb2.deletemany(
            collectionname=collectionname, query=json.dumps(query), recursive=recursive))
        result:querys_pb2.deletemanyreply = await protowrap.RPC(self, request)
        if(result == None): return 0
        return result.affectedrows
    async def Watch(self, collectionname, paths:list, callback):
        request = base_pb2.envelope(command="watch")
        request.data.Pack(watch_pb2.watch (collectionname=collectionname, paths=paths))
        result:watch_pb2.watchreply = await protowrap.RPC(self, request)
        self.watches[result.id] = callback
        return result.id
    async def UnWatch(self, id:str):
        request = base_pb2.envelope(command="unwatch")
        request.data.Pack(watch_pb2.unwatch (id=id))
        result:watch_pb2.unwatchreply = await protowrap.RPC(self, request)

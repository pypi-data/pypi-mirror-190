...
class MetricVisualizer:
    ...
    def log_metric(self, trial_name=None, metric_name=None, value=0, unit=None):
        ...
        if trial_name is None:
            trial_name = "Trial{}".format(len(self.metrics[metric_name]) + 1 if metric_name in self.metrics else 1)
    ...
    def box_plot(self, by='trial', engine='matplotlib', save_path=None, show=True, **kwargs):
        """
        ...
        num_metrics = len(self.metrics.keys())
        # get the number of trials
        num_trials = len(self.metrics[list(self.metrics.keys())[0]].keys())
        ...
        for i, metric_name in enumerate(metrics.keys()):
            ...
            ax.boxplot(
                ...
                medianprops=dict(color=kwargs.get("median_color", "black")),
    ...
    def trajectory_plot(self, by='trial', engine='matplotlib', save_path=None, show=True, **kwargs):
        ...
        for i, metric_name in enumerate(metrics.keys()):
            ...
            elif kwargs.get("agg", 'mean') == 'median':
                ax.plot(
                    xticks + i * width,
            ...
            # draw the error bar
            if kwargs.get("errorbar", True):
                ax.errorbar(
                    xticks + i * width,
                    np.average(values, axis=-1),
                    yerr=np.std(values, axis=-1),
                    color=colors[i],
                    **kwargs.get("errorbar_kwargs", {})
                )
            # draw the scatter plot
    ...
    def bar_plot(self, by='trial', engine='matplotlib', save_path=None, show=True, **kwargs):
        ...
        for i, metric_name in enumerate(metrics.keys()):
            # get the values
            values = np.array(list(metrics[metric_name].values()))
            # draw the bar plot
            ax.bar(
                xticks + i * width,
                np.average(values, axis=-1),
    ...
    def sk_rank_plot(self, plot_type='box', engine='matplotlib', save_path=None, show=True, **kwargs):
        """
        ...
        if plot_type == 'box':
            return mv.box_plot(
                ...
                save_path=save_path,
                show=show,
                xlabel=kwargs.get("xlabel", "Trial"),
    ...
    def _get_table_data(self, **kwargs):
        ...
        for mn in transposed_metrics.keys():
            ...
            for i, trial in enumerate(metrics.keys()):
                ...
                _data[-1].append(round(np.percentile(metrics[trial], 75) - np.percentile(metrics[trial], 25), 2))

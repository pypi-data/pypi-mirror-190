...
TEST(types_bytes_get_set, "as_bytes getting and setting")
{
    ...
    assert_false(
        as_bytes_set(&b, test_len + 2, &v, 1)); // should fail out of bounds
    assert_false(
        as_bytes_copy(&b, test_len + 2, &v, 1)); // should fail out of bounds
    assert_false(
        as_bytes_copy(&b, 0, &v, test_len + 2)); // should fail out of bounds
    assert_false(as_bytes_copy(&b, test_len / 2, &v,
                               test_len)); // should fail out of bounds

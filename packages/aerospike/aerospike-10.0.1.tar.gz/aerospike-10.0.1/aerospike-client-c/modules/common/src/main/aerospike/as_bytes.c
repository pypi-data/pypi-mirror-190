...
static inline as_bytes *as_bytes_cons(as_bytes *bytes, bool free,
                                      uint32_t capacity, uint32_t size,
                                      uint8_t *value, bool value_free,
                                      as_bytes_type type)
{
    if (!bytes)
        return bytes;
 ...
 *	@return On success, the initializes bytes. Otherwise NULL.
 */
as_bytes *as_bytes_init_wrap(as_bytes *bytes, uint8_t *value, uint32_t size,
                             bool free)
{
 ...
 */
uint32_t as_bytes_copy(const as_bytes *bytes, uint32_t index, uint8_t *value,
                       uint32_t size)
{
 ...
 */
uint32_t as_bytes_get_var_int(const as_bytes *bytes, uint32_t index,
                              uint32_t *value)
{
 ...
 */
bool as_bytes_set(as_bytes *bytes, uint32_t index, const uint8_t *value,
                  uint32_t size)
{
    if (index + size > bytes->capacity)
        return false;
 ...
 *	@relatesalso as_bytes
 */
uint32_t as_bytes_set_var_int(const as_bytes *bytes, uint32_t index,
                              uint32_t value)
{
    uint8_t *begin = bytes->value + index;
...
bool as_bytes_to_string(const uint8_t *bytes, uint32_t bytes_size, char *str,
                        uint32_t str_size)
{
...
bool as_bytes_to_string_with_prefix(const uint8_t *bytes, uint32_t bytes_size,
                                    char *str, uint32_t str_size)
{
...
static inline uint8_t hex_char_to_byte(const char c)
{
    if (c >= '0' && c <= '9') {
        return (uint8_t)(c - '0');
    }
...
int as_bytes_from_string(uint8_t *bytes, uint32_t bytes_size, const char *str)
{

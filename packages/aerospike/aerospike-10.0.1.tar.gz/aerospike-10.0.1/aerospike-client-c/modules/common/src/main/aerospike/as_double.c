...
static as_double *as_double_cons(as_double *value_ptr, bool free, double value)
{
    if (!value_ptr) {
        return value_ptr;
    }
...
void as_double_val_destroy(as_val *val)
{
...
uint32_t as_double_val_hashcode(const as_val *val)
{
    as_double *value_ptr = as_double_fromval(val);
...
char *as_double_val_tostring(const as_val *val)
{

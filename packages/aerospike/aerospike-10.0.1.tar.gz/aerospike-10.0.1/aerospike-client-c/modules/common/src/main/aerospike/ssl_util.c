...
static apr_array_header_t *as_apr_array_make(apr_pool_t *pp, size_t nelts,
                                             size_t eltsz)
{
...
static char *asn1_string_to_utf8(apr_pool_t *p, ASN1_STRING *asn1str)
{
    ...
    ASN1_STRING_print_ex(bio, asn1str,
...
static char *modssl_X509_NAME_ENTRY_to_string(apr_pool_t *p,
                                  ...
                                  const char *onf, apr_array_header_t **entries)
{
    ...
    else if (strEQ(onf, "id-on-dnsSRV") && (value->type == V_ASN1_IA5STRING) &&
 ...
 */
static BOOL modssl_X509_getSAN(apr_pool_t *p, X509 *x509, int type,
                               const char *onf, int idx,
                               apr_array_header_t **entries)
{
    ...
    if (!x509 || (type < GEN_OTHERNAME) ||
        ((type == GEN_OTHERNAME) && (nid == NID_undef)) || (type > GEN_RID) ||
        (idx < -1) || !(*entries = as_apr_array_make(p, 0, sizeof(char *)))) {
        *entries = NULL;
        return FALSE;
    }
        ...
        for (i = 0; i < sk_GENERAL_NAME_num(names); i++) {
            ...
            case GEN_OTHERNAME:
                if (OBJ_obj2nid(name->d.otherName->type_id) == nid) {
                    if (((idx == -1) || (n == idx))) {
                        parse_otherName_value(p, name->d.otherName->value, onf,
                                              entries);
                    }
                    n++;
                }
                                   ...
                                   BOOL allow_wildcard, server_rec *s)
{
    ...
    if (!matched) {
        as_log_warn("Cert %s for name '%s'",
                    matched == TRUE ? "matches" : "does not match", name);
    }
#endif
...
bool as_tls_match_name(X509 *x509, const char *name, bool allow_wildcard)
{
    ...
    //
    return modssl_X509_match_name((apr_pool_t *)NULL, x509, name,
                                  allow_wildcard, (server_rec *)NULL);
}

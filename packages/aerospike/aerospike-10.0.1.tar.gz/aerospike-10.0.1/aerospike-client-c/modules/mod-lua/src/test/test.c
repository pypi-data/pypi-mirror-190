...
{
    atf_test_result *res =
        (atf_test_result *)cf_malloc(sizeof(atf_test_result));
    res->test = test;
    res->success = true;
    res->message[0] = '\0';
    return res;
...
uint32_t atf_suite_size(atf_suite *suite) { return suite->size; }
...
void atf_suite_result_print(atf_suite_result *suite_result)
{
    if (suite_result->success < suite_result->size) {
        printf("[✘] %s: %d/%d tests passed.\n", suite_result->suite->name,
               suite_result->success, suite_result->size);
...
{
    atf_suite_result *res =
        (atf_suite_result *)cf_malloc(sizeof(atf_suite_result));
    res->suite = suite;
    res->size = 0;
    res->success = 0;
    return res;
...
void atf_assert(atf_test_result *result, const char *exp, const char *file,
                int line)
{
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s [at %s:%d]", exp, file, line);
    result->success = false;
}

void atf_assert_true(atf_test_result *result, const char *exp, const char *file,
                     int line)
{
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s is not true. [at %s:%d]", exp, file, line);
    result->success = false;
}

void atf_assert_false(atf_test_result *result, const char *exp,
                      const char *file, int line)
{
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s is not false. [at %s:%d]", exp, file, line);
    result->success = false;
}

void atf_assert_null(atf_test_result *result, const char *exp, const char *file,
                     int line)
{
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s is not NULL. [at %s:%d]", exp, file, line);
    result->success = false;
}

void atf_assert_not_null(atf_test_result *result, const char *exp,
                         const char *file, int line)
{
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s is NULL. [at %s:%d]", exp, file, line);
    result->success = false;
}

void atf_assert_int_eq(atf_test_result *result, const char *actual_exp,
                       int64_t actual, int64_t expected, const char *file,
                       int line)
{
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s == %" PRId64 ", when %" PRId64
             " was expected. [at %s:%d]",
             actual_exp, actual, expected, file, line);
    result->success = false;
}

void atf_assert_int_ne(atf_test_result *result, const char *actual_exp,
                       int64_t actual, int64_t expected, const char *file,
                       int line)
{
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s == %" PRId64
             ", when it shouldn't be. [at %s:%d]",
             actual_exp, actual, file, line);
    result->success = false;
}

void atf_assert_double_eq(atf_test_result *result, const char *actual_exp,
                          double actual, double expected, const char *file,
                          int line)
{
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s == %f when %f was expected. [at %s:%d]",
             actual_exp, actual, expected, file, line);
    result->success = false;
}

void atf_assert_string_eq(atf_test_result *result, const char *actual_exp,
                          const char *actual, const char *expected,
                          const char *file, int line)
{
    snprintf(
        result->message, sizeof(result->message),
        "assertion failed: %s == \"%s\", when \"%s\" was expected. [at %s:%d]",
        actual_exp, actual, expected, file, line);
    result->success = false;
}

void atf_assert_log(atf_test_result *result, const char *exp, const char *file,
                    int line, const char *fmt, ...)
{
    ...
    snprintf(result->message, sizeof(result->message),
             "assertion failed: %s. %s [at %s:%d]", exp, msg, file, line);
    result->success = false;
}
...
void atf_log(FILE *f, const char *level, const char *prefix, const char *file,
             int line, const char *fmt, ...)
{
    char msg[LOG_MESSAGE_MAX];
    va_list ap;
...
void atf_log_line(FILE *f, const char *level, const char *prefix,
...
void atf_log_linev(FILE *f, const char *level, const char *prefix,

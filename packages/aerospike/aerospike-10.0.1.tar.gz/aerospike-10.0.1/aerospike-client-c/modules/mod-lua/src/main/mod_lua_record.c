...
as_rec *mod_lua_pushrecord(lua_State *l, as_rec *r)
{
    ...
    mod_lua_box *box = mod_lua_pushbox(
        l, r->_.free ? MOD_LUA_SCOPE_LUA : MOD_LUA_SCOPE_HOST, r, CLASS_NAME);
    return (as_rec *)mod_lua_box_value(box);
}
...
void bin_names_callback(char *bin_names, uint32_t nbins, uint16_t max_name_size,
                        void *udata)
{

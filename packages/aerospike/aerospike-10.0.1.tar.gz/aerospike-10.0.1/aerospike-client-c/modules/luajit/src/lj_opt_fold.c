...
{
    return INT64FOLD(kfold_int64arith(ir_k64(fleft)->u64, ir_k64(fright)->u64,
                                      (IROp)fins->o));
}
...
{
    GCstr *s =
        lj_str_new(J->L, (const char *)ir_kptr(fleft), (size_t)fright->i);
...
LJFOLDF(kfold_strref_snew)
{
    ...
    if (irref_isk(fins->op2) && fright->i == 0) {
        ...
        PHIBARRIER(ir);
        fins->op2 =
            emitir(IRTI(IR_ADD), ir->op2, fins->op2); /* Clobbers fins! */
        fins->op1 = str;
...
LJFOLDF(reassoc_intarith_k64)
{
    ...
    IRIns *irk = IR(fleft->op2);
    if (irk->o == IR_KINT64) {
        uint64_t k = kfold_int64arith(ir_k64(irk)->u64, ir_k64(fright)->u64,
                                      (IROp)fins->o);
        PHIBARRIER(fleft);
        fins->op1 = fleft->op1;
...
retry:
    ...
    if (fins->op2 >= J->cur.nk) {
        ...
        *fright = *IR(fins->op2);
        ...
        key +=
            (fins->op2 & 0x3ffu); /* Literal mask. Must include IRCONV_*MASK. */
    }
...
TRef LJ_FASTCALL lj_opt_cse(jit_State *J)
{
    ...
    if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE)) {
        ...
        while (ref > lim) {
            if (IR(ref)->op12 == op12)
                return TREF(
                    ref, irt_t(IR(ref)->t)); /* Common subexpression found. */
            ref = IR(ref)->prev;
        }
    }

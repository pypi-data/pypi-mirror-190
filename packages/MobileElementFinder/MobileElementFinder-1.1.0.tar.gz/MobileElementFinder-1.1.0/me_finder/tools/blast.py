...
def run_blast(ctx: ExecutionContext, db_name: str, contigs: str) -> str:
    ...
    try:
        run_shell(blast_path, '-query', contigs, '-out', blast_result_path,
                  ...
                  '-word_size', ctx.cnf_get_default('getint', 'blast', 'word_size'), '-soft_masking',
                  ctx.cnf_get_default('getboolean', 'blast', 'soft_masking'), '-task',
                  ctx.cnf_get_default('get', 'blast', 'task'), '-outfmt', 15)

...
class InelsMqtt:
    ...
    def __init__(
        ...
        self.__entity_keys = dict[str, tuple[str, int]]()
    ...
    def __on_discover(
        ...
        inels_type = INELS_DEVICE_TYPE_DICT[device_type]
        ...
        if device_type in DEVICE_TYPE_DICT and status == "status":
            dev_val = DeviceValue(
                ...
                inels_type=inels_type,
                inels_value=msg.payload
            )

            self.__discovered[msg.topic] = msg.payload
            self.__last_values[msg.topic] = dev_val#msg.payload
        ...
        if len(self.__listeners) > 0 and msg.topic in self.__listeners:
            ...
            if len(self.__listeners[msg.topic]) > 0:
                for unique_id in list(self.__listeners[msg.topic]): #prevents the dictionary increased in size during iteration exception
                    ...
                    self.__listeners[msg.topic][unique_id](self.__messages[msg.topic].__dict__[key][index])

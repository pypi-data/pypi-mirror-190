...
class DeviceValue(object):
    ...
    def __find_ha_value(self) -> None:
        ...
        if self.__device_type is SWITCH:  # outlet switch
            ...
            elif self.__inels_type in [SA3_02M, SA3_04M, SA3_06M]:
                re = []
                for relay in self.__trim_inels_status_bytes(INELS_DEVICE_TYPE_DATA_STRUCT_DATA[self.__inels_type], RELAY):
                    re.append((int(relay, 16) & 1) != 0)
                
                #digital_inputs = self.__trim_inels_status_values(
                #    INELS_DEVICE_TYPE_DATA_STRUCT_DATA[self.__inels_type], SW, "")
                #digital_inputs = f"0x{digital_inputs}"
                #digital_inputs = f"{int(digital_inputs, 16):0>8b}"
                #sw = []
                #for i in range(SA3_AMOUNTS[self.__inels_type]):
    ...
    def __trim_inels_status_values(
        ...
        data = self._DeviceValue__inels_status_value

        if type(self._DeviceValue__inels_status_value) != str:
            data = self.decode("utf-8")
        data = data.split("\n")[:-1]

...
from meili_ros_lib.maths import quaternion_to_euler, convert_yaw_to_meili_angle
...
class Agent:
        ...
        try:
            for index, msg in enumerate(self.msg_pose):
                ...
                yaw = round(quaternion_to_euler(
                    ...
                    3)
                meili_angle = convert_yaw_to_meili_angle(yaw)
                self.send_message_client(
                    constants.EVENT_LOCATION,
                    {"xm": x, "ym": y, "rotation": meili_angle},
        ...
        try:
            for index, msg in enumerate(self.msg_speed):
                if isinstance(msg, Odometry):
                    ...
                    self.log_info("[ROSAgent] Robot speed is not available")
                    capture_message("[ROSAgent] Robot speed is not available")
        except Exception as e:
            self.log_info("[ROSAgent] Ping speed Error: %s ", e)
            capture_exception(e)
    ...
    def callback_status(self, msg, goal_id, status_id, vehicle_uuid):
        ...
        if status_id == 2 and self.task_started[vehicle_position] == 1:
            self.task_started[vehicle_position] = self.task_preempted(goal_id, vehicle_uuid)
    ...
    def task_preempted(self, goal_id, vehicle_uuid):
        ...
        self.log_info(f"[ROSAgent] Task Preempted with goal_id {goal_id}")
        return 0
    ...
    def check_distance_of_array(self, msg):
        ...
        for pose in msg.poses:
            ...
            if first:
                previous_point = [pose.pose.position.x, pose.pose.position.y]
                first = False
            distance += sqrt((point[0] - previous_point[0]) ** 2 + (point[1] - previous_point[1]) ** 2)
            path.append(point)
    ...
    def callback_plan(self, msg, vehicle_uuid):
        ...
        if self.task_started[vehicle_position]:

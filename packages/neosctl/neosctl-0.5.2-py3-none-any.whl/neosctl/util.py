...
def dumps_formatted_json(payload: typing.Dict) -> str:
    """Dump formatted json.
...
def send_output(msg: str, exit_code: int = 0) -> None:
...
def process_response(response: httpx.Response, render_callable: callable = prettify_json) -> None:
...
def get_schema_profile(
    ...
    **kwargs,
) -> typing.Union[schema.Profile, schema.OptionalProfile]:
    if allow_missing:
...
def get_user_profile_section(c: configparser.ConfigParser, profile_name: str, allow_missing: bool = False) -> dict:
    try:
...
def patch(ctx: typer.Context, url: str, **kwargs) -> httpx.Response:

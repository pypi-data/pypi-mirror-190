...
class TestRDependenciesWebSocket:
    ...
    async def test_rip_list(
        ...
        async with client.websocket_connect(
            ...
            data = await receive_json(websocket, timeout=WEBSOCKET_RECEIVE_TIMEOUT * 2)
    ...
    async def test_rip_unknown_command(
        ...
        async with client.websocket_connect(
            ...
            assert msgs[1]["status"] == "update"
            assert msgs[1]["request_id"] == request_id
            assert msgs[1]["stdout"] is None
            assert "unknown command: rip hello_world" in msgs[1]["stderr"]

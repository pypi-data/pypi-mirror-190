...
class EBusItem(object):
    def __init__(self, items, index, name, units, logger):
        if not name:
            name='io%d' % index
        self._items=items
        self._units=units
    ...
    def unit(self, unit):
        with self._lock:
            if isinstance(unit, str):
                unit=self._units.getByName(unit)
            if unit != self._unit:
                self._unit = unit
                self._updated=True
    ...
    def unitstr(self):
        try:
            return self._units.getByNumber(self.unit)
        except:
            return ''
...
class EBusInputItem(EBusItem):
    def __init__(self, items, index, name, units, logger):
        super(EBusInputItem, self).__init__(items, index, name, units, logger)
        self._input=True
...
class EBusOutputItem(EBusItem):
    def __init__(self, items, index, name, units, logger):
        super(EBusOutputItem, self).__init__(items, index, name, units, logger)
        self._input=False
...
class EBusItems(object):
    ...
    def createInput(self, index, name, signalChangeDelta=0):
        with self._lock:
            item=self.get(index)
            if not item:
                item=EBusInputItem(self, index, name, self._units, self._logger)
    ...
    def createOutput(self, index, name, signalChangeDelta=0, unit=0xFF):
        with self._lock:
            item=self.get(index)
            if not item:
                item=EBusOutputItem(self, index, name, self._units, self._logger)
                item.setDeltaSignalChange(signalChangeDelta)
                item.unit=unit
                self._items[index]=item
    ...
    def table(self):
        if self.count()>0:
            with self._lock:
                ...
                for item in self.inputs():
                    t.add_row(['input', item.index, item.name, item.valuestr(), int(item.age())])
                for item in self.outputs():
                    t.add_row(['output', item.index, item.name, item.valuestr(), int(item.age())])

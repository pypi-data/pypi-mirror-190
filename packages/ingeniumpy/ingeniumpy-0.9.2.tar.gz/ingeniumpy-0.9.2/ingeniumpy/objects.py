...
class IngComponentType(Enum):
    ...
    def is_threephasemeter(self): return self in [90]
...
"""
Voltage: 2 bytes [101 + i*2, 102 + i*2]
Current: 2 bytes [107 + i*2, 108 + i*2]

Active_power: 2 bytes [113 + i*2, 114 + i*2]
Power_factor: 2 bytes [125 + i*2, 126 + i*2]

Frequency: 2 bytes [143, 144]

Active_energy: 4 bytes [145 + i*4, 146 + i*4, 147 + i*4, 148 + i*4]
Reactive_energy: 4 bytes [157, 158, 159, 160]

THDV: 2 bytes [161 + i*2, 162 + i*2]
THDI: 2 bytes [167 + i*2, 168 + i*2]
...
class IngThreePhaseMeter(IngObject):
    def __init__(self, *args):
        ...
        self._voltage = [NetworkNumber(2, 102 + i * 2) for i in range(2)]
        self._current = [NetworkNumber(2, 107 + i * 2) for i in range(2)]

        self._active_power = [NetworkNumber(2, 113 + i * 2) for i in range(2)]
        self._power_factor = [NetworkNumber(2, 125 + i * 2) for i in range(2)]

        self._frequency = NetworkNumber(2, 143)

        self._active_energy = [NetworkNumber(4, 145 + i * 4) for i in range(2)]
        self._reactive_energy = NetworkNumber(4, 157)

        self._thdv = [NetworkNumber(2, 161 + i * 2) for i in range(2)]
        self._thdi = [NetworkNumber(2, 167 + i * 2) for i in range(2)]
    ...
    async def update_state(self, p: Package) -> bool:
        ...
        for i, n in enumerate(self._voltage):
            if n.update_state(p):
                print("VOLTAGE({}): {}".format(i + 1, n.get_value()))
                return True
        for i, n in enumerate(self._current):
            if n.update_state(p):
                print("CURRENT({}): {}".format(i + 1, n.get_value()))
                return True
        for i, n in enumerate(self._active_power):
            if n.update_state(p):
                print("ACTIVE_POWER({}): {}".format(i + 1, n.get_value()))
                return True
        for i, n in enumerate(self._power_factor):
            if n.update_state(p):
                print("POWER_FACTOR({}): {}".format(i + 1, n.get_value()))
                return True
        if self._frequency.update_state(p):
            print("FREQUENCY: {}".format(self._frequency.get_value()))
            return True
        for i, n in enumerate(self._active_energy):
            if n.update_state(p):
                print("ACTIVE_ENERY({}): {}".format(i + 1, n.get_value()))
                return True
        if self._reactive_energy.update_state(p):
            print("REACTIVE_ENERGY: {}".format(self._reactive_energy.get_value()))
            return True
        for i, n in enumerate(self._thdv):
            if n.update_state(p):
                print("THDV({}): {}".format(i + 1, n.get_value()))
                return True
        for i, n in enumerate(self._thdi):
            if n.update_state(p):
                print("THDI({}): {}".format(i + 1, n.get_value()))
                return True

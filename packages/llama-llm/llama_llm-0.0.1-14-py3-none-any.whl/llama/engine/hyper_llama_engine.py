...
from llama.engine.nodes import CheckMatchNode, ValueNode, LlamaNode, MetricNode, GetFieldNode, Node
    ...
    def setup_input(self, output_node, nodes):
        ...
        for program_node in nodes:
            if type(program_node) == ValueNode:
                assert node is None, "TODO: support input setup for programs with multiple inputs"
                node = program_node

        #print("Setting up input for node", node)

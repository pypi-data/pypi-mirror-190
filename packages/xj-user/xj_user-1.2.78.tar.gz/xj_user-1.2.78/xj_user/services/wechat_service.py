    ...
    def wechat_login(self, phone_code, login_code, sso_serve_id=None, detail_params=None):
        ...
        if not current_user:
            ...
            try:
                ...
                saler = UserRelateToUser.objects.filter(user_id=current_user.get('id', None), user_relate_type_id=2).first()
                if saler:
                    data, err = UserRelateToUserService.add(
                        {
                            "user_id": current_user.get('id', None),
                ...
                res, err = RoleService.is_this_role(user_id=current_user.get('id', None), role_key="BID-SALESMAN")  # 如果是业务人员
                if res:
                    data, err = UserRelateToUserService.add(
                        {
                            "user_id": current_user.get('id', None),

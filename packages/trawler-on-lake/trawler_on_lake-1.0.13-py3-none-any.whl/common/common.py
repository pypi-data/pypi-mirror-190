...
def write_kafka_stream(stream: DataFrame, spark_job_conf: dict, output_name):
    ...
    if output_conf.get('processing_time') is not None:
        ...
        stream \
            ...
            .start() \
            .awaitTermination()
    elif output_conf.get("once") is not None:
        print(f'You choose to write into kafka stream with trigger.once={output_conf.get("once")}')
        stream \
            .writeStream \
            .format("kafka") \
            .options(**output_conf['kafka_options']) \
            .trigger(once=output_conf['once']) \
            .outputMode("append") \
            .start() \
            .awaitTermination()
    else:
...
def write_delta_stream(spark_job_conf: dict, output_name: str, stream: DataFrame):
    ...
    if output_stream_conf.get('processing_time') is not None:
        ...
        print(f'You choose to write into delta stream with trigger.once={output_stream_conf.get("once")}')
        stream \
            .writeStream \
            .format("delta") \
            .outputMode("append") \

...
class DeepSeqEurekaModel:
    ...
    def __init__(
        ...
        if data_dir:
            ...
            data_dir= f'{str(Path.home())}/myfiles/DeepEurekaModel_data'
        ...
        if parameter_dict:
            ...
            parameter_dict = {
            'user_inter_num_interval': f"[{interval},Inf)",
            'item_inter_num_interval': f"[{interval},Inf)",
        ...
        parameter_dict.update({'data_path': f'{self.data_dir}',
        ...
        data_rename = data.rename(columns={user_id: 'user_id:token', item_id: 'item_id:token', timestamp: 'timestamp:float'})
        data_rename[['user_id:token','item_id:token','timestamp:float']].to_csv(f'{data_dir}/{name}/{name}.inter', index=False, sep='\t')
    ...
    def _predict(
        ...
        for internal_user_id in list(range(nums))[1:]:
            _, topk_iid_list = full_sort_topk([internal_user_id], self.model, test_data, k=topk, device=self.config['device'])

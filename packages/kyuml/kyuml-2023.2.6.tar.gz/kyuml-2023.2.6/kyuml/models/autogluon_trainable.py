...
class AutogluonTrainable:
    ...
    def _fit(
        ...
        predictor = TabularPredictor(
            label=self.label, eval_metric=eval_metric, path=MODEL_DIR.joinpath(self.model_name, self.model_version)
        )
    ...
    def set_local_path(self) -> None:
        ...
        for model_name in trainer.get_model_names():
            trainer.set_model_attribute(
                model_name, "path", f"{MODEL_DIR}/{self.model_name}/{self.model_version}/models/{model_name}/"
            )

        ...
        for idx, group in enumerate(groups):
            ...
            if command:
                ...
                for idx2, module in enumerate(group):
                    ...
                    if stop_processing:
                        self.printer.normal(f"Processing aborted at step {group_nbr}.{module_nbr}.")
                        self.printer.normal(
    ...
    def deploy(
        ...
        try:
            ...
            for ss, substack in manifest['sub_stacks'].items():
                ...
                substacks.append(substack['source_path'].split(os.path.sep)[0])
            ...
            for key, value in manifest['deploy'].items():
                if target_stage not in value['applies_to_stages']:
                    ...
                    for module in source_modules:
                        if module in value['exclude_modules'] or module in substacks:
                            self.printer.verbose(f'Exclude module {module}')
                        elif module in value['include_modules'] or len(value['include_modules']) == 0:
                            deploy_actions[module] = {
                                "source": os.path.join(source_path, module, ''),
                    ...
                    for ss, substack in manifest['sub_stacks'].items():
                        ...
                        source_path = os.path.join(
                            source_dir, value['source_stage'], substack['source_path'], ''
                            )
                        target_path = os.path.join(
                            target_dir, substack['target_path'], ''
                            )

                        if os.path.exists(source_path):
                            deploy_actions[f'substack -> {substack["target_path"]}'] = {
                                "source": source_path,
                                "target": target_path,
                            }
                        else:

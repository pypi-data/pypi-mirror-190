...
from blazetest.core.run_test.runner_facade import TestRunner, TestSessionResult
...
def run_tests(
    ...
    test_session_result: TestSessionResult = test_runner.run_tests()

    s3_bucket_path = get_s3_bucket_path(
        s3_bucket=test_session_result.s3_bucket, timestamp=test_session_result.timestamp
    )

    logger.info(
        f"Lambda function: {test_session_result.lambda_function_name}, "
        f"S3 bucket: {test_session_result.s3_bucket}",
    )
    logger.info(f"Reports have been saved to {s3_bucket_path}")
    logger.info(
        f"Time taken to execute {test_session_result.tests_count} tests: {test_session_result.duration}"
    )
    logger.info(
        f"Initial run: passed tests: {test_session_result.tests_passed} out of {test_session_result.tests_count}"
    )
    logger.info(
        f"{test_session_result.failed_tests} tests have been retried, "
        f"{test_session_result.failed_tests_on_retry} have failed even after retrial"
    )
    logger.info(
        f"Summary: {test_session_result.tests_count - test_session_result.failed_tests_on_retry} have passed."
    )

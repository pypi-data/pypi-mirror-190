...
class TestRunner:
    ...
    def __init__(
        self, pytest_args: List[str], config: BlazetestConfig, retry_number: int = 1
    ):
        """
        ...
        :param retry_number: How many times the failed test will be retried
        """
    ...
    def run_tests(self) -> TestSessionResult:
        """
        ...
        if failed_tests:
            logger.info(
                f"Retrying running {len(failed_tests)} failed tests {self.retry_number} times"
            )
            retry_test_results = self.__run_in_parallel(
                node_ids=failed_tests * self.retry_number
            )
        ...
        return TestSessionResult(
            ...
            failed_tests_on_retry=failed_tests_on_retry,
        )

...
class Node:
    ...
    def add_connection(self, next_node_title: str, probability_of_success: float, parent_was_succesful: bool):
        ...
        if not connection:
            ...
            print('The node "' + self.title +
                  '" already has a connection to "' + next_node_title + '"')
            return False
    ...
    def delete_connection(self, node_title: str):
        ...
        if(before_i == len(self.connections)):
            print('There is no connection from node "' +
                  self.title + '" to node "' + node_title + '".')
            return False
    ...
    def add_connection_multiple_parents(self, parent_nodes: dict, probability_of_success: float):
        ...
        remodeled_connections = [{field: connection[field] for field in list(
            connection.keys())[0:-2]} for connection in self.multiple_parents_connections]

        if parent_nodes in remodeled_connections:
            print('The node "' + self.title +
                  '" already has a parent connection "' + str(parent_nodes) + '"')
            return False
...
class BayesianNetwork:
    ...
    def one_parent_probabilistic_inference(self, node_title: str):
        ...
        if len(connections) < 2:
            print('The amount of connections parent "' + parent.title +
                  '" have to node "' + node_title + '" arent enough.\n')
            return False
        ...
        for connection in connections:
            if connection['parent_success']:
                success_probability += parent_probability['success'] * \
                    connection['success']
                fail_probability += parent_probability['success'] * \
                    connection['fail']
            else:
                success_probability += parent_probability['fail'] * \
                    connection['success']
                fail_probability += parent_probability['fail'] * \
                    connection['fail']

        if success_probability + fail_probability < 0.95:
            print(
                'Something went wrong when calculating the probability, currently on node "' + node_title + '"')
            return False
            ...
            for parent_index in range(0, len(parents)):
                if connection[parents[parent_index].title]:
                    calculation_probabilites.append(
                        parent_probabilities[parent_index][parents[parent_index].title]['success'])
                else:
                    calculation_probabilites.append(
                        parent_probabilities[parent_index][parents[parent_index].title]['fail'])

...
class Simulate:
    ...
    def run(self, combat_system: CombatSystem, config=None, q_intermediate=None) -> bool:
        ...
        for n in tqdm(range(self.N)):
            ...
            if n % 50 == 0 and q_intermediate is not None:
    ...
    def intermediate_statistics(self):
        ...
        report.write(f"{'A Won:':<20}{x[0] / N:.2f}{os.linesep}")
        report.write(f"{'B Won:':<20}{x[1] / N:.2f}{os.linesep}") 
        report.write(f"{'Draw:':<20}{x[2]  / N:.2f}{os.linesep}")
        report.write(f"{'Mutual Annihilation:':<20}{x[3] / N:.2f}{os.linesep}")

...
class GPT2Dataset(Dataset):
    ...
    def get_data_for_ray_dataset(self):
        ...
        return np.array([self.data[i:i+self.block_size+1] for i in range(len(self.data) - self.block_size)]) # add 1 to data length so we can create the labels later (data[x + 1:] is the label for data[x:])

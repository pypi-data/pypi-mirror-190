...
class EnrollRecordServices:
    ...
    def record_list(params, need_pagination=None):
        ...
        enroll_obj = EnrollRecord.objects.filter(**params).exclude(enroll_status_code=124)
        ...
        user_infos = DetailInfoService.get_list_detail(None, user_id_list=user_ids)
        enroll_list = JoinList(enroll_list, user_infos, "user_id", "user_id").join()
        # 返回数据

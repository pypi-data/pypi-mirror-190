...
from config.permissions import DFMethods
...
class Command(BaseCommand):
    ...
    def get_fields(model):
        ...
        non_related_fields = [field for field in fields if not field.related_model]
        return non_related_fields
    ...
    def handle(self, *args, **options):
        ...
        for model in models:
            ...
            if Permission.objects.filter(content_type=content_type, name__startswith='Custom |').exists():
                """
                ...
                for action in actions:
                    for field in fields:
                        ...
                        name = f'Custom | {action.capitalize()} {str(model_name).capitalize()} {field.name}'
                        Permission.objects.get_or_create(codename=codename, name=name, content_type=content_type)

                    codename = f'{action}--{model_name}'
                    name = f'Custom | {action.capitalize()} {str(model_name).capitalize()}'
                    Permission.objects.get_or_create(codename=codename, name=name, content_type=content_type)
            else:
                for action in actions:
                    for field in fields:

...
def backup_instance():
    ...
    p = subprocess.run(f"pcluster ssh -n  {cluster} -i {homedir}/.{cluster}.pem \"sudo mv /etc/munge/munge.key /etc/munge/munge.key.bk && sudo rm -rf /home/ec2-user/.munge.bk &&  mv /home/ec2-user/.munge /home/ec2-user/.munge.bk \"", stdout=subprocess.PIPE, shell=True)
    if p.returncode != 0:
    ...
    resp = ec2_client.create_image(InstanceId=instance_id, Name=instance_name, NoReboot=True, BlockDeviceMappings=[{'DeviceName': '/dev/xvda','Ebs': {'DeleteOnTermination': False}}])
    imid = resp["ImageId"]
    while True:
        time.sleep(10)
    ...
    if len(conf["HeadNode"]["CustomActions"]["OnNodeUpdated"]["Args"]) > 1 :
        buckets = conf["HeadNode"]["CustomActions"]["OnNodeUpdated"]["Args"][1]
        config.set("default", "s3", buckets)
        with open(str(Path.home() / ".pcluster.ini"), "w") as f:
            config.write(f)

    p = subprocess.run(f"pcluster ssh -n  {cluster} -i {homedir}/.{cluster}.pem \"sudo mv /etc/munge/munge.key.bk /etc/munge/munge.key && mv /home/ec2-user/.munge.bk /home/ec2-user/.munge \"", stdout=subprocess.PIPE, shell=True)
    if p.returncode != 0:
        logging.error(p.stdout.decode("ascii"))
        raise click.Abort()
...
def get_pcluster_config():
    ...
    p = subprocess.run(f"aws s3 cp s3://altos-pcluster/{aws_region}.json {conf_path} ", stdout=subprocess.PIPE, shell=True)
    if p.returncode != 0:
        if p.stdout != b"":
            logging.error(p.stdout.decode("ascii"))
...
def create(master_instance_type, compute_instance_type, max_count, min_count, ami):
    ...
    template = env.get_template("pcluster_cli/pcluster.yaml")
    ...
    with open(f"{homedir}/.pcluster.yaml", "w") as f:
        f.write(output)
    p = subprocess.run(f"aws s3 cp pcluster_cli/node-startup.sh s3://altos-pcluster/{name}.sh", stdout=subprocess.DEVNULL, shell=True)

    assert p.returncode == 0, "Failed to copy the node startup script t s3://altos-pcluster/"
...
def delete(backup):
    ...
    while status["clusterStatus"] == "DELETE_IN_PROGRESS":
        ...
        if p.returncode == 0:
            ...
            break
        print(".", end="", flush=True)
    subprocess.run(f"aws s3 rm s3://altos-pcluster/{cluster}.sh", stdout=subprocess.PIPE, shell=True)

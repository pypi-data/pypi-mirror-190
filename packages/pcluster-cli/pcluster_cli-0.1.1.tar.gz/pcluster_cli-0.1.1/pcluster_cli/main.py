...
def get_pcluster_config():
    ...
    p = subprocess.run(f"aws s3 cp s3://altos-pcluster-cli-config/{aws_region}.json {conf_path} ", stdout=subprocess.PIPE, shell=True)
    if p.returncode != 0:
        if p.stdout != b"":
            logging.error(p.stdout.decode("ascii"))
...
def create(master_instance_type, compute_instance_type, max_count, min_count, ami):
    ...
    with open(f"{homedir}/.pcluster.yaml", "w") as f:
        f.write(output)
    p = subprocess.run(f"aws s3 cp {pcluster_cli_path}/node-startup.sh s3://altos-pcluster-cli-config/{name}.sh", stdout=subprocess.DEVNULL, shell=True)

    assert p.returncode == 0, "Failed to copy the node startup script to s3://altos-pcluster-cli-config/"
...
def delete(backup):
    ...
    while status["clusterStatus"] == "DELETE_IN_PROGRESS":
        ...
        if p.returncode == 0:
            ...
            break
        print(".", end="", flush=True)
    subprocess.run(f"aws s3 rm s3://altos-pcluster-cli-config/{cluster}.sh", stdout=subprocess.PIPE, shell=True)

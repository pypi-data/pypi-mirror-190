...
class ContactService(BaseService):
    ...
    def fieldset(self):
        custom_fields = self.XML.DataService.query(
            "DataFormField",
            1000,
            0,
            {"FormId": self.FORM_FIELDS.CONTACT},
            ["Id", "Name", "Label", "DataType"],
            "Id",
            False,
        )
        fields = self.get_table_field_definitions('Contact')
        for custom_field in custom_fields:
            field_name = f"_{custom_field['Name']}"
            if custom_field["DataType"] == 13:

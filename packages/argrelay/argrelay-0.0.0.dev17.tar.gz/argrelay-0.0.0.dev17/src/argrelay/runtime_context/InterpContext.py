...
from argrelay.schema_config_interp.DataEnvelopeSchema import envelope_class_, context_control_, envelope_metadata_
...
class InterpContext:
        ...
        if self.last_found_envelope_ipos >= 0:
            ...
            if context_control_ in prev_envelope.data_envelope:
                ...
                for arg_type_to_push in arg_type_list_to_push:
                    if arg_type_to_push in prev_envelope.data_envelope[envelope_metadata_]:
                        if arg_type_to_push in self.curr_container.assigned_types_to_values:
                            ...
                            if arg_value.arg_source.value <= ArgSource.ImplicitValue.value:
                                ...
                                arg_value.arg_value = prev_envelope.data_envelope[envelope_metadata_][arg_type_to_push]
                        else:
                            self.curr_container.assigned_types_to_values[
                                arg_type_to_push
                            ] = AssignedValue(
                                prev_envelope.data_envelope[envelope_metadata_][arg_type_to_push],
    ...
    def query_envelopes(self):
        ...
        query_dict = {
            f"{envelope_metadata_}.{envelope_class_}": self.curr_container.search_control.envelope_class,
        }
        ...
        for arg_type in self.curr_container.search_control.types_to_keys_dict.keys():
            if arg_type in self.curr_container.assigned_types_to_values:
                query_dict[f"{envelope_metadata_}.{arg_type}"] = self.curr_container.assigned_types_to_values[
                    arg_type
                ].arg_value

...
class ServiceLoader(AbstractLoader):
    ...
    def generate_envelope_id(self, data_envelopes: list):
        for data_envelope in data_envelopes:
            if envelope_id_ not in data_envelope[envelope_metadata_]:
                if envelope_class_ == ServiceEnvelopeClass.ClassHost.name:
                    data_envelope[envelope_metadata_][envelope_id_] = data_envelope[
                        envelope_metadata_
                    ][ServiceArgType.HostName.name]
                if envelope_class_ == ServiceEnvelopeClass.ClassService.name:
                    data_envelope[envelope_metadata_][envelope_id_] = (
                        data_envelope[envelope_metadata_][ServiceArgType.HostName.name]
                        + "." +
                        data_envelope[envelope_metadata_][ServiceArgType.ServiceName.name]
                    )

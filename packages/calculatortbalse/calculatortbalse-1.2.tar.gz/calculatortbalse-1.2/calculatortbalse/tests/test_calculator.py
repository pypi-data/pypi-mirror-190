...
def test_simple_operations():
    ...
    for operation in operations_order:
        ...
        calculator_method = getattr(calculator, operation)
        method_output = calculator_method(number).answer()
        if operation == "add":
            assert math.isclose(
                calculator_memory + number, method_output, abs_tol=TOLERANCE
            )
        ...
        calculator_memory = method_output
...
def test_root_and_reset():
    ...
    for _ in range(TEST_REPEAT_COUNT):
        ...
        method_output = calculator.root(degree).answer()
        assert math.isclose(
            calculator_memory ** (1 / degree), method_output, abs_tol=TOLERANCE
        )
...
def test_operations_zero():
    ...
    calculator_output = calculator.answer()
    assert calculator_output == ZERO

    calculator_output = calculator.add(ZERO).answer()
    assert calculator_output == ZERO

    calculator_output = calculator.subtract(ZERO).answer()
    assert calculator_output == ZERO

    calculator_output = calculator.multiply(ZERO).answer()
    assert calculator_output == ZERO

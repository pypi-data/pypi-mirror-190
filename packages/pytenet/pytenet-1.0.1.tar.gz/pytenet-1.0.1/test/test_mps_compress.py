...
class TestMPS(unittest.TestCase):
    ...
    def test_compression(self):
        ...
        d = 7
        D = [1, 4, 13, 21, 15, 7, 1]
        mps = ptn.MPS(np.random.randint(-4, 5, size=d), [np.random.randint(-2, 3, size=Di) for Di in D], fill='random')
        print(mps.bond_dims)
        ...
        mps.compress(tol=0)
        print(mps.bond_dims)
        print(np.linalg.norm(mps.as_vector() - psi))

#        # fictitious quantum numbers
#        qd0 = [1, 0, -2, 1]
#        qd1 = [-1, 0, 2, 0, 1]
#
#        # fictitious virtual bond quantum numbers
#        qD0 = [1, 2, -1, -3, 0]
#        qD1 = [-2, -2, 1, -4, -7, 0, -2, 4, 3, 5, 1, -5, -5, 1]
#        qD2 = [4, -2, -2, 0, 3, -5, -2]
#
#        A0 = randn_complex((len(qd0), len(qD0), len(qD1)))
#        A1 = randn_complex((len(qd1), len(qD1), len(qD2)))
#
#        # enforce block sparsity structure dictated by quantum numbers
#        mask = ptn.qnumber_outer_sum([qd0, qD0, -qD1])
#        A0 = np.where(mask == 0, A0, 0)
#        mask = ptn.qnumber_outer_sum([qd1, qD1, -qD2])
#        A1 = np.where(mask == 0, A1, 0)

...
__all__ = ['PEPS', 'merge_PEPS_tensor_pair', 'split_PEPS_tensor', 'compress_PEPS_tensors']


class PEPS(object):
    """
    Projected entangled pair state (PEPS) class.

    The (i,j)-th PEPS tensor has dimension `[d, D[i,j], D[i,j+1], D[i+1,j], D[i+1,j+1]]` with
    `d` the physical dimension at each site and `D` the array of virtual bond dimensions.

    Quantum numbers are not supported yet.
    """
    ...
    def __init__(self, d, D, fill=0.0):
        ...
        Args:
            d: physical dimension of each site (same for all sites)
            D: virtual bond dimensions
            fill: explicit scalar number to fill PEPS tensors with, or
        ...
        self.d = d
        ##self.D = D
        # create array of PEPS tensors
        # boundary bond dimensions must be 1
        assert np.all(D[0] == 1) and np.all(D[-1] == 1) and np.all(D[:, 0] == 1) and np.all(D[:, -1] == 1)
        if isinstance(fill, int) or isinstance(fill, float) or isinstance(fill, complex):
            self.A = [[np.full((d, D[i,j], D[i,j+1], D[i+1,j], D[i+1,j+1]), fill)
...
#        """Left- or right-orthonormalize the PEPS using QR decompositions."""
#        if len(self.A) == 0:
#            return
#
#        if mode == 'left':
...
#    def compress(self, tol=0, mode='left'):
#        """
#        Compress a matrix product state by sequential merging and SVD splitting
#        of neighboring tensors, truncating small singular values based on tolerance `tol`.
#        """
#        if mode == 'left':
#            for i in range(len(self.A) - 1):
#                self.A[i], self.A[i+1], self.qD[i+1] = compress_PEPS_tensors(
#                    self.A[i], self.A[i+1], self.qd, self.qd,
#                    (self.qD[i], self.qD[i+2]), 'right', tol)
#                assert is_qsparse(self.A[i],   [self.qd, self.qD[i],   -self.qD[i+1]])
#                assert is_qsparse(self.A[i+1], [self.qd, self.qD[i+1], -self.qD[i+2]])
#        elif mode == 'right':
#            for i in range(len(self.A) - 1):
#                self.A[i], self.A[i+1], self.qD[i+1] = compress_PEPS_tensors(
#                    self.A[i], self.A[i+1], self.qd, self.qd,
#                    (self.qD[i], self.qD[i+2]), 'left', tol)
#                assert is_qsparse(self.A[i],   [self.qd, self.qD[i],   -self.qD[i+1]])
#                assert is_qsparse(self.A[i+1], [self.qd, self.qD[i+1], -self.qD[i+2]])
#        else:
#            raise ValueError('mode = {} invalid; must be "left" or "right".'.format(mode))
    ...
    def as_vector(self):
        ...
        for i in range(1, len(self.A)):
            psi = merge_PEPS_tensor_pair(psi, self.A[i])
        assert psi.ndim == 3
    ...
    def __add__(self, other):
        """Add PEPS to another."""
        return add_PEPS(self, other)

    def __sub__(self, other):
        """Subtract another PEPS."""
        return add_PEPS(self, other, alpha=-1)
...
def merge_PEPS_tensor_pair(A0, A1):
    """Merge two neighboring PEPS tensors."""
...
def split_PEPS_tensor(A, qd0, qd1, qD, svd_distr, tol=0):
    """
    Split a PEPS tensor with dimension `d0*d1 x D0 x D2` into two PEPS tensors
    with dimensions `d0 x D0 x D1` and `d1 x D1 x D2`, respectively.
    """
    assert A.ndim == 3
    d0 = len(qd0)
    d1 = len(qd1)
    assert d0 * d1 == A.shape[0], 'physical dimension of PEPS tensor must be equal to d0 * d1'
...
def compress_PEPS_tensors(A0, A1, qd0, qd1, qD, svd_distr, tol=0):
    """
    Compress a PEPS tensor pair by merging and subsequent SVD splitting,
    truncating small singular values based on tolerance `tol`.
    """
    A = merge_PEPS_tensor_pair(A0, A1)
    return split_PEPS_tensor(A, qd0, qd1, qD, svd_distr, tol)


def add_PEPS(mps0, mps1, alpha=1):
    """
    Logical addition of two matrix product states (effectively sum virtual bond dimensions),
    with the second PEPS scaled by 'alpha'.
    """
    ...
    if L == 1:
        ...
        # simply add PEPS tensors
        mps.A[0] = mps0.A[0] + alpha*mps1.A[0]
        # consistency check
        assert is_qsparse(mps.A[0], [mps.qd, mps.qD[0], -mps.qD[1]]), \
            'sparsity pattern of PEPS tensor does not match quantum numbers'
    elif L > 1:

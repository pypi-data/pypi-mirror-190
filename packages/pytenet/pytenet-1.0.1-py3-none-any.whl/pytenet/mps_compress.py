...
__all__ = ['MPS', 'merge_MPS_tensor_pair', 'split_MPS_tensor', 'compress_MPS_tensors']
...
class MPS(object):
    ...
    def compress(self, tol=0, mode='left'):
        ...
        if mode == 'left':
            for i in range(len(self.A) - 1):
                self.A[i], self.A[i+1], self.qD[i+1] = compress_MPS_tensors(
...
def compress_MPS_tensors(A0, A1, qd0, qd1, qD, svd_distr, tol=0):
    """
    Compress a MPS tensor pair by merging and subsequent SVD splitting,
    truncating small singular values based on tolerance `tol`.
    """
    A = merge_MPS_tensor_pair(A0, A1)
    return split_MPS_tensor(A, qd0, qd1, qD, svd_distr, tol)

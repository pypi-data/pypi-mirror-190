...
def main():
    ...
    if arg1 == "download-images":
        ...
        return
    if arg1 == "decorate-with-annotation":
        import os
        import numpy as np
        output_dir = "./decorated"
        ...
        for file_name, annotation in g:
            ...
            for ann in parsed_annotations:
                coord = np.asarray(ann['coord']) * [source_img.height, source_img.width, source_img.height, source_img.width]
                draw.rectangle(((coord[0], coord[1]), (coord[2], coord[3])), width=3)
                draw.text((coord[0] + 5, coord[1]), ann['label'])
            source_img.save(output_dir + "/" + file_name, "JPEG")
        return

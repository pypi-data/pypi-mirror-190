...
# Ensures correct holoviews_samap.core.util is sourced
from .. import util
...
class Dataset(Element):
    ...
    def select(self, selection_expr=None, selection_specs=None, **selection):
        ...
        * predicate expression: A holoviews_samap.dim expression, e.g.:

            from holoviews_samap import dim
            ds.select(selection_expr=dim('x') % 2 == 0)

        Args:
            selection_expr: holoviews_samap.dim predicate expression
...
holoviews_samap.util.transform.dim expression. Use the selection_specs keyword
argument to specify a selection specification""")
    ...
    def sample(self, samples=[], bounds=None, closest=True, **kwargs):
        ...
        # see https://github.com/holoviz/holoviews_samap/issues/1173

...
class InfoPrinter(object):
    ...
    def target_info(cls, obj, ansi=False):
        ...
        target_footer = ("\nTo see the options info for one of these target specifications,"
                         "\nwhich are of the form {type}[.{group}[.{label}]], do holoviews_samap.help({type}).")
    ...
    def object_info(cls, obj, name, backend, ansi=False):
        ...
        element_url ='http://holoviews_samap.org/reference/elements/{backend}/{obj}.html'
        container_url ='http://holoviews_samap.org/reference/containers/{backend}/{obj}.html'
        ...
        msg = ("\nOnline example: {link}" if link else ''
               + "\nHelp for the data object: holoviews_samap.help({obj})"
               + " or holoviews_samap.help(<{lower}_instance>)")

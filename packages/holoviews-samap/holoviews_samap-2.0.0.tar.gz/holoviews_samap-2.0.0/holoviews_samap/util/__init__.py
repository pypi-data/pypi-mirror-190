...
def examples(path='holoviews_samap-examples', verbose=False, force=False, root=__file__):
    """
    ...
    def _expand_options(cls, options, backend=None):
        ...
        if not Store.renderers:
            raise ValueError("No plotting extension is currently loaded. "
                             ...
                             "holoviews_samap.plotting before applying any "
                             "options.")
...
class extension(_pyviz_extension):
    """
    Helper utility used to load holoviews_samap extensions. These can be
        ...
        for backend, imp in imports:
            try:
                ...
                self.param.warning("%s could not be imported, ensure %s is installed."
                             % (backend, backend))
            try:
                __import__('holoviews_samap.plotting.%s' % imp)

    ...
    def wrapped(element):
        ...
        try:
            ...
            if not any(fn.__name__.endswith(suffix) for suffix in disabled_suffixes):
                if type(holoviews_samap.archive) is not FileArchive and 'text/html' in mime_data:
                    holoviews_samap.archive.add(element, html=mime_data['text/html'])
            ...
            msg =  '<i> [Call holoviews_samap.ipython.show_traceback() for details]</i>'
...
def display(obj, raw_output=False, **kwargs):
    ...
    if not Store.loaded_backends() and isinstance(obj, Dimensioned):
        raise RuntimeError('To use display on a HoloViews object ensure '
                           'a backend is loaded using the holoviews_samap '
                           'extension.')

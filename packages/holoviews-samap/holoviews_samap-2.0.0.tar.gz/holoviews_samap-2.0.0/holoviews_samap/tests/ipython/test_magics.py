...
try:
    from holoviews_samap import ipython            # noqa (Import test)
    from holoviews_samap.ipython import IPTestCase
except ImportError:
    raise SkipTest("Required dependencies not satisfied for testing magics")

from holoviews_samap.operation import Compositor
...
class ExtensionTestCase(IPTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.ip.run_line_magic("load_ext", "holoviews_samap.ipython")
        for renderer in Store.renderers.values():
            renderer.comm_manager = CommManager

    def tearDown(self):
        Store._custom_options = {k:{} for k in Store._custom_options.keys()}
        self.ip.run_line_magic("unload_ext", "holoviews_samap.ipython")
        del self.ip
        super().tearDown()
...
class TestOptsMagic(ExtensionTestCase):
    ...
    def setUp(self):
        ...
        self.cell("from holoviews_samap import DynamicMap, Curve, Image")
...
class TestCompositorMagic(ExtensionTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.cell("import numpy as np")
        self.cell("from holoviews_samap.element import Image")

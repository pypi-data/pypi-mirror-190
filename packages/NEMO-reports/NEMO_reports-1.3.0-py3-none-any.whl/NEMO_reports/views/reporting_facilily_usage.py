...
def get_facility_usage_duration_and_data(
    ...
    if params.get_bool("training", "on"):
        trainings = training_sessions(start, end).annotate(
            timedelta=ExpressionWrapper(F("duration") * 60 * 1000000, output_field=DurationField()),
        )
        if extra_filter:
            trainings = trainings.filter(extra_filter)
        total_duration += trainings.aggregate(Sum("timedelta"))["timedelta__sum"] or datetime.timedelta(0)
        if data:
...
def to_billable_items(
    ...
    if billable:
        ...
        billable.timedelta = obj.timedelta

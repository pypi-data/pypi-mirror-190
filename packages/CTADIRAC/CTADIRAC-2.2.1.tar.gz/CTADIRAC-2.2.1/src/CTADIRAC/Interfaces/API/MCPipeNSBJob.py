...
class MCPipeNSBJob(Job):
    ...
    def __init__(self, cpu_time=259200):
        ...
        self.div_ang = ["0.0022", "0.0043", "0.008", "0.01135", "0.01453"]
    ...
    def set_div_ang(self, div_ang=None):
        if div_ang is not None:
            if div_ang.replace(", ", ",").split(sep=",") != self.div_ang:
                DIRAC.gLogger.error(
                    "Unknown div_ang option: %s. Option for simulation step is: %s"
                    % (div_ang, ", ".join(str(x) for x in self.div_ang))
                )
                DIRAC.exit(-1)
            else:
                self.output_file_metadata["div_ang"] = self.div_ang
        else:
            self.div_ang = None

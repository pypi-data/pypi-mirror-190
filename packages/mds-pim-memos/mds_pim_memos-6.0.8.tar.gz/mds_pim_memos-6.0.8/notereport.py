...
import tempfile, os, time, subprocess, logging, json
...
class ReportOdt(Report):
    ...
    def optimize_tags(cls, html, from_tag, to_tag, attrs):
        ...
        while True:
            ...
            attr2 = ';'.join([
                '%(key)s:%(value)s' % {
                    'key': x,
                    'value': attrs.get(reverse_key[x], {}).get(st3_dict[x], st3_dict[x]),
                    }
    ...
    def execute(cls, ids, data):
        ...
        (ext1, cont1, dirprint, title) = super(ReportOdt, cls).execute(ids, data)

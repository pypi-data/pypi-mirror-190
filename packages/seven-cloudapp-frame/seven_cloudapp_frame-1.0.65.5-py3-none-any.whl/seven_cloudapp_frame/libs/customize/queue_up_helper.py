...
@LastEditTime: 2022-09-27 15:08:55
                        ...
                        if hash_value:
                            if first_is_process == True and i == 0:
                                ...
                                if now_timestamp > first_end_timestamp + (i * int(config.get_value("queue_confirm_time", 8))):
                                    is_delete = True
                                    self.logger_info.info("【踢掉办理用户2】" + SevenHelper.json_dumps({"zset_name": zset_name, "first_hash_value": first_hash_value, "user_id": data, "now_timestamp": now_timestamp, "pop_timestamp": first_end_timestamp + (i * int(config.get_value("queue_confirm_time", 8)))}))
                            else:
                                ...
                                if now_timestamp > pop_timestamp + ((i + 1) * int(config.get_value("queue_confirm_time", 8))):
                                    is_delete = True
                                    self.logger_info.info("【踢掉办理用户3】" + SevenHelper.json_dumps({"zset_name": zset_name, "first_hash_value": first_hash_value, "user_id": data, "now_timestamp": now_timestamp, "pop_timestamp": pop_timestamp + ((i + 1) * int(config.get_value("queue_confirm_time", 8)))}))

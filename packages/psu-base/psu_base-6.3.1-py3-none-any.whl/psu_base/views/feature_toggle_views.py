...
def feature_list(request):
    ...
    is_global_admin = auth_service.has_authority(
        allowable_role_list, require_global=True
    )

    if is_global_admin and utility_service.is_in_primary_app():
        app_options = utility_service.get_app_options()
    else:
        app_options = {utility_service.get_app_code(): utility_service.get_app_name()}

    # Get all features (app and globals)
    app_features = Feature.objects.filter(app_code__in=app_options)
    ...
    return render(
        request,
        "psu_base/feature_toggle/list.html",
        {
...
def add_feature(request):
    ...
    if is_global_admin and app_code == "global_override":
        ...
        app_code = None
        default = "Y"
    # Sub-app could have been specified
    elif app_code in app_options:
        pass
    ...
    if enable_str:
        try:
            ...
            message_service.post_error(
                f"Unable to process the given enable date: {enable_str}"
            )
    if disable_str:
        try:
            ...
            message_service.post_error(
                f"Unable to process the given disable date: {disable_str}"
            )
...
def modify_feature(request):
    ...
    if prop == "title":
        ...
        # Warn on enable date for already-active feature
        if prop == "enable_date" and feature_instance.status == "Y" and value:
            message_service.post_warning("Enable date has been set on an already-active feature.")
        elif prop == "disable_date" and feature_instance.status == "N" and value:
            message_service.post_warning("Termination date has been set on an already-disabled feature.")
    else:
    ...
    if feature_instance.app_code is None:
        ...
        if not is_global_admin:
            ...
            raise HttpResponseNotAllowed(
                "Only global admins may modify global features"
            )

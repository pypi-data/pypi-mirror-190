...
\tRaised From:    {src}
    ...
    try:
        ...
        ee.path = path[:128] if path else path
        ee.parameters = parameters[:500] if parameters else parameters
        ee.code_detail = src[:128] if src else src
        ...
        ee.error_friendly = error_display[:128] if error_display else error_display
        ee.error_system = error_system[:128] if error_system else error_system
        ee.debug_info = debug_info[:128] if debug_info else debug_info
        ee.save()

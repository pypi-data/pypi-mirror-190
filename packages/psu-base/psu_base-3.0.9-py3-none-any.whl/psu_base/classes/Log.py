...
class Log:
    ...
    def __init__(self, reduce_logging=None):
        """
        reduce_logging: When True, debug level logging will be skipped
        """
        self.logger = logging.getLogger('psu')
        self.fn_times = {}

        if reduce_logging:
            self.reduce_logging = True
        elif reduce_logging is not None and not reduce_logging:
            self.reduce_logging = False
        else:
            # Reduce logging during AWS health checks
            request = CrequestMiddleware.get_request()
            if request and 'HTTP_USER_AGENT' in request.META and 'HealthChecker' in request.META['HTTP_USER_AGENT']:
                self.reduce_logging = True
    ...
    def error(self, msg, trace_error=True, strip_html=False):
        ...
        if trace_error:
            ...
            self.logger.error(f"{strip_tags(msg) if strip_html else msg} {trace}")
        else:

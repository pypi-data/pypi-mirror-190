...
class Auth:
    ...
    def has_authority(self, authority_code, sso_user_only=False, require_global=False):
        ...
        # Dynamic roles from template-tags will come in as plain-text
        # If a DynamicRole was given, expand it out to the full role list
        role_list = []
        ...
        #     ]
        # )
        log.debug(f"\n{self.sso_user.username if sso_user_only else self.get_user().username} {'has' if allowed else 'does  not have'} {authority_code}")
        return allowed

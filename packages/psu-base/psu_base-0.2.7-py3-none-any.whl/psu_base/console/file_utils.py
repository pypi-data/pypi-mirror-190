...
def get_file_content_string(file_path):
    ...
    if not os.path.exists(file_path):
        out.print_color("Given file does not exist: {0}".format(file_path), 'red')
        return None
    ...
    try:
        ...
        out.print_color("Error opening file: {0}".format(file_path), 'red')
        out.print_color(ee, 'red')
...
def replace_line(file_path, line_starts_with, new_text, strip_test_line=True):
    ...
    try:
        ...
        out.print_color("Could not modify file: {0}".format(file_path), 'red')
        out.print_color(ee, 'red')

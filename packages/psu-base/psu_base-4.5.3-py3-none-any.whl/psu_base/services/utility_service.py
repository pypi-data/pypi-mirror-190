...
def get_app_code():
    ...
    primary_app_code = app_in_use = get_setting('APP_CODE').upper()

    # Starting with v 3.0.0, an app can have multiple sub-apps
    sub_apps = get_setting('SUB_APPS')
    if sub_apps:
        ...
        sub_app_code = pieces[sub_app_index].upper() if len(pieces) > sub_app_index else None
        ...
        sub_app_info = {
            'last_app': last_app, 'current_app': app_in_use, 'is_sub_app': is_sub_app, 'app_changed': app_changed
        }
...
def get_sub_app_info(primary_app_code=None):
    sub_app_info = {'last_app': primary_app_code, 'current_app': None, 'is_sub_app': None, 'app_changed': False}
...
def get_app_name():
    ...
    if sub_apps:
        ...
        if app_code in sub_apps:
            return sub_apps[app_code]
...
def is_term(string):
    ...
    if is_non_production():
        log.warning("DEPRECATED: utility_service.is_term() has moved to validation_service")
...
def is_psu_id(string):
    ...
    if is_non_production():
        log.warning("DEPRECATED: utility_service.is_psu_id() has moved to validation_service")
    return validation_service.is_psu_id(string)


def has_unlikely_characters(value, unlikely_characters='`!*=\\$%^[]{}<>;'):
    if is_non_production():
        log.warning("DEPRECATED: utility_service.has_unlikely_characters() has moved to validation_service")

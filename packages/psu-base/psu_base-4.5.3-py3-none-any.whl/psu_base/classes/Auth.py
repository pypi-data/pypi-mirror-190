    ...
    def __init__(self, from_sso=None, auto_resume=False, app_change=False):
        ...
        if auto_resume or app_change:
            ...
            if saved and not app_change:
                ...
                sso_username = saved['sso_user'].get('username') if saved['sso_user'] else None
                imp_username = saved['impersonated_user'].get('username') if saved['impersonated_user'] else None
                prx_username = saved['proxied_user'].get('username') if saved['proxied_user'] else None
        ...
        # New from SSO
            ...
            if not self.sso_user.is_valid():
                ...
                if str(from_sso).endswith('-high'):
                    message_service.post_error("You must log in with your Odin account, not your High account.")

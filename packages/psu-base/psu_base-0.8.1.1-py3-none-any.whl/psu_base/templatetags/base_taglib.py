...
def if_logged_in(parser, token=None):
    try:
        ...
        nodelist = parser.parse((f"end_{tag_name}",))
        parser.delete_first_token()
        log.debug("Calling auth_service.is_logged_in() ...")
        result = auth_service.is_logged_in("base_taglib.if_logged_in")
        log.debug(f"auth_service.is_logged_in() returned {result}")
        return support.ConditionalResponseNode(nodelist, result)

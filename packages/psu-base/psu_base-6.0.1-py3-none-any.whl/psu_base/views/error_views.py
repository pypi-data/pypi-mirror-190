...
from psu_base.decorators import require_authority, trace
...
def error_list(request):
    ...
    if not error_filter:
        app_errors = (
            Error.objects.filter(app_code=app_code)
            .exclude(status="R")
            .exclude(status="I")
        )
    else:
...
def ignore_similar(request, error_id):
    ...
    try:
        ...
        if error:
            ...
            if system_ee:
                errors = Error.objects.filter(app_code=app_code, status="N", error_system=system_ee)
                if len(errors) <= 1:
                    errors = Error.objects.filter(
                        app_code=app_code, status="N", error_system__startswith=system_ee[:20]
                    )

            elif posted_ee:
                errors = Error.objects.filter(app_code=app_code, status="N", error_friendly=posted_ee)
                if len(errors) <= 1:
                    errors = Error.objects.filter(
                        app_code=app_code, status="N", error_friendly__startswith=posted_ee[:20]
                    )

            elif debug_info:
                errors = Error.objects.filter(app_code=app_code, status="N", debug_info=debug_info)
                if len(errors) <= 1:
                    errors = Error.objects.filter(
                        app_code=app_code, status="N", debug_info__startswith=debug_info[:20]
                    )
        ...
        error_service.unexpected_error("Unable to ignore errors", ee)

    return redirect("psu:errors")


@trace()
def authentication_error(request):
    return render(request, "psu_base/error/authentication_error.html", {})

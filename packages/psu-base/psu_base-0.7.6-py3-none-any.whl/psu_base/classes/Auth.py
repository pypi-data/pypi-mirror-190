...
class Auth:
    ...
    def get_user(self):
        ...
        if self.is_impersonating():
            utility_service.append_session_log('Impersonating!')
            return self.impersonated_user
        else:
            utility_service.append_session_log(f"Not Impersonating. I'm {str(self.sso_user)}")
    ...
    def __init__(self, from_sso=None, auto_resume=False):
        utility_service.append_session_log('Getting an Auth object...')
        # Resume from session
        if auto_resume:
            log.debug(f"Auth: auto_resume")

...
from psu_base.services import utility_service, email_service, message_service, auth_service
...
log = Log()
allowable_role_list = DynamicRole().power_user()
        ...
        for ww in keywords.split():
            if ww and len(ww) > 2:
                    ...
                    if ':' in ww:
                        ...
                        if len(pieces[1]) <= 2:
                            ...
                            app_emails = app_emails.filter(
                                Q(to__icontains=pieces[1]) | Q(cc__icontains=pieces[1]) | Q(bcc__icontains=pieces[1])
                            )
                    # Otherwise check all recipient types (not sender)
                    else:
                        app_emails = app_emails.filter(Q(to__icontains=ww) | Q(cc__icontains=ww) | Q(bcc__icontains=ww))
                ...
                # Handle some specified properties
                    ...
                    if len(pieces[1]) <= 2:
                    ...
                    elif pieces[0].lower() in ['before', 'after', 'on', 'since', 'initiator', 'by']:
                        # If date is expected
                        if pieces[0].lower() in ['before', 'after', 'on', 'since']:
                            try:
                                ...
                                if pieces[0].lower() == 'before':
                                    app_emails = app_emails.filter(date_created__lt=cd.datetime_instance)
                                elif pieces[0].lower() in ['after', 'since']:
                                    app_emails = app_emails.filter(date_created__gte=cd.datetime_instance)
                                else:
                                    next_day = cd.datetime_instance + timedelta(days=1)
                                    app_emails = app_emails.filter(date_created__gte=cd.datetime_instance)
                                    app_emails = app_emails.filter(date_created__lt=next_day)
                        ...
                        # If specifying the initiator
                            ...
                            app_emails = app_emails.filter(initiator__icontains=pieces[1])
                    else:
    ...
    if email_instance:
        ...
        if email_instance.email_template:
            ...
            if email_instance.context:
                try:
                    context = re.sub(r'<(\w+):[^>]+>', r"auth_service.look_up_user_cache('\1')", email_instance.context)
                    context = eval(context)
                except Exception as ee:
                    message_service.post_warning("Email may not display correctly. Resending it is not recommended.")

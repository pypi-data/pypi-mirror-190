...
class HeaderNavTab(template.Node):
    ...
    def render(self, context):
        ...
        if icon_only:
            pieces.append(f"""<span class="fa {icon}" aria-hidden="true" title="{label}"> </span>""")
...
class HeaderNavMenuItem(template.Node):
    ...
    def render(self, context):
        ...
        # Item is allowed for all admins if no authorities are specified
        allowed = auth_service.has_authority('admin')

        # A list of required authorities may be provided
        authorities = attrs.get('authorities')
        if type(authorities) is str:
            authorities = [x.strip(' \'"').lower() for x in authorities.strip('[], ').split(',')]

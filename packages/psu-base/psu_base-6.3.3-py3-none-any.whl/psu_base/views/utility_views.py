...
def get_id_tag(request, identifier=None):
    # Do not allow any random person to look up an ID
    # PowerUsers should be allowed this access
    allow = auth_service.has_authority("~PowerUser")
    # In certain cases, a non-power user might be allowed
    # (i.e. A coordinator in the OAR Extensions app will not be a power user)
    if not allow:
        allow = utility_service.allow_id_lookups()

    if allow and identifier:

...
class Finti:
    ...
    def get(self, path, parameters=None, include_metadata=False):
        ...
        # A Django app may want/need to simulate SSO Proxy calls while making actual calls to another Finti app
        simulate_when_possible = self.simulate_when_possible
        if str(simulate_when_possible).upper().startswith("SSO"):
            if "/sso_proxy" not in path:
                simulate_when_possible = False
            else:
                simulate_when_possible = "SSO"

        # If simulating responses, or preparing to simulate them in the future, prepare a Faki instance
        if self.simulate or self.prepare_offline or simulate_when_possible:
            self.faki = Faki(
        ...
        simulated = False
        if self.simulate or simulate_when_possible:
            try:
        ...
        is_simulating = self.simulate or simulate_when_possible
        must_simulate = self.simulate and not simulate_when_possible
        if is_simulating and (simulated or must_simulate):
            ...
            try:
                ...
                if utility_service.is_development() and simulate_when_possible == "SSO":
                    # If set to SSO when running development, it may be that SSO_Proxy is not available locally
                    # If lookup is needed for SSO Proxy endpoint, use a deployed instance
                    if "oit.pdx.edu" not in api_url:
                        log.warning("Getting SSO Proxy data from DEVL Finti")
                        api_url = f"https://sf-dev.oit.pdx.edu/{path}".strip("/")

...
def send(
    ...
    try:
        if email_template:
            if not email_template.endswith('.html'):
                email_template = f"{email_template}.html"
            html = render_to_string(email_template, context)
    except Exception as ee:
        log.error(f"Unable to render template: {email_template}")
        log.debug(str(ee))
    try:
        if email_template:
    ...
    if invalid:
        ...
        try:
            ...
            error_service.unexpected_error(
                f"Error sending email: {subject}", ee
            )
    ...
    if not suppress_status_messages:
        if status == 'S' and not suppress_success_message:
            message_service.post_success(f'<span class="fa fa-envelope" aria-hidden="true"></span>&nbsp; Email Sent: {subject}')
        elif status != 'S':
            message_service.post_warning(f"Unable to send email: {subject}")

        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            # Otherwise, send somewhere else
                ...
                if utility_service.get_flash_variable('new_impersonation'):
                    log.info("Impersonated user does not have access to last page. Redirecting...")
                else:
...
def trace(metrics=False, log_result=False):
    """
    Trace function (including parameters) with error handling and optionally log the result
    """
    def decorator(func):
        @wraps(func)
        def _wrapped_func(*args, **kwargs):
            ...
            try:
                ...
                error_service.record(ee, "Raised by @trace() decorator")
                had_error = True
                # ToDo: Detect if this was a view, and return a valid response (redirect to error page)

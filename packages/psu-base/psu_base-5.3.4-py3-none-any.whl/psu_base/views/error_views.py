...
def ignore_similar(request, error_id):
    ...
    try:
        ...
        if error:
            ...
            if system_ee:
                errors = Error.objects.filter(status="N", error_system=system_ee)
                if len(errors) <= 1:
                    errors = Error.objects.filter(status="N", error_system__startswith=system_ee[:20])

            elif posted_ee:
                errors = Error.objects.filter(status="N", error_friendly=posted_ee)
                if len(errors) <= 1:
                    errors = Error.objects.filter(status="N", error_friendly__startswith=posted_ee[:20])

            elif debug_info:
                errors = Error.objects.filter(status="N", debug_info=debug_info)
                if len(errors) <= 1:
                    errors = Error.objects.filter(status="N", debug_info__startswith=debug_info[:20])
...
def authentication_error(request):
    return render(request, "error/authentication_error.html", {})

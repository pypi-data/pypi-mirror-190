...
class User:
    ...
    def is_valid(self):
        log.trace([str(self), str(self.proxy), self.email_address])
        utility_service.append_session_log(f"Is {str(self)} with email '{self.email_address}' valid?")
    ...
    def __init__(self, src, as_proxy=False):
        if type(src) is dict:
            ...
            log.debug("User: evaluates to True, I guess")
            log.trace([src])
            user_data = src

...
from psu_base.services import utility_service, auth_service, validation_service
from psu_base.templatetags.tag_processing import html_generating, static_content
...
def variable(*args, **kwargs):
    return Variable.get_value(args[0], default_value=kwargs.get("default_value"), data_type=kwargs.get("data_type"))
...
@register.filter
def convenient_date(date_instance, nvl=None):
    if date_instance:
        try:
            return ConvenientDate(date_instance)
        except:
            log.warn(f"Invalid date (convenient_date): {date_instance}")
    return nvl


@register.simple_tag()
def humanize(date_instance):
    if date_instance:
        try:
            return ConvenientDate(date_instance).humanized()
        except:
            log.warn(f"Invalid date (humanize): {date_instance}")
    return ""
...
@register.simple_tag(takes_context=True)
def display_name(context, *args):
    """
    If not args given, return current user's name (original functionality)
    If arg is given, use it to look up a User and display their name (new in v5+)
    """
    if args:
        user_data = args[0]
        if type(user_data) is User:
            ...
            user_instance = auth_service.look_up_user_cache(user_data)
            if user_instance:
                return user_instance.display_name
            else:
                return user_data
    else:
...
def banner_character_functions(*args, **kwargs):
    return static_content.banner_character_functions(*args, **kwargs)
...
@register.inclusion_tag("psu_base/components/pagination.html")
...
@register.tag()
def banner_name_input(parser, token):
    return html_generating.BannerNameNode(token.split_contents())


@register.tag()
def banner_text_input(parser, token):
    return html_generating.BannerTextNode(token.split_contents())
...
def js_prompt(parser, token):
    ...
    {%js_prompt icon="fa-comments-o" title="Tell Me..." callback="my_action"%}
    """
    log.trace()
    tokens = token.split_contents()
    try:
...
@register.inclusion_tag("psu_base/layout/header/admin_submenu.html", takes_context=True)
def header_nav_submenu(context, submenu):
    """Example:"""
    return html_generating.HeaderNavSubMenu(submenu)
...
@register.inclusion_tag("psu_base/components/expandable_id.html", takes_context=True)
def expandable_id(context, identifier):
    """Example: {%expandable_id "mjg"%} or {%expandable_id 1049650%}"""
    model = context.flatten()
    model["identifier"] = identifier
    return model


@register.inclusion_tag("psu_base/components/id_tag.html", takes_context=True)
def id_tag(context, user_instance_or_info):
    """Example: {%id_tag "mjg"%} or {%id_card current_user%}"""
    model = context.flatten()
    if type(user_instance_or_info) is User:
        model["user_instance"] = user_instance_or_info
    else:
        model["user_instance"] = auth_service.look_up_user_cache(user_instance_or_info)
    return model


@register.inclusion_tag("psu_base/components/id_card.html", takes_context=True)
...
@register.inclusion_tag("psu_base/components/_smokescreen_spinner.html", takes_context=True)
...
@register.inclusion_tag("psu_base/layout/assets/admin_script.html", takes_context=True)
def admin_script(context):
    return {
        "scripts": utility_service.get_admin_scripts(
            context.request, auth_service.get_user().username
        )
    }


@register.inclusion_tag("psu_base/layout/messages/banner_downtime.html")
def downtime_messages():
    ...
    return {
        "finti_downtime_url": finti_downtime_url,
        "finti_downtime_env": finti_downtime_env,
    }


@register.simple_tag()
def flash_message_birth_date():

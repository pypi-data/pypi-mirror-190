...
def clean_banner_name(request):
    ...
    try:
        data['clean_name'], data['name_changed'] = banner_service.clean_name(name_string)
        response = JsonResponse(data)
    except Exception as ee:
        response = JsonResponse(data)
        response.status_code = 403
...
def clean_banner_chars(request):
    ...
    try:
        data['clean_content'], data['content_changed'] = banner_service.clean_special_characters(content_string)
        response = JsonResponse(data)
    except Exception as ee:
        response = JsonResponse(data)
        response.status_code = 403
...
def toggle_simulation(request):
    current_status = bool(utility_service.get_session_var('finti_pause_simulation', False))
...
def end_session(request):
    ...
    return render(request, 'errors/session_expiration.html', {})

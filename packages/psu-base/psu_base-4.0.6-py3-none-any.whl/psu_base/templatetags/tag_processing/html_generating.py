...
from psu_base.services import utility_service, auth_service, error_service, banner_service, message_service
...
class BannerNameNode(template.Node):
    ...
    def render(self, context):
        ...
        value, changed = banner_service.clean_name(attrs.get('value'))
        ...
        if utility_service.is_development() and not utility_service.get_session_var('banner_character_functions'):
...
class BannerTextNode(template.Node):
    ...
    def render(self, context):
        ...
        # Value needs special processing
        value, changed = banner_service.clean_special_characters(attrs.get('value'))
        attrs['value'] = value if value else ''

        # Onchange needs special processing
        _onchange = attrs.get('onchange')
        if _onchange:
            onchange = "clean_banner_characters($(this), function(){" + _onchange + "});"
        else:

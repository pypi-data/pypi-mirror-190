...
from psu_base.services import message_service, utility_service, error_service, validation_service
import requests
import json
...
class Finti:
    ...
    @staticmethod
    def encode_special_chars(content, limit_special_chars=True):
        """
        ...
        return str([ord(c) for c in content]).strip('[]').replace(' ', '')
        ...
        if parameters:
            try:
                for kk, vv in parameters.items():
                    parameters[kk], changed = validation_service.remove_special_characters(vv)
                    if changed:
                        log.warning(f"Parameter {kk}'s special-characters have been altered.")
            except Exception as ee:
                error_service.record(ee, "Removing special characters from Finti parameters")
        ...
        if payload:
            try:
                for kk, vv in payload.items():
                    payload[kk], changed = validation_service.remove_special_characters(vv)
                    if changed:
                        log.warning(f"Parameter {kk}'s special-characters have been altered.")
            except Exception as ee:
                error_service.record(ee, "Removing special characters from Finti parameters")

...
class Hemu:
    ...
    def pytoexe(self):
        self.cha1='';self.cha2='';self.fnam='';self.fpla='';self.iconam='';self.dis1=''
        def pyfile():
        ...
        def conv1():
            ...
            dis=self.fpla[:2]
            if(self.iconam!=''):
                cmd=dis+' & cd '+self.fpla+' & py -m pip install pyinstaller & pyinstaller --onefile -w -i '+self.iconam+' '+self.fnam
            else:
                cmd=dis+' & cd '+self.fpla+' & py -m pip install pyinstaller & pyinstaller --onefile -w '+self.fnam
            te=Popen(cmd,shell=True,stdout=PIPE,stderr=PIPE)
            out,err=te.communicate()
            out=out.decode('utf-8')
            err=err.decode('utf-8')
            self.dis1=out+'\n'+err
            te1=self.dis1.find('Building EXE from EXE-00.toc completed successfully.')
            if(te1!=-1):
                f1l3.place_forget()
                f1l2.place_forget()
                f1l5.place_forget()

...
class Condenser:
    def __init__(self):
        ...
        self.有效换热面积 = None  # 管子外表面积，m^2
        self.冷却水流通面积 = None  # 冷却水流通面积
        self.设计传热系数 = None  # W/(m^2.℃)
    ...
    def set_info(self, 类型=None, 设计凝汽器热负荷=None, 管子材料=None, 管子数量=None, 有效长度=None, 管子外径=None,
                 ...
                 循环水体积流量=None, 循环水质量流量=None, bp=None, 有效换热面积=None, 洁净度=None,
                 ):
        """
        ...
        if self.有效换热面积 is None:
            if self.管子数量 is not None:
                ...
                self.冷却水流通面积 = _area * self.管子数量
            else:
                logger.warning(f"缺少凝汽器有效换热面积或管子数量")

        if self.设计循环水流量_t_h is not None:
        ...
        if self.设计传热系数 is None:
            _ln_t = self.cal_对流平均温差(self.t_cyc_out_d, self.t_cyc_in_d, self.设计背压)
            self.设计传热系数 = self.设计凝汽器热负荷 / _ln_t / self.有效换热面积 * 1000
    ...
    def cal_运行清洁度系数(self, 凝汽器热负荷, t_cyc_hot, t_cyc_cold, bp, 循环水流量, 冷却管管材和壁厚修正系数,
        ...
        基本传热系数 = self.设计传热系数 / math.sqrt(self.cal_管内平均流速(循环水质量流量=循环水流量))

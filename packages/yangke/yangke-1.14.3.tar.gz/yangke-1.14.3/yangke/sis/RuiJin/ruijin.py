...
from yangke.sis.RuiJin.tags import TagsRead, TagsWrite
...
condenser.set_info(类型='双背压', 设计凝汽器热负荷=494.68, 有效长度=14,
                   t_cyc_in=22.3, bp=4.75, 有效换热面积=64000, 洁净度=0.85,
                   循环水体积流量=101000, 传热系数=2705,
                   )
condenser.cal_基本传热系数K0(0.9, 22)
...
def optimize():
    snapshot = read_data(TagsRead, ip="172.18.240.191", port=12084, user="admin", password="admin")

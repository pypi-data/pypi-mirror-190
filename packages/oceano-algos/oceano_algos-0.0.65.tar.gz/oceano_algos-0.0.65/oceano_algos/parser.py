...
__all__ = ['Driver', 'get_driver', 'CategoryScrapper', 'get_category_df_from_page', 'update_page_num', 'run',
           'SingleItemScrapper', 'get_params_df', 'get_sold_qty', 'get_seller_params', 'get_description',
           'get_rates_vs_percentage', 'get_timestamp_from_row', 'get_first_feedback_timestamp', 'get_promotion', 'run',
           'MultipleItemScrapper', 'run', 'ShopstatScrapper', 'authorize', 'get_single_item_df', 'run']
...
def run(self, code_list, postgres_creds, save_to_postgres=False):
    ...
    logger.info(f"SUCCESS: started scrapping multiple ({len(code_list)}) items")

    dfs = []

    for code in tqdm(code_list):
        # try to scrap code, except - pass it:
        try:
            start_time = time()
            ...
            sleep(60)
            logger.info(f"Finished waiting 60 s")
            pass
...
def authorize(self):
    ...
    # wait page up to 60 secs to upload authorization page:
    WebDriverWait(driver, 60).until(EC.presence_of_element_located((By.CSS_SELECTOR, "input[name='email']")))
    ...
    # wait up to 60 secs until redirect the main page:
    WebDriverWait(driver, 60).until(EC.url_to_be("https://app.shopstat.ru/"))

    # wait up to 60 secs until main page to upload:
    WebDriverWait(driver, 60).until(EC.presence_of_element_located((By.CSS_SELECTOR, ".MuiCircularProgress-root"))) # graph on page
...
def get_single_item_df(self, code):
    ...
    # wait page up to 60 secs to upload either exit button or item table: (if exit banner appears it would only waste 5 secs once trying to wait for item_table_locator to upload)
    ...
    try:
        WebDriverWait(driver, 60).until(EC.presence_of_element_located(item_table_locator))
    except:
        WebDriverWait(driver, 60).until(EC.presence_of_element_located(exit_button_locator))
    ...
    try:
        ...
        sleep(1)
        # wait page up to 60 secs to upload item table:
        WebDriverWait(driver, 60).until(EC.presence_of_element_located(item_table_locator))
    except:
        pass
...
def run(self, code_list, postgres_creds, save_to_postgres=False):
    ...
    dfs = []
    for code in tqdm(code_list):
        # try to scrap item, except - pass it:
        try:
            start_time = time()

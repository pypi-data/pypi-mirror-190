...
class NeyroNet:
	...
	def multithread_train_network(self, train, l_rate, n_epoch, threads=2, err_val_threshold=0):
		threads_training = [[] for _ in range(threads)]
		i = 0
		for data in train:
			threads_training[i].append(data)
			i += 1
			if(i >= threads): i = 0
			
		threads_list = []
		for i in range(threads):
			threads_list.append(Thread(target=self.train_network, args=(threads_training[i], l_rate, int(n_epoch/threads), err_val_threshold)))

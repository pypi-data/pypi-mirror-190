...
class MonitorThread(threading.Thread):
    def __init__(self,platform,device,package,filename=None,save_detail=False,divided_core_nums=False):
    ...
    def run(self):
        try:
            if self.platform == "android":
                ...
                dump_thread.start()
                self.monitor_pro  = app_monitor(1, self.filename, self.package, serial=self.device, duration=3600 * 24,
                ...
                cmd = f'"{sys.executable}" "{os.path.join(ROOT_DIR, "ios_monitor.py")}" -filename "{self.filename}" ' \
                      f'-time {3600 * 24} -process {process_name} -udid {self.device} -bundle_id {self.package} ' \
                ...
                self.monitor_pro  = PCMonitorThread(self.package, timeInterval=1, testTime=3600 * 24, filename=self.filename,

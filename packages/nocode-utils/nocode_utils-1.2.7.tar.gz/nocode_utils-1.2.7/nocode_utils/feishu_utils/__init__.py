    ...
    def rate_limiter(self, path, function_name, line_number, emails):
        ...
        for email in emails:
            if email not in self.rate_limit:
                self.rate_limit[email] = {key: current_time}
                valid_emails.append(email)
            else:
                if key not in self.rate_limit[email]:
                    self.rate_limit[email][key] = current_time
                    valid_emails.append(email)
                else:
                    last_time = self.rate_limit[email][key]
                    if int(current_time) - int(last_time) > self.interval:
                        self.rate_limit[email][key] = current_time
                        valid_emails.append(email)
        return valid_emails

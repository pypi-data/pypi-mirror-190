...
class MultiWechatWebhookPlugin(notify.NotificationPlugin):
    ...
    def notify_users(self, group, event, fail_silently=False, **kwargs):
        ...
        title = u"有新的通知来自 {} 项目".format(event.project.slug + (event.environment if (not event.project.slug.endswith("prod") and not event.project.slug.endswith("int") and event.environment) else ""))
        data = {
            "msgtype": "markdown",
            "markdown": {
                "content": u"#### {title} \n > {message} \n [查看]({url})".format(

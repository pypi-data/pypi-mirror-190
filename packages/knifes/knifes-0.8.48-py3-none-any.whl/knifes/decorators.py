...
def decrypt_and_check_params(param_keys: Set = None, header_param_keys: Set = None):
    ...
    def outer_wrapper(view_func):
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            if not request.headers.get('xDev'):
            ...
            # if request.headers.get('xBody'):  # 新版接口
            #     if not request.headers.get('xDev') and request.params != json.loads(aes.decrypt(request.headers.get('xBody'), settings.AES_BODY_KEY)):
            #         return ApiResult.missingParam(_('参数非法'))
            #
            # if request.params.get('vs'):    # 老版接口

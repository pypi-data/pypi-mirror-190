...
def decrypt_and_check_params(param_keys: Set = None, header_param_keys: Set = None):
    ...
    def outer_wrapper(view_func):
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            if not request.headers.get('vi', request.headers.get('xHeader')):
                ApiResult.missingParam()
            request.header_params = json.loads(aes.decrypt(request.headers.get('vi', request.headers.get('xHeader')), settings.AES_HEADER_KEY))
            # 检查参数

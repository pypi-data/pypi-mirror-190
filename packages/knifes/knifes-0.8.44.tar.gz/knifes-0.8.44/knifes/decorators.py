...
def decrypt_and_check_params(param_keys: Set = None, header_param_keys: Set = None):
    ...
    def outer_wrapper(view_func):
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            encrypt_params = {}
            is_online = not request.headers.get('xDev')
            if is_online:
                encrypt_params = json.loads(aes.decrypt(request.params.get('vs', request.headers.get('xBody')), settings.AES_BODY_KEY))
            ...
            if not (request.headers.get('vi') or request.headers.get('xHeader')):
                ApiResult.missingParam()
            request.header_params = json.loads(aes.decrypt(request.headers.get('vi') or request.headers.get('xHeader'), settings.AES_HEADER_KEY))
            # 检查参数

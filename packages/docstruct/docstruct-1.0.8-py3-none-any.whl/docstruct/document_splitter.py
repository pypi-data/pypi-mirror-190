from docstruct import Document, Character, TextBlock


class DocumentSplitter:
    def __init__(self, document: Document, byte_threshold: int, encoding: str):
        self.document = document
        self.threshold = byte_threshold
        self.encoding = encoding
        self.chunks: list[list[TextBlock]] = []
        self.text_blocks_size: dict[TextBlock:int] = {}
        self.set_text_byte_size(document)
    ...
    def get_chunks(self) -> list[list[TextBlock]]:
        self._split_to_chunks(self.document)
        self.add_current_chunk()
        return self.chunks

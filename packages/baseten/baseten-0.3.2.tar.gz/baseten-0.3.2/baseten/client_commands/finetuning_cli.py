...
@click.option(
    ...
    required=True,
    help="Type of finetuning job to run.",
)
@click.option("--name", required=True, help="Name for this finetuning job")
...
def deploy(run_id: str, idle_time_minutes: int):
    ...
    click.echo(f"Access the deployed model at {deployed_model.blueprint_url}")

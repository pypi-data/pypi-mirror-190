    ...
    if not os.path.exists(path):
        try:
            ...
            if exc.errno == EEXIST and os.path.isdir(path):
                return
            logger.error("Folder " + path + "does not exist and unable to create folder - consider using environment variable NORGATEDATA_ROOT for a different folder" )
            sys.exit()

    # Check folder is writeable
    checkfilename = path + "/" + "checkfile.tmp"
    try:
        ...
        logger.error("... consider using environment variable NORGATEDATA_ROOT for a different folder")
        sys.exit()

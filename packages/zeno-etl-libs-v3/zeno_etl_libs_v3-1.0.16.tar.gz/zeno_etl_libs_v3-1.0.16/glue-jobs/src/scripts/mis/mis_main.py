...
parser.add_argument('-dt', '--mis_db_date', default="2022-05-01", type=str, required=False)
...
parser.add_argument('-cm', '--choose_month', default="04", type=str, required=False)
...
try:
    ...
    fofo_breakup_format = pd.read_csv(
        r'D:\MIS Automation\FOFO Additional Field\Return_net_value_including_tax\Format\breakup_master_fofo_format.csv')
    fofo_unified_format = pd.read_csv(
        r'D:\MIS Automation\FOFO Additional Field\Return_net_value_including_tax\Format\unified_master_fofo_format.csv')

    breakup_master_fofo = fofo_breakup_format.merge(breakup_master_fofo,
                                                    on=['tag_flag', 'fofo_distributor'], how='outer')
    unified_master_fofo = fofo_unified_format.merge(unified_master_fofo,
                                                    on=['tag_flag', 'fofo_distributor'], how='outer')

    cols_to_move = ['tag_flag', 'fofo_distributor']
    ...
    breakup_master.to_csv(r'D:\MIS\MIS Net_value_0 Issue\Zippin_returns_corrected_data\breakup_master_{}.csv'.format(choose_month),
                          index=False)
    breakup_master_fofo.to_csv(
        r'D:\MIS\MIS Net_value_0 Issue\Zippin_returns_corrected_data\breakup_master_fofo_{}.csv'.format(
            choose_month), index=False)
    unified_master_fofo.to_csv(
        r'D:\MIS\MIS Net_value_0 Issue\Zippin_returns_corrected_data\unified_master_fofo_{}.csv'.format(
            choose_month), index=False)
    unified_master.to_csv(
        r'D:\MIS\MIS Net_value_0 Issue\Zippin_returns_corrected_data\unified_master_{}.csv'.format(
            choose_month), index=False)
...
s3.write_to_db_from_s3_csv(table_name='drugs-mis-2022-05-01',file_s3_uri='s3://prod-mis-datalake/drug-tables-old/drugs-2022-05-01/',db=rs_db,schema='public')
...
athena = Athena()
# conn = athena.connection(schema_name='mis-datalake-drug-tables-old')
return_npi_saleable = pd.read_csv(r'D:\MIS\tables\old_drug_table\mis_2022_05_01.csv')

return_npi_saleable['company'] = 'a'


db = DB()
db.open_connection()
schema = 'public'
table_name = 'drugs-mis-2022-05-01'

...
parser.add_argument('-mqb', '--minimum_quantity_buffer_at_sending_store', default="1", type=str, required=False)
...
minimum_quantity_buffer_at_sending_store = args.minimum_quantity_buffer_at_sending_store
...
logger.info("minimum_quantity_buffer_at_sending_store- " + str(minimum_quantity_buffer_at_sending_store))
logger.info("")
...
minimum_quantity_buffer_at_sending_store = int(minimum_quantity_buffer_at_sending_store)
# =============================================================================
# store clusters
# =============================================================================
...
for cluster in cluster_list:
    ...
    for i in cluster_stores:
        ...
        # Keeping minimum_quantity_buffer_at_sending_store
        clus_inv['ci-orignal'] = clus_inv['ci']
        clus_inv['ci'] = clus_inv['ci'] - int(minimum_quantity_buffer_at_sending_store)
        clus_inv = clus_inv[clus_inv['ci']>0]
        ...
        orders_clus_inv = orders_clus_inv[~(orders_clus_inv['store-id-x'] ==
                                orders_clus_inv['clus-store-id'])]

        clus_inv_all_sum = orders_clus_inv.groupby(['order-number','pso-id','drug-id'],
                                            as_index=False).agg({
            'clus-store-inv': ['sum']
        }).reset_index(drop=True)
        clus_inv_all_sum.columns = ["-".join(x) for x in clus_inv_all_sum.columns.ravel()]
        clus_inv_all_sum.rename(columns={'order-number-':'order-number',
                                         'pso-id-':'pso-id',
                                         'drug-id-': 'drug-id',
                                         'clus-store-inv-sum': 'clus-inv'}, inplace=True)

        orders_clus_inv = pd.merge(left=orders_clus_inv,
                                   right=clus_inv_all_sum,
                                   how='left', on=['order-number','pso-id','drug-id'], suffixes=('-x', '-y')).rename(
            columns={'store-id-y': 'clus-store-id'})
    ...
    # Adding Last store in multistore
    ...
    # Handling Case when In case of Multistore, If by adding last store total inventory to order is greater than quantity to order
    ff_using_multi_store['clus-store-inv'] = np.where((((ff_using_multi_store['cumsum'] ==
                                              ff_using_multi_store['temp']) &
                                             (ff_using_multi_store['cond'] == 'red'))
                                            &(ff_using_multi_store['cumsum']>ff_using_multi_store['quantity-to-order'])),
                                            ff_using_multi_store['clus-store-inv'] - (ff_using_multi_store['cumsum']-ff_using_multi_store['quantity-to-order']),
                                            ff_using_multi_store['clus-store-inv'])

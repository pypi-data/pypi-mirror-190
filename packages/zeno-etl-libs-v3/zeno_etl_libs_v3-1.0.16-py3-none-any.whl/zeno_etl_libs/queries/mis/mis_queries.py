...
sales_query = """
            ...
            left join "prod2-generico"."inventory-1" b on
                a."inventory-id" = b."id"
            left join public."drugs{suffix_to_table}" c on
                c."id" = b."drug-id"
...
customer_returns_query = """
 SELECT
    ...
    JOIN "prod2-generico"."customer-return-items-1" b ON
        a."id" = b."return-id"
    LEFT JOIN "prod2-generico"."inventory-1" c ON
        c."id" = b."inventory-id"
    LEFT JOIN public."drugs{suffix_to_table}" d ON
        d."id" = c."drug-id"
...
inventory_query = """
        ...
        left join public."drugs{suffix_to_table}" b on
            a."drug-id" = b."id"
...
purchase_from_wc_query = """
    ...
    left join
            public."drugs{suffix_to_table}" c on
        c."id" = b."drug-id"
    join
...
zippin_return_data_query = """
    ...
    from
        ...
        left join "prod2-generico"."inventory-1" y on
            y."id" = r."inventory-id"
        left join  public."drugs{suffix_to_table}" p on
            p."id" = y."drug-id"
        ...
        where
            ...
            and dni."is-active" = 1
            -- and d."store-id" not in (295, 293, 302, 282, 313, 291, 283, 316, 311, 299, 303, 298)
...
workcell_return_data_query = """
        ...
        left join "prod2-generico"."inventory" y on
            r."inventory-id"= y."id"
        left join public."drugs{suffix_to_table}" o on
            o."id" = y."drug-id"
...
local_purchase_data_query = """
        ...
        left join
                public."drugs{suffix_to_table}" d on
            a."drug-id" = d."id"
        left join
...
ethical_margin_query = """
    ...
    join public."drugs{suffix_to_table}" d on
        d."id" = a."drug-id"
    where
        c."credit-period">0
        and d."type" = 'ethical'
...
other_files_inventory_at_dc_near_expiry_data_query = """
    ...
    join "prod2-generico"."invoice-items" b on
        a."id" = b."invoice-id"
    join public."drugs{suffix_to_table}" c on
        b."drug-id" = c."id"
    where
        "status" = 'approved'
...
goodaid_dc_inventory_query = """
    ...
    from
        ...
        join "prod2-generico"."invoice-items" b on
            a."invoice-item-id" = b.id
        join public."drugs{suffix_to_table}" dgs on
            dgs.id = a."drug-id"
        where
            "dgs"."company" = 'GOODAID'
        group by

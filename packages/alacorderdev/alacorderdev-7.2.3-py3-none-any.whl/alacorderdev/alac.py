...
def map(conf, method) -> pd.DataFrame: # test!  
	batches = conf['batches']
	path_in = conf['in_path']
	path_out = conf['out_path']
	case_max = conf['case_max']
	...
	start_time = time.time()
	on_batch = 0
	allCustomOutputs = pd.DataFrame()
	allCustomOutputsList = []
	uselist = False
	for i, c in enumerate(batches):
		exptime = time.time()
		b = pd.DataFrame()
		...
		b['CaseInfoOutputs'] = b['AllPagesText'].map(lambda x: getCaseInfo(x))
		b['CaseNumber'] = b['CaseInfoOutputs'].map(lambda x: x[0])
		customOutputs = b['AllPagesText'].map(lambda x: method(x))
		try:
			allCustomOutputs.append(customOutputs, ignore_index=True)
		except (NameError, TypeError):
			allCustomOutputsList = pd.DataFrame({b['CaseNumber']:customOutputs},index=[i])
			uselist = True
		on_batch += 1
		console_log(conf, on_batch,exptime,'Exporting charges to table...')
	log_complete(conf, start_time)
	on_batch = 0
	if uselist == True:
		return allCustomOutputsList
	else:
		return allCustomOutputs

def write(conf, method) -> pd.DataFrame: # test! 
	batches = conf['batches']
	path_in = conf['in_path']
	path_out = conf['out_path']
	case_max = conf['case_max']
	...
	for i, c in enumerate(batches):
		...
		if out_ext == "xls":
			with pd.ExcelWriter(path_out) as writer:
				output.to_excel(writer, sheet_name="output-table")
		elif out_ext == "pkl":
			output.to_pickle(path_out+".xz",compression="xz")
		elif out_ext == "xz":
			output.to_pickle(path_out,compression="xz")
		elif out_ext == "json":
			output.to_json(path_out)
		elif out_ext == "csv":
			output.to_csv(path_out,escapechar='\\')
		elif out_ext == "md":
			output.to_markdown(path_out)
		elif out_ext == "txt":
			output.to_string(path_out)
		elif out_ext == "dta":
			output.to_stata(path_out)
		elif out_ext == "no_export" or print_log == True:

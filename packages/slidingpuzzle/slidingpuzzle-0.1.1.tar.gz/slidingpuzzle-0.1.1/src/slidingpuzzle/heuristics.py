...
def has_row_conflict(
    ...
    ``ignore_x``). Optionally ignore a specific column.
    ...
    for col1 in range(w):
        ...
        if BLANK == tile1 or col1 == ignore_x or get_goal_y(h, w, tile1) != y:
            continue
        for col2 in range(col1 + 1, w):
            ...
            tile2 = board[y, col2]
            # check conflict conditions
            if BLANK == tile2 or col2 == ignore_x or get_goal_y(h, w, tile2) != y:
                continue
...
def has_col_conflict(
    ...
    for row1 in range(h):
        tile1 = board[row1, x]
        # check conflict conditions
        if BLANK == tile1 or row1 == ignore_y or get_goal_x(h, w, tile1) != x:
            continue
        for row2 in range(row1 + 1, h):
            ...
            tile2 = board[row2, x]
            # check conflict conditions
            if BLANK == tile2 or row2 == ignore_y or get_goal_x(h, w, tile2) != x:
                continue

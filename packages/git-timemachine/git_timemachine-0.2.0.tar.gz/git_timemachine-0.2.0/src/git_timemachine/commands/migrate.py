...
def amend_commit(src_repo, dest_repo, log, pre_cmd):
    os.chdir(src_repo)
    diff_file = f'{tempfile.gettempdir()}/{tempfile.mktemp()}' % ()
    ...
    os.system(f'git apply --index --binary {diff_file}')
    os.system('git add .')
    os.system("git commit --message='%s' --date='%s'" % (msg.replace("'", "\\'"), log['date']))
...
def migrate_command(ctx, log_file, growth, execute, src_repo, dest_repo):
    ...
    for log in logs:
        ...
        amend_commit(src_repo, dest_repo, log, execute)

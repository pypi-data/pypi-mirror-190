...
class Curl:
    ...
    def _setup(self, url, method = "GET", request_version = "HTTP/1.1"):
        ...
        self.easy.setopt(pycurl.CONNECTTIMEOUT, 5)
    ...
    def bridge(self, client):
        ...
        dprint("Setting up bridge")
        self.client = client

        # Setup read/write callbacks
        self.easy.setopt(pycurl.READFUNCTION, self._read_callback)
        self.easy.setopt(pycurl.WRITEFUNCTION, self._write_callback)
        self.easy.setopt(pycurl.HEADERFUNCTION, self._write_callback)
    ...
    def set_useragent(self, useragent):
        ...
        self.easy.setopt(pycurl.USERAGENT, useragent)
...
class MCurl:
    ...
    def _socket_action(self, sock_fd, ev_bitmask):
        ...
        dprint("%d, %d, %d" % (status, handle_count, len(self._handles)))
    ...
    def add(self, curl: Curl):
        ...
        while True:
            with self._lock:
                dprint("Handles = %d" % len(self._handles))
    ...
    def perform(self):
        ...
        with self._lock:
            rlen = len(self._rlist)
            wlen = len(self._wlist)
            if rlen != 0 or wlen != 0:
    ...
    def select(self, curl: Curl, idle = 30):
        ...
        while (rlist or wlist):
            ...
            if exs:
                dprint("Exception, breaking")
                break
            if ins:
                dprint("Read activity for " + curl.url)
                for i in ins:
                    if i is curl_sock:
                        out = curl.client
                        wdata = cdata
            ...
            if max_idle < time.time():
                ...
                dprint("Server connection timeout")
                break

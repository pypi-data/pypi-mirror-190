...
def generator_of_samples():
    for i in range(100):
        yield {'description': f'descr{i}', 'tokens': np.zeros(i), 'idx': i}
...
def sample_generator(tokenizer, data, batch_size):
    for i in range(0, len(data), batch_size):
        yield tokenizer(data[i: i + batch_size])


def test_create_from_hugginface_tokenization(tmp_path):
    ...
    df_mmap = DataFrameMmap.from_generator(
        ...
        sample_generator=sample_generator(tokenizer, texts, 4),
        batch_size=2,
        mode='batch',
        verbose=True
    )

    assert df_mmap.get_columns() == ['token_type_ids', 'attention_mask', 'input_ids']

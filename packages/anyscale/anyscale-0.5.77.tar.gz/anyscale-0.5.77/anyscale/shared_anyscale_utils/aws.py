...
class AwsArn:
    ...
    def _validate_fields(self):
        ...
        if not isinstance(self.partition, str):
            raise ValueError(
                "Partition is of type {}, not str".format(type(self.partition))
            )

        if not isinstance(self.service, str):
            raise ValueError(
                "Service is of type {}, not str".format(type(self.service))
            )

        if not isinstance(self.region, (str, type(None))):
            raise ValueError(
                "Region is of type {}, not Optional[str]".format(type(self.region))
            )
        ...
        if not isinstance(self.resource_id, str):
            raise ValueError(
                "ResourceId is of type {}, not str".format(type(self.resource_id))
            )

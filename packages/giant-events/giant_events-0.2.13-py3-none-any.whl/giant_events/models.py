...
class AbstractEvent(TimestampMixin, PublishingMixin, URLMixin):
    ...
    content = PlaceholderField(slotname="event_content", related_name="%(app_label)s_content")
    ...
    location = models.ForeignKey(
        to=Location, null=True, on_delete=models.SET_NULL, related_name="%(app_label)s"
    )
...
class Event(AbstractEvent):
    """
    Base Event model implementation. Inheriting directly from the
    AbstractEvent class with no further customisation.
    """

    class Meta(AbstractEvent.Meta):
        swappable = swapper.swappable_setting("giant_events", "Event")
        verbose_name = "Event"
        verbose_name_plural = "Events"

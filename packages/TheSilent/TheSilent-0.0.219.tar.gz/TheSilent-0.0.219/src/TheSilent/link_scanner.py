...
def link_scanner(url, secure = True, tor = False, depth = 1, my_file = " ", crawl = "all", parse = " ", title = False):
    #crawl all
    if crawl == "all":
        if secure == True:
            my_secure = "https://"
            ...
            if parse == " ":
                ...
                for i in website_list:
                    try:
                        ...
                        if tor == True:
                            my_request = web_session.get(i, verify = False, headers = user_agent, proxies = tor_proxy, timeout = (60, 120)).text

                        if tor == False:
                            my_request = web_session.get(i, verify = False, headers = user_agent, timeout = (5, 30)).text

                        result = re.findall("<title>.+</title>", my_request)
                        clean = result[0].replace("<title>", "")
                        clean = clean.replace("</title>", "")
                        title_list.append(clean + ": " + i)

                        print(cyan + clean + ": " + i)

                    except IndexError:
                        print(red + "ERROR! (NO TITLE) " + i)
        ...
        if title == True:
            ...
            if parse == " ":
                ...
                while True:
                    ...
                    if crawl_count >= crawl:
                        try:
                            website_list.pop(crawl_count)
                            crawl_count -= 1

...
class redisQueueConfirm():
    def __init__(self, rdb:redis.Redis, name:str, length:int=0, timeout:int=300):
        ...
        self.collectorLock = self.rdb.lock("redis_lock:redisQueueConfirmdCollectorLock", timeout=30)
        self.queueOperaLock = self.rdb.lock("redis_lock:%s:queueOperaLock" % name, timeout=5)

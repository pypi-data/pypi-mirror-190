...
class JSVisitorContext(VisitorContext):
    ...
    def copy(
        ...
        node: t.Optional[ast.AST] = None,
        scope: t.Optional[JSScope] = None,
        break_suffix: t.Optional[str] = None,
        has_yield: t.Optional[bool] = None
    ) -> 'JSVisitorContext':
...
class CodeGen(Visitor[JSVisitorContext]):
    ...
    def visit_For(self, node: ast.For, ctx: JSVisitorContext):
        ...
        if isinstance(node.target, ast.Name):
            if node.target.id not in ctx.variables:
                target.write(f'let')
    ...
    def visit_Try(self, node: ast.Try, ctx: JSVisitorContext):
        return (
            ...
            .write_if(node.handlers, "catch(_err) {%s}" % '\n'.join(
                BaseBuilder()
                .write_if(handler.type, lambda: f'if (_err instanceof {self.visit(handler.type, ctx)})')
                .write(SurroundBuilder(surround='{}', separator=';')

...
class DockerRunner(object):
    def __init__(self, images=None, args=''):
        ...
        self.docker_cmd = f"docker run {args} -t --rm {COMMON_VOLUMES} {CURRENT_USER} {args} --workdir={CWD} {self.DOCKER_IMAGE_NAME} "
    ...
    def run(self, args):
        ...
        if os.system(self.docker_cmd + args) != 0:
...
class BuildCommand:
    ...
    def run(self):
        ...
        if os.path.exists("/.dockerenv") or not opt.docker:
            ...
            if hasattr(opt, 'gdbserver') and opt.gdbserver == True:
                opt.docker_args += ' -p 1234:1234'
            if opt.func.__name__ in ["conan_shell", "conan_run"]:
                opt.docker_args += ' -i'
            DockerRunner(opt.docker_image, opt.docker_args).run(sys.argv)

    ...
    def add_argument(self, parser):
        for _, output in self.output_data.items():
            if output.get('platform') == 'qemu-arm-virt':
                parse_args(parser)
                return
        parser.add_argument('-u', '--serial_device', default=None,
...
class DockerRunner(object):
    def __init__(self, images=None, interactive=False):
        ...
        self.docker_cmd = f"docker run -{interactive_str}t --rm {COMMON_VOLUMES} {CURRENT_USER} -p 1234:1234 --workdir={CWD} {self.DOCKER_IMAGE_NAME} "

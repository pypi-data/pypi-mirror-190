...
from feagi_agent import configuration as feagi_config
# from robot_controllers.freenove_smartcar import configuration as agent_config
        ...
        try:
            ...
            runtime_data["feagi_state"] = \
                router.register_with_feagi(feagi_ip=feagi_host,
                                           ...
                                           agent_type=feagi_config.agent_settings['agent_type'],
                                           agent_id=feagi_config.agent_settings['agent_id'],
                                           agent_ip=runtime_data["host_network"]["ip_address"],
                                           agent_data_port=feagi_config.agent_settings['agent_data_port'],
                                           agent_capabilities=feagi_config.capabilities)
...
def feagi_setting_for_registration():
    ...
    FEAGI
    """
    feagi_ip_host = feagi_config.feagi_settings["feagi_host"]
    api_port = feagi_config.feagi_settings["feagi_api_port"]
    app_data_port = feagi_config.agent_settings["agent_data_port"]
...
def msg_processor(self, msg, msg_type):
    ...
    if 'ultrasonic' in msg_type and msg.ranges[1]:
        ...
        if avg_intensity > feagi_config.capabilities["infrared"]["threshold"]:
            return {
                'ir': {
                    sensor_id: False
                }
...
def control_data_processor(data):
    ...
    if control_data is not None:
        if 'motor_power_coefficient' in control_data:
            feagi_config.capabilities["motor"]["power_coefficient"] = float(control_data['motor_power_coefficient'])
        if 'robot_starting_position' in control_data:
            for position_index in control_data['robot_starting_position']:
                feagi_config.capabilities["position"][position_index]["x"] = \
                    float(control_data['robot_starting_position'][position_index][0])
                feagi_config.capabilities["position"][position_index]["y"] = \
                    float(control_data['robot_starting_position'][position_index][1])
                feagi_config.capabilities["position"][position_index]["z"] = \
                    float(control_data['robot_starting_position'][position_index][2])
        return feagi_config.capabilities["motor"]["power_coefficient"], \
               feagi_config.capabilities["position"]

...
import requests
import sys
from feagi_agent.robot_controllers.freenove_smartcar.Led import *
from feagi_agent.robot_controllers.freenove_smartcar.PCA9685 import PCA9685
import feagi_agent.robot_controllers.freenove_smartcar.configuration as freenove_config
...
class Servo:
    ...
    def move(self, feagi_device_id, power):
        try:
            if feagi_device_id > 2 * freenove_config.capabilities['servo']['count']:
            ...
            self.device_position = float((power * freenove_config.network_settings['feagi_burst_speed'] / 0.5) +
    ...
    def motor_converter(motor_id):
        ...
        """
        # motor_total = freenove_config.capabilities['motor']['count'] #be sure to update your motor total in configuration.py
        # increment = 0
...
def main():
    ...
    agent = FEAGI.get_agent()
    runtime_data["feagi_state"] = FEAGI.feagi_registration(feagi_host=feagi_host,
                                                           api_port=api_port)
    ipu_channel_address = FEAGI.feagi_inbound(agent.agent_settings["agent_data_port"])
    opu_channel_address = FEAGI.feagi_outbound(agent.feagi_settings['feagi_host'],
    ...
    agent.feagi_settings['feagi_burst_speed'] = float(runtime_data["feagi_state"]['burst_duration'])
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #
    ...
    flag = False
    keyboard_flag = True
    rolling_window_len = freenove_config.capabilities['motor']['rolling_window_len']
    motor_count = freenove_config.capabilities['motor']['count']
    msg_counter = 0
    # rpm = (50 * 60) / 2
    # DC motor has 2 poles, 50 is the freq and it's constant (why??) and 60 is the
    # seconds of a minute
    # w = (rpm / 60) * (2 * math.pi)  # 60 is second/minute
    # velocity = w * (configuration.freenove_config.capabilities['motor']['diameter_of_wheel'] / 2)
        ...
        try:
            for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
                if keyboard_flag:
                    ...
                    if freenove_config.capabilities['camera']['disabled'] is not True:
                        retina_data = retina.frame_split(image,
                                                         freenove_config.capabilities['camera']['retina_width_percent'],
                                                         freenove_config.capabilities['camera']['retina_height_percent'])
                        for i in retina_data:
                            if 'C' in i:
                                retina_data[i] = retina.center_data_compression(retina_data[i],
                                                                                freenove_config.capabilities['camera'][
                            ...
                            if 'prev' not in i:
                                ...
                                if 'C' in i:
                                    ...
                                    rgb_data, previous_data_frame[previous_name] = \
                                        retina.get_rgb(data,
                                                       freenove_config.capabilities[
                                                           'camera'][
                                                           'central_vision_compression'],
                                                       previous_data_frame[
                                                           previous_name],
                                                       name,
                                                       freenove_config.capabilities['camera']['deviation_threshold'])
                                else:
                                    previous_name = str(i) + "_prev"
                                    rgb_data, previous_data_frame[previous_name] = \
                                        retina.get_rgb(data, freenove_config.capabilities['camera']['peripheral_vision_compression'],
                ...
                if ir_data:
                    for ir_sensor in range(int(freenove_config.capabilities['infrared']['count'])):
                ...
                if message_from_feagi is not None:
                    ...
                    if freenove_config.capabilities['motor']['disabled'] is not True:
                    ...
                    if freenove_config.capabilities['servo']['disabled'] is not True:
                ...
                if flag == 10:
                    ...
                    if msg_counter < feagi_burst_counter:
                        feagi_opu_channel = FEAGI.sub_initializer(opu_address=opu_channel_address)
                        if feagi_burst_speed != agent.feagi_settings['feagi_burst_speed']:
                            agent.feagi_settings['feagi_burst_speed'] = feagi_burst_speed
                for id in range(motor_count):
                    motor_power = window_average(rolling_window[id])
                    motor_power = motor_power * freenove_config.capabilities["motor"]["power_amount"]

...
def populate_resource_with_assumed_values(
    hub, resource, resource_type, override_none=False, convert_to_present: bool = False
):
    assumed_values = (
    ...
    for missing_key, assumed_value in assumed_values.items():
        if missing_key not in resource or (
            override_none and resource[missing_key] is None
        ):

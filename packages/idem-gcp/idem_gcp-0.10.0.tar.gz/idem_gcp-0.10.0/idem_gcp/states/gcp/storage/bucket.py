...
async def present(
    ...
    r"""Create or update a storage bucket resource. Changes to the bucket will be readable immediately after writing, but configuration changes may take time to propagate.
        ...
        Args:
            ...
            retention_policy(Dict[str, Any], optional): The bucket's retention policy. The retention policy enforces a minimum retention time for all objects contained in the bucket, based on their creation time. Any attempt to overwrite or delete objects younger than the retention period will result in a PERMISSION_DENIED error. An unlocked retention policy can be modified or removed from the bucket via a storage.buckets.update operation. A locked retention policy cannot be removed or shortened in duration for the lifetime of the bucket. Attempting to remove or decrease period of a locked retention policy will result in a PERMISSION_DENIED error. Defaults to None.
                * effective_time (str, optional): Server-determined value that indicates the time from which policy was enforced and effective. This value is in RFC 3339 format.
                * is_locked (bool, optional): Once locked, an object retention policy cannot be modified.
    ...
    if resource_id:
        ...
        resource_id = hub.tool.gcp.resource_prop_utils.construct_resource_id(
            "storage.bucket", {**locals(), "bucket": name}
        )
    ...
    if old:
        changes = hub.tool.gcp.utils.compare_states(old, plan_state, "storage.bucket")
        ...
        # always take new_state from get, as it returns assumed default values
        await _get_new_state(hub, ctx, user_project, result, resource_id)
        ...
        if ctx["test"]:
            ...
            # if None is given, then a potential create request would set the default value
...
async def _get_new_state(hub, ctx, user_project, result, resource_id):
    get_ret = await hub.exec.gcp.storage.bucket.get(
        ctx, resource_id=resource_id, projection="full", user_project=user_project
    )

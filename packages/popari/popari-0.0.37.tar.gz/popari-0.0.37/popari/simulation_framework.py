...
def synthesize_metagenes_nsf(num_genes, num_spatial_metagenes, n_nonspatial_metagenes, spatial_metagene_parameter, nonspatial_metagene_parameter,  original_metagenes=None, replicate_variability=None, normalize=True):
    """Test nsf metagene synthesis
    
    """
    # TODO: delete the below lines for generalizability
    spatial_metagene_parameter = 12
    nonspatial_metagene_parameter = 8
    
    num_metagenes = num_spatial_metagenes + n_nonspatial_metagenes 
    metagenes = np.zeros((num_metagenes, num_genes))

#     last_index = None
    for index in range(num_spatial_metagenes):
        gene_indices = np.random.choice(num_spatial_metagenes, size=num_genes, replace=True)
        metagenes[index, gene_indices] = spatial_metagene_parameter
            
    for index in range(num_spatial_metagenes, num_metagenes):
        gene_indices = np.random.choice(n_nonspatial_metagenes, size=num_genes, replace=True)
        metagenes[index, gene_indices] = nonspatial_metagene_parameter
    
    if normalize:
        magnitudes = np.sum(metagenes, axis=1, keepdims=True)
        metagenes = metagenes / magnitudes
       
    return metagenes, magnitudes
...
def synthesize_cell_embeddings_nsf(layer_labels, distributions, cell_type_definitions, num_cells, num_real_metagenes, n_noise_metagenes=3, signal_sigma_x=0.1, background_sigma_x=0.03, sigma_x_scale=1.0):
    """Generate synthetic cell embeddings.
...
class DomainCanvas():
    ...
    def __init__(self, points: np.ndarray, domain_names: Sequence[str], canvas_width: int = 400, density: float =10, precision=5):
        """Create DomainCanvas.

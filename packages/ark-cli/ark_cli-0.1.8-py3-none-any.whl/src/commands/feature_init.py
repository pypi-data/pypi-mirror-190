...
def init_feature(app_name: str, feature_name: str, package_name: str, platform: str):
    ...
    target_dir_name = app_name.title() + "-Feature-" + feature_name.title()
    ...
    git_clone(
        ArkDefinitions.REPO_ANDROID_FEATURE_TEMPLATE,
        project_dir,
        'main')
    ...
    if platform == Platform.ANDROID:
        __process_android_feature(project_dir, app_name, feature_name, package_name)
    else:
        __process_ios_feature()


def __process_android_feature(
        project_dir: str,
        app_name: str,
        feature_name: str,
        package_name: str
    ...
    find_files_recursively(
        project_dir,
        partial(__on_walk_android_feature_project_file,
                app_name=app_name,
    ...
    walk_dirs(
        project_dir,
        partial(__on_walk_android_feature_project_dir,
                feature_name=feature_name,
                package_name=package_name)
    )
...
def __on_walk_android_feature_project_file(
        file_name: str,
        app_name: str,
        feature_name: str,
        package_name: str
    ...
    if not is_binary(file_name):
        ...
        text = text.replace(ArkDefinitions.ANDROID_FEATURE_NAME, feature_name). \
            replace(ArkDefinitions.ANDROID_FEATURE_NAME.title(), feature_name.title()). \
            replace(ArkDefinitions.ANDROID_APP_NAME, app_name.title()). \
...
def __on_walk_android_feature_project_dir(dir_name: str, feature_name: str, package_name: str):
    new_dir_name = dir_name
    if ArkDefinitions.ANDROID_FEATURE_NAME in new_dir_name:
        new_dir_name = new_dir_name.replace(ArkDefinitions.ANDROID_FEATURE_NAME, feature_name)

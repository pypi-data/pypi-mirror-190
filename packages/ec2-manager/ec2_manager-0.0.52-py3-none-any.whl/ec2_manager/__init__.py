...
class EC2Manager:
    ...
    def _get_secret_envs(self, name):
        """
        ...
        for secret_file_name in self._config['instances'][name].get('secret_files', []):
            ...
            _, ext = os.path.splitext(secret_file_name)
            if ext == '.env':
                data = self._s3_client.get_object(
                    Bucket=self._aws_backend_bucket,
                    Key=f'{self._aws_backend_folder}/{secret_file_name}'
                )
                envs.update(dotenv.dotenv_values(stream=data['Body'], encoding='utf-8'))
        return envs
    ...
    def _update_repo(self, name):
        ...
        """
        self.upload_file_secret_files(name)
    ...
    def _get_file_secret_download_commands(self, name):
        ...
        for secret_file_name in self._config['instances'][name].get('secret_files', []):
            ...
            commands.append(
                f'aws s3 cp '
                f's3://{self._aws_backend_bucket}/{self._aws_backend_folder}/{secret_file_name} '
                f'{secret_file_name}'
            )
        return commands

    def upload_file_secret_files(self, name):
        """
        Uploads the secret files listed in the config.
        """
        for secret_file_name in self._config['instances'][name].get('secret_files', []):
            secret_file_name = secret_file_name.strip('/').replace('\\', '/')
            secret_file_path = os.path.join(os.getcwd(), 'compose', secret_file_name)
            if os.path.exists(secret_file_path):
                self._logger.info(f'Uploading secret file "{secret_file_path}"...')
                with open(secret_file_path, 'rb') as secret_file:
                    self._s3_client.upload_fileobj(
                        secret_file,
                        self._aws_backend_bucket,
                        f'{self._aws_backend_folder}/{secret_file_name}'
                    )

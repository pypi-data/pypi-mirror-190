...
def background_segmentation(data, idxs_laser_on, border_pix=10):
    ...
    # valid_pixels = largestCC*mask

    # background pixels are pixels when laser on which are not correlated with one another
    ...
    valid_pix_idxs = np.unravel_index(np.argsort(corr_pixels_laser_on, axis=None)[:np.rint(np.divide(np.sum(largestCC), 20)).astype(int)], corr_pixels_laser_on.shape)
    ...
    while np.sum(least_corr_pixels*mask) == 0:
        mask = np.zeros_like(img_to_segment)
        mask[border_pix:-border_pix, border_pix:-border_pix] = 1

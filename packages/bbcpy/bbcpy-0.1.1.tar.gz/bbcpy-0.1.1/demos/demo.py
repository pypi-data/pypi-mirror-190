...
clf = make_pipeline(vary, svm.SVC(C=1))
print(cross_val_score(clf, imagVPaw.lfilter(band).epochs(ival), ERD1.y, cv=cv))

# bbcpy.functions.statistics.cov=bbcpy.pipeline.FuncClass(bbcpy.functions.statistics.cov,target='trial') #moved to statistics.py

...
from sklearn.pipeline import _fit_one, _name_estimators
...
class Pipeline(sklearn.pipeline.Pipeline):
    ...
    def _transform(self, X, y=None):
        ...
        if y is None:
            ...
            for _, _, transform in self._iter():
                try:
                    Xt, yt = transform.transform(X=Xt, y=yt)
                except Exception:
...
def make_pipeline(*steps, memory=None, verbose=False):
    ...
    to the lowercase of their datatypes automatically.
    Parameters
    ----------
    *steps : list of estimators.
    ...
    for i, step in enumerate(steps):
        if type(step) is bbcpy.functions.base.ImportFunc:
            steps[i] = step.trafo()
        elif type(step) in [types.FunctionType, np.ufunc]: # callabel(step) could be an alternative but problematic
            # with some trafos e.g. CSO
            steps[i] = bbcpy.pipeline.AutoTransformer(step)
    #steps = tuple(steps)
    return Pipeline(sklearn.pipeline._name_estimators(steps), memory=memory,
                    verbose=verbose)
...
def make_union(*transformers, n_jobs=None, verbose=False):
    ...
    names automatically based on their datatypes. It also does not allow weighting.
    Parameters
    ----------

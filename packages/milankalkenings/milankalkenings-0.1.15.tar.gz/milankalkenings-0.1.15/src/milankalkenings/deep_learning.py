...
class Debugger(BatchHandler):
    ...
    def overfit_batch(self, module: Module,
                      batch_debug: list,
                      n_iters: int,
...
class Trainer(BatchHandler):
    ...
    def train(self, max_epochs: int, freeze_pretrained: bool = False):
        """
    ...
    def train_overkill_epoch(self, lr_initial: float, freeze_pretrained: bool):
        ...
        for i, batch in enumerate(self.setup.loader_train):
            ...
            lr = lr_initial
            for i in range(self.setup.overkill_max_decays + 1):
                module = load_secure(self.setup.checkpoint_running)
                optimizer = self.setup.optimizer_class(params=module.parameters(), lr=lr)
                self.train_batch(module=module, optimizer=optimizer, batch=batch, freeze_pretrained=freeze_pretrained)
    ...
    def train_overkill(self, max_epochs: int, freeze_pretrained: bool = False):
        ...
        for epoch in range(max_epochs):
            print("epoch", epoch + 1)
            best_lrs_epoch, loss_val_epoch, violations = self.train_overkill_epoch(freeze_pretrained=freeze_pretrained, lr_initial=lr)

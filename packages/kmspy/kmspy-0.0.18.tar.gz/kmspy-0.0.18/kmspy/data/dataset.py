...
class DataLoader:
    ...
    def make_gen(self, stop=False):
        if stop and hasattr(self, "_gen") and isinstance(self._gen, PrefetchGenerator):
            self._gen.stop()
        self._gen = PrefetchGenerator(self.dataset.data, num_prefetch=self._prefetch_factor*self._batch_size, num_workers=self._num_workers, shuffle=self._shuffle, processing_func=self._collate_fn, name="DataLoader", start=False)

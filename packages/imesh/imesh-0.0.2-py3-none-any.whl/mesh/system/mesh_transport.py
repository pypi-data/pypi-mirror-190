...
class MeshSession(Session):
    ...
    def with_context(self, fn: Any, remote: bool) -> Any:
        return Mesh.context_safe(lambda: self.context_safe(fn, remote))

    def context_safe(self, fn: Any, remote: bool) -> Any:
        if self.attachments:
    ...
    def peek(self) -> bytes:
        return self.with_context(lambda: self.session.peek(), False)

    def pop(self, timeout: int) -> bytes:
        return self.with_context(lambda: self.session.pop(timeout), False)

    def push(self, payload: bytes, metadata: Dict[str, str]):
        return self.with_context(lambda: self.session.push(payload, metadata), True)

    def release(self, timeout: int):
        return self.with_context(lambda: self.session.release(timeout), False)

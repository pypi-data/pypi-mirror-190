...
#        self.initConsoleWidget()
    ...
    def add_state_info(self,key):
        if key == 'ProductionHeader':
            ...
            self.state_cascades = self.legend_viewer.state_info('Cascades')

        elif key == 'Photons':
            self.state_photons['Photons'] = {'n_total': self.photons.n_tracks, 'n_steps': self.photons.n_steps}

        elif key == 'Tracks':
            self.state_tracks = self.legend_viewer.state_info('Tracks')
...
#        children.append(dict(name='show_nuclei',   title='show_nuclei',  type='bool', value=False))
...
#        elif name == 'show_nuclei':
#            self.legend_viewer.setVisible_legend_nuclei(value)

...
def basic():
	global proceed
	proceed = False
	print("Chad Extractor v2.8 ( github.com/ivan-sincek/chad )")
	print("")
	...
	if len(value) > 0:
		if key == "-t" and args["template"] is None:
			...
			if not os.path.exists(args["results"]):
				...
				args["results"] = [args["results"] + os.path.sep + file for file in os.listdir(args["results"]) if not file.endswith(ext)]
				if not args["results"]:
					error("No Chad results or plaintext files were found")
			else:

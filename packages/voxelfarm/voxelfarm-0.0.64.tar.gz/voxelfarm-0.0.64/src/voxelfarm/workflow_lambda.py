...
class workflow_lambda_host:
    ...
    def create_report(self, vf : voxelfarmclient.rest, workflow_request : request, update_type : str, name : str, report_lambda : str, region : str, lod : int, inputs : dict, fields : dict = None):
        script_dir = self.lambda_framework.get_working_dir()
        if report_lambda.startswith(script_dir):
            self.lambda_framework.log('view_lambda is an absolute path')
        else:
            self.lambda_framework.log('view_lambda is a relative path')
            report_lambda = os.path.join(script_dir, report_lambda)
    ...
    def create_export(self, vf : voxelfarmclient.rest, workflow_request : request, update_type : str, name : str, export_lambda : str, region : str, lod : int, inputs : dict):
        script_dir = self.lambda_framework.get_working_dir()
        if export_lambda.startswith(script_dir):
            self.lambda_framework.log('view_lambda is an absolute path')
        else:
            self.lambda_framework.log('view_lambda is a relative path')
            export_lambda = os.path.join(script_dir, export_lambda)

        lambda_file = open(export_lambda)
        result = vf.create_lambda_python(
            project=workflow_request.project_id, 
            type=vf.lambda_type.Report,

...
class BatteryDemoComposer(GenericDemoComposer):
    def render(self):
        ...
        for level in range(0, 101, 5):
            ...
            for charging in (False, True):
                for alt_mode in (False, True):
                    self._monitor._state.alt_mode_timeout = 1 if alt_mode else 0
        ...
        self._monitor._state.alt_mode_timeout = 0
        self._print_horiz_sep("Specials", total_width)
        self._print_rows(
            *[
                _format_special("Disabled", self._monitor._renderer.update_disabled()),
                _format_special("Config reloading", self._monitor._renderer.update_busy()),
                _format_special("Empty daemon data bus", self._monitor._renderer.update_no_data()),
                _format_special("Critical failure", self._monitor._renderer.update_on_error()),
                _format_special("Post-failure timeout", self._monitor._renderer.update_idle()),
                _format_special("Alt mode on A/C", label_alt_ac),
                _format_special("Initialzing", self._monitor._renderer.update_init()),
            ]
        )

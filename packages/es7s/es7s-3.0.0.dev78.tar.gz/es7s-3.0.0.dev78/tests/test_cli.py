...
import es7s.cli
import es7s.cli.exec_hilight_num
from es7s.shared import destroy_logger, destroy_io, init_logger, init_config, init_io, IoParams
...
def cli_streams(request) -> CommandStreams:
    ...
    yield CommandStreams(es7s.cli._entrypoint.callback, stdout, stderr)
...
class TestHelp:
    ...
    @pytest.mark.parametrize("group", es7s.cli._entrypoint.all_groups, ids=rt_str)
    ...
    @pytest.mark.parametrize("command", es7s.cli._entrypoint.all_commands, ids=rt_str)
...
class TestCommand:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ["exec hilight-num --demo", "text text"],
            ["exec list-dir .", 'total'],
    ...
    def test_command_run(self, cli_streams: CommandStreams, argstr: str, expected_output: str):
        ...
        assert result.exit_code == 0, "Exit code > 0"
        assert re.search(expected_output, cli_streams.stdout.read()), "Expected output not found"

...
class GenericRenderer(IMonitorRenderer):
    def __init__(
        ...
        self._monitor_setup = monitor_setup
    ...
    def _echo(self, string: RT) -> str:
        if self._monitor_setup.config.debugging_markup:
            string = self._apply_debugging_markup(string)

        if self._monitor_setup.cache_output:
    ...
    def _invalidate_cache(self):
        ...
        if not self._last_rendered_ts:
            return
        if time.time() - self._last_rendered_ts >= self._monitor_setup.cache_ttl:
    ...
    def update_primary(self) -> str:
        ...
        if self._monitor_setup.alt_mode and self._monitor_state.is_alt_mode:
            result.append(pt.Fragment(fmt=self._output_alt_st, close_this=False))

        if sep_left := self._monitor_setup.config.separator_left:
            result.append(sep_left)

        if self._monitor_setup.network_comm_indic:
            if self._monitor_state.network_comm:
                result.append(self._output_network_comm)
            else:
                result.append(pt.Fragment(" "))

        data_fmtd = self._monitor_state.data_fmtd
        if len(data_fmtd) > self._output_width:
            get_logger().warning(f"Primary string length exceeds max: {data_fmtd!r}")
        result.extend(data_fmtd._fragments)

        if sep_right := self._monitor_setup.config.separator_right:
    ...
    def render_progress_bar(self, result_parts: list[RT], max_len: int) -> pt.Text:
        ...
        if self._monitor_state.ratio:
            overw_st.append(self._monitor_setup.ratio_styles_map.find(self._monitor_state.ratio))
            fill_threshold = round(max_len * self._monitor_state.ratio)

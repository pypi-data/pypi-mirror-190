...
class Frame_Ti(tk.Frame):
    def __init__(self, master,exam):
        ...
        self.ti = []
        self.user_anwser = [-1 for i in range(len(exam))]  #0表示A,1表示B，2表示C，3表示D，-1为未选择
        self.index = 1
        self.exam = exam
        self.btns = []
...
class Window:
    def __init__(self,parent):
        ...
        self.parent = parent
        self.frame_ti = None
        self.exam = get_exam(r"data\data.json")
    ...
    def Frame_Tihao(self, parent,score_label):
        ...
        for i in range(ceil(len(self.exam)/10)):
            ...
            for j in range(10):
                ...
                btn["command"] = lambda x=k: self.frame_ti.refresh(x, self.exam[x-1],btns,score_label)

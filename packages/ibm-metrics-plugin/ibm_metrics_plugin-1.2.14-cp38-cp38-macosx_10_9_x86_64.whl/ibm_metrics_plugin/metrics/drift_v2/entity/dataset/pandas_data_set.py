...
class PandasDataSet(DriftDataSet):
    ...
    async def initialize(self,
        ...
        self.set_control_box(drift_controls=drift_controls)
        ...
        if (self.problem_type is ProblemType.REGRESSION) and (
            ...
            self.score_data(scoring_fn, data, prediction_column)

        elif (prediction_column not in self.data_schema or probability_column not in self.data_schema):
            self.score_data(scoring_fn, data, prediction_column, probability_column)
        ...
        if problem_type in (ProblemType.BINARY, ProblemType.MULTICLASS):
            ...
            accuracy_drift_flag = ControlBoxManager(
            ).get_control_box().get_enable_accuracy_drift()
    ...
    def score_data(
        ...
        if self.problem_type is ProblemType.REGRESSION:
            data[prediction_column] = self._split_and_score(scoring_fn, data)
            self.data_schema = {
                **self.data_schema,
                prediction_column: self.convert_column_dtypes_to_native(
                    data[prediction_column].dtype)}
        else:
            result = self._split_and_score(scoring_fn, data)

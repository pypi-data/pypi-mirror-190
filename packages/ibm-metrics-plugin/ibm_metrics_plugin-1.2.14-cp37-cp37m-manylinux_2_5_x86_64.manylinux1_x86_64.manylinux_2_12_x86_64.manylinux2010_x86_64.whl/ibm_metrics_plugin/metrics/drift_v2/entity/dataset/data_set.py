...
from ibm_metrics_plugin.metrics.drift_v2.utils.constants import (
    DATA_QUALITY_ISSUES_COLUMN, META_MODEL_CONFIDENCE_COLUMN,
...
class DriftDataSet:
    ...
    async def factory(self,
                      ...
                      drift_controls: dict = {},) -> "DriftDataSet":
    ...
    def set_control_box(self, drift_controls: dict = {}) -> None:
        self.drift_controls = drift_controls
        ...
        if drift_controls is None:
            drift_controls = {}

        control_box.set_config_map(drift_controls)

    def score(self, transactions: pd.DataFrame,
              record_id_column: str = "scoring_id",
              record_timestamp_column: str = "scoring_timestamp") -> pd.DataFrame:
        result_df = transactions[[record_id_column, record_timestamp_column]].copy()

        if (self.problem_type in (ProblemType.BINARY, ProblemType.MULTICLASS)):
            # 1. Split probability array into individual probabilities
            ...
            is_probability_list = transactions[probability_column].apply(
                lambda x: isinstance(x, (list, np.ndarray))).all()
            is_probability_str = transactions[probability_column].apply(
                lambda x: isinstance(x, (str))).all()

            if is_probability_str:
                transactions[class_probability_columns] = pd.DataFrame(
                    transactions[probability_column].apply(
                        lambda x: json.loads(x)).values.tolist(),
                    index=transactions.index)
            elif is_probability_list:
                transactions[class_probability_columns] = pd.DataFrame(
                    transactions[probability_column].values.tolist(), index=transactions.index)
            else:
                raise ValueError(
                    f"Column {probability_column} is not a list or a string. {transactions[probability_column].dtype}")

            for column in class_probability_columns:
                result_df = result_df.assign(**{column: transactions[column]})
            # 2. Score against meta model to get meta model confidence
            if (self.has_accuracy_drift):
                result_df = result_df.assign(
                    **{META_MODEL_CONFIDENCE_COLUMN: self.meta_model.predict_proba(transactions)})

        # 3. Find data quality issues from baseline dataset
        issues_result = DataQualityIssue.score(
            transactions=transactions,
            baseline_data_quality_issues=self.data_quality_issues.values())
        result_df = result_df.assign(**{DATA_QUALITY_ISSUES_COLUMN: issues_result})
        result_df = result_df.replace({np.nan: None, float("nan"): None})
    ...
    def to_dict(self) -> dict:
        ...
        dict_ = {
            ...
            "drift_controls": self.drift_controls,
    ...
    def from_dict(cls, data: dict) -> "DriftDataSet":
        ...
        try:
            ...
            dataset.drift_controls = summary_dict["drift_controls"]

...
class WhiteboardLine(WhiteboardObject):
    ...
    def draw_handles(self, layer, gc):
        gc.SetPen(wx.BLACK_PEN)
        gc.SetBrush(wx.LIGHT_GREY_BRUSH)

        for p in self.handles:
            (x, y) = p;
            gc.DrawRectangle(x-3, y-3, 7, 7)
    ...
    def toxml(self, action="update"):
        ...
        for p in self.points:
            (x,y) = p
            xml_str += str(round(x)) + "," + str(round(y)) + ";"
        xml_str += "'"

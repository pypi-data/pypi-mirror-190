...
def stockInformationHistoric(url, fileName):
    ...
    for x in range(len(date)):
        if x - 1 >= 0:
            returnArray.append([stockPrice[x], (float(stockPrice[x]) - float(stockPrice[x-1])), (100*(float(stockPrice[x]) - float(stockPrice[x-1])) / float(stockPrice[x-1])), None, newVolume[x], datetime.datetime.month, datetime.datetime.day, datetime.datetime.year])
        else:
            returnArray.append([stockPrice[x], None, None, None, newVolume[x], datetime.datetime.month, datetime.datetime.day, datetime.datetime.year])

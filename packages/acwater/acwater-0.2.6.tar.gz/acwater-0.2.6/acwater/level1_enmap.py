...
class Level1_ENMAP(object):
    ...
    def __init__(self,
        ...
        # self.landmask = landmask  # string for landmask
        ...
        # elif isinstance(data_obj, GeoArray):
        #
        #     # Not implemented!
        #     # Here a data_object ist needed that contains vnir and swir, i.e. it includes both detector lat/lons and detector metadata (gains, offsets)
        #     # A solution could be to redefine the class EnMAP_Detector_SensorGeo(_EnMAP_Image) for that to contain both lat/lon, gains+offsets
        #     # for lat/lon (sensor geometry) consider adjustment vnir to swir offset, required at read block

        #     self.detector_data = data_obj
        #
        #     self.detector_meta = detector_meta
        #     # self.detector_meta.unitcode = "TOARad"
        #     # self.detector_meta.unit = 'mW m^-2 sr^-1 nm^-1'
        #     # self.detector_meta.offsets = 0
        #     # self.detector_meta.gains = 1
        #
        #     self.detector_meta.lats = detector_meta.lats
        #     self.detector_meta.lons = detector_meta.lons
        #
        #     self.platform_meta = platform_meta # data_obj.meta
    ...
    def read_block(self, size, offset, bands):
        ...
        # BANDS_ENMAP = get_bands_dict('wavelength')
        #
        # if self.detector_name == 'vnir':
        #     # assert len(self.detector_meta.wvl_center) == len(BANDS_ENMAP['band_names'])
        #     # must have same number of bands as predefined
        #     # BANDS_ENMAP= get_bands_dict(self.detector_meta.wvl_center)
        #     ibands = np.array([self.bands_enmap.index(b) for b in bands])  # bands values
        #
        # elif self.detector_name == 'swir':
        #     raise NotImplementedError
        # elif self.detector_name == 'vnswir':
        #     # bands are read from GeoArray in data.metadata, not Detector metadata
        #     # BANDS_ENMAP = get_bands_dict(self.detector_data.metadata.band_meta['wavelength'])
        #     ibands = np.array([self.bands_enmap.index(b) for b in bands])  # bands values
        # else:
        #     raise NotImplementedError

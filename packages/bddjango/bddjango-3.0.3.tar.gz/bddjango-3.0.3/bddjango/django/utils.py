...
def get_count(qs_ls, expired_time=0):
    if not expired_time:
        return qs_ls.count()

    _qs_ls = qs_ls.order_by().values("pk")
    query = f"cache__{_qs_ls.query}"
    # query = base64.b64encode(bytes(query, 'utf-8')).hex()  # 最好是64进制之类
    # print("cache query --- ", type(query), len(query), "---", query)
...
def paginate_qsls_to_dcls(qs_ls, serializer, page: int, per_page=16, context=None, cache_expired_time=0, get_page_dc=True):
    ...
    if 0:
        ...
        # qsls = search_models.TitleBase.objects.filter(
        #     wu_li_qi_shi_ye_ma__isnull=False,
        #     wu_li_jie_shu_ye_ma__isnull=False,
        #     yin_shua_ye_ma__isnull=False,
        #     book_name__contains="苗族",
        # )
        # qsls = 1
        dc_ls = serializer(qs_ls[20: 30], many=True, context=context).data
        # dc_ls = serializer(qsls.order_by()[20: 30], many=True, context=context).data
        # dc_ls = serializer(qsls.order_by('pk')[20: 30], many=True, context=context).data
        # dc_ls = serializer(qsls.order_by('pk')[20: 30], many=True, context=context).data

        # F(order_type).asc(nulls_first=True)
        print('--- paginate_qsls_to_dcls.tt --- 1', tt.now(1))
    ...
    page_obj = qs_ls[start_i: end_i + 1]

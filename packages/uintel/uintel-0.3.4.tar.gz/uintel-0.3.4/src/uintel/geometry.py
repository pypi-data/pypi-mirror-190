...
def _check_inputted_datasets(raster_path, vector_path):
    ...
    try:
        ...
        print("RasterIO could not be located. Please install rasterio seperately into this Python environment, or run 'pip install uintel[full]'")
        return
...
def calculate_zonal_statistics(raster_path: str, vector_path: str, raster_band: int = 1, allow_touching_pixels: bool = True, show_progress: bool = True) -> dict:
    ...
    try:
        ...
        print("GDAL could not be located. Please install GDAL seperately into this Python environment, or run 'pip install uintel[full]'")
        return
    try:
        import rasterio as rio
    except ModuleNotFoundError:

        ...
        while dq:
            ...
            for f in files:
                ...
                try:
                    ...
                    log.info(f'WARNING: cannot stat {f} ({e})')
                    continue

                if follow_links and S_ISLNK(lstat.st_mode):
                    try:
                        fstat = f.stat()
                    except OSError as e:
                        # If we encounter a broken symbolic link when 'follow_symlinks' is
                        # on, assume the destination is not mounted and lock it to prevent
                        # its contents from being removed from the tree.
                        log.info(f'WARNING: cannot follow link {f} ({e}), locking')
                        self.lock_path(virtual_path)
                        continue

                srcf = SourceFile(f, fstat)

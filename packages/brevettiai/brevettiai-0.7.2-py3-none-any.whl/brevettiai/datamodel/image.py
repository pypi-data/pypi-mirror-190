...
class SequenceRange:
    ...
    def slice(self, sequence):
        if self.end == 0:
            return sequence[np.cast(self.start, np.int32):]
        else:
            return sequence[np.cast(self.start, np.int32):np.cast(self.end, np.int32)]

    def slice_padded(self, sequence, constant_value=0):
        start = np.cast(self.start, np.int32)
        end = np.cast(self.end, np.int32)
        if end <= 0:

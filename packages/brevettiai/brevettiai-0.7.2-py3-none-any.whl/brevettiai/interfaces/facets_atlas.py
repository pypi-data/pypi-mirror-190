...
def build_facets_from_dataset(dataset, facet_dive, facet_sprite=None, count=4096, exclude_rows=None):
    ...
    io_tools.write_file(facet_dive, JsonEncoder().encode(facet_data))
...
def create_atlas(sprites, io=None, verbose=True):
    """Create atlas jpeg buffer from list of sprites or paths"""
    is_bgr = False
    if hasattr(sprites, "to_list"):
        sprites = sprites.to_list()
    if type(sprites[0]) == str:
        if io is None:
            raise ValueError("io module must be provided to load thumbnails")
        sprites = list(load_files(
            paths=sprites, callback=_make_thumbnail, io=io, monitor=verbose,
...
def export_facets(path, info, sprites: List[Union[str, np.ndarray]], io):
    """
    ...
    :return:
    """
    json_path = io.path.join(path, "facets.json")
    atlas_path = io.path.join(path, "spriteatlas.jpeg")

    info = JsonEncoder().encode(info.to_dict(orient="records"))
    io.write_file(json_path, info)

    atlas = create_atlas(sprites, io)
    io.write_file(atlas_path, atlas)

...
@click.option(
    "-t", "--target", default=15, type=int, help="Target AC (after defensive bonus)."
)
def combat(
    mojo,
    base,
    ...
    for m in range(10):
        ...
        sns.histplot(rolls, ax=ax[m], bins=10, binrange=[0, 10]).set(
            title=f"Mojo: {m+1}"
        )
    plt.show()
...
@characterize.command()
@click.option(
    "-l", "--mojo_low", default=1, type=int, help="Number of mojo points to use."
)
@click.option(
    "-h", "--mojo_high", default=1, type=int, help="Number of mojo points to use."
)
...
@click.option(
    "-a", "--armor", default=15, type=int, help="Target AC (after defensive bonus)."
)
...
def damage(
    ...
    ax = []
    fig, ax = plt.subplots(
        nrows=(mojo_high + 1 - mojo_low), sharey=True, figsize=(13, 4)
    )
    ...
    for mojo in range(mojo_low, mojo_high + 1):
        ...
        for n in range(samples):
            ...
            print(
                f"Roll: {mrolls} ToHit: {toHit}({hits}) DamageHit: {toHit++armor}({damages}) Damage:{damage_rolls} ({damage})"
            )
            vmax = max(damage, vmax)

        print(
            f"Attacker(Skill: {skill} Mojo: {mojo} Damage: '1d{damage_die}') Defender(Target: {target} Armor: {armor}) 'Adjusted To Hit': {toHit} 'Adjusted To Damage': {toHit+armor}"
        )
        ...
        if mojo_high == mojo_low:
            sns.histplot(rolls, ax=ax, bins=vmax + 1, binrange=[0, vmax + 1]).set(
                title=f"Mojo: {mojo}"
            )
        else:
            sns.histplot(
                rolls, ax=ax[mojo - mojo_low], bins=vmax + 1, binrange=[0, vmax + 1]
            ).set(title=f"Mojo: {mojo}")
    plt.show()
...
@click.option(
    "-a", "--armor", default=5, type=int, help="Target AC (after defensive bonus)."
)
...
def damage1(
    ...
    a = [[], []]
    stats = {"damage": [], "hit": [], "miss": []}
    index = []

    for mojo in range(mojo_low, mojo_high + 1):
        actions = []
        for n in range(samples):
            action = m.actionMelleAttack(
                mojo, toHit, armor, ddcount, ddsides, dbonus, proficiency
            )
        ...
        print(
            f"Attacker(Skill: {skill} Mojo: {mojo} Damage: '1d{ddsides}') Defender(Target: {target} Armor: {armor}) 'Adjusted To Hit': {toHit} 'Adjusted To Damage': {toHit+armor}"
        )
        ...
        print(
            f"Average of damge Overage:       {round(damage_overage_sum/damage_count, 2)}"
        )
        if damage_count > 0:
            print(
                f"Average damage inflicted:       {round(damage_sum/damage_count, 2)}"
            )
        print(f"Max Damage roll:                {vmax}")
    ...
    df.plot(
        kind="bar",
        stacked=True,
        title=f"OffSkill:{skill} Target:{target} Armor:{armor}",
...
@click.option("-d", "--damage_die", default=8, type=int, help="Damage to roll.")
@click.option(
    "-e", "--extra_damage", default=0, type=int, help="Extra Damage addition."
)
def combat1(
    mojo_low,
    mojo_high,
...
def attribute(
    ...
    for n in range(samples):
        rolls.append(
            mojoRoll1(base=base, proficiency=proficiency, mojo=mojo, bonus=bonus)
        )
        if vmax < rolls[n]:
            vmax = rolls[n]
        rsum += rolls[n]
...
@click.option(
    "--target1", default=15, type=int, help="Target AC (after defensive bonus)."
)
@click.option(
...
@click.option(
    "--target2", default=15, type=int, help="Target AC (after defensive bonus)."
)
@click.option(
...
def fight(
    ...
    print(
        f"Offense: Base:{base1} Proficiency:{proficiency1}. Mojo:{mojo1} (per attack). Damage:{damage1}+{extra1}"
    )
    print(f"Defense: Target: {target1}. Armor: {armor1}")
    print(f"Player 2: Starts with {hp2} HP")
    print(
        f"Offense: Base:{base2} Proficiency:{proficiency2}. Mojo:{mojo2} (per attack). Damage:{damage2}+{extra2}"
    )
    print(f"Defense: Target: {target2}. Armor: {armor2}")
    rolls = []
    for n in range(rounds):
        mroll = mojoRoll1(
            modifier=base1, mojo=mojo1, proficiency=proficiency1, bonus=bonus1
        )
        ...
        if damage <= 0:
            ...
            print(
                f"Player1 rolled a {mroll} hitting Player2's armor!. damage of {damage} is not more than armor: {armor2}"
            )
        else:
            hp2 -= damage - armor2
            print(
                f"Player1 rolled a {mroll} hitting Player2 for {damage}. Player2's armor blocked {min(armor2, damage)} dealing {damage-armor2} hp. Player 2 now has {hp2} HP left."
            )
        if hp2 <= 0:
            break
        mroll = mojoRoll1(
            modifier=base2, mojo=mojo2, proficiency=proficiency2, bonus=bonus2
        )
        ...
        if damage <= 0:
            ...
            print(
                f"Player2 rolled a {mroll} hitting Player1's armor!. damage of {damage} is not more than armor: {armor1}"
            )
        else:
            hp1 -= damage - armor1
            print(
                f"Player2 rolled a {mroll} hitting Player1 for {damage}. Player1's armor blocked {min(armor1, damage)} dealing {damage-armor1} hp. Player 1 now has {hp1} HP left."
            )
        if hp1 <= 0:
            break

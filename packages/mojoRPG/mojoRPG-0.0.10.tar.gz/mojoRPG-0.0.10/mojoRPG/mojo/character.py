...
class Character(object):
    ...
    @property
    def initiative(self):
        return self._initiative_roll + self.getAttribute("dexterity") + self.mojoCurrent
    ...
    def heal(self, hit_points):
        new_hp = min(self.hpMax(), self.hpCurrent + hit_points)
        self._hit_points["current"] = new_hp
        print(
            f"{self.name} healed {hit_point}. Hp:{self.hp} (Temp:{self.hpTemp} Max:{self.hpMax})."
        )

    def rest(self):
        self._hit_points["current"] = self.hpMax
        self.mojoCurrent = self.mojoMax
        print(
            f"{self.name} rested. Hp:{self.hp} (Temp:{self.hpTemp} Max:{self.hpMax})."
        )
    ...
    def useMojo(self, mojo):
        ...
        print(
            f"  {self.name} used {mojo_used} mojo and now has {self.mojo} left. "
            + f"{self.mojoAvailable} max available to use this round."
        )
    ...
    def rollInitiative(self):
        self._initiative_roll = Die(sides=10, count=1).roll()
        print(
            f"{self.name} rolled a {self._initiative_roll} for initiative. "
            + f"(mojo+dex+roll=initiative) {self.mojoCurrent}+{self.getAttribute('dexterity')}+{self._initiative_roll} = {self.initiative}"
        )
    ...
    def attack(self, defender, oMojo, dMojo):
        ...
        action["attack"]["overHit"], action["attack"]["overDamage"] = m.getHitDamage(
            attack=m.getAttackValue(roll),
            target=defender.toHit(dMojo),
        ...
        if action["attack"]["overDamage"] > 0:
            action["total_damage"] = (
                action["damage"]["value"] + action["attack"]["overDamage"]
            )
        else:
            action["total_damage"] = 0
        print(
            f'  Attack: roll: {action["attack"]["roll"]} = ({action["attack"]["value"]}) '
            + f'OverHit:{action["attack"]["overHit"]} OverDamage: {action["attack"]["overDamage"]} '
            + f'DamageRoll:{action["damage"]["roll"]}({action["damage"]["value"]}) '
    ...
    def print(self):
        ...
        print(
            f"Offensive Skill:{self.getSkill('offense'):<2} Defensive Skill: {self.getSkill('defense'):<2}"
        )
        print("===============================================")

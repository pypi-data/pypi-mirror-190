    ...
    def get_guest_user_from_request(self, req: Request) -> Optional[GuestUser]:
        ...
        try:
            ...
            if token.get("user") is None:
                raise ValueError(_("Guest token does not contain a user claim"))
            if token.get("resources") is None:
                raise ValueError(_("Guest token does not contain a resources claim"))
            if token.get("rls_rules") is None:
                raise ValueError(_("Guest token does not contain an rls_rules claim"))
            if token.get("type") != "guest":
                raise ValueError(_("This is not a guest token."))

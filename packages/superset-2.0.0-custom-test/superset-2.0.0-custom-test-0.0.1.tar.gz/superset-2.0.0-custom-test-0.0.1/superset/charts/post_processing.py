...
# t(aggregatorName): superset-frontend/plugins/plugin-chart-pivot-table/src/react-pivottable/TableRenderers.jsx
pivot_v2_aggfunc_localization_map = {
  'Count': __('Count'),
  'Count Unique Values': __('Count Unique Values'),
  'List Unique Values': __('List Unique Values'),
  'Sum': __('Sum'),
  'Average': __('Average'),
  'Median': __('Median'),
  'Sample Variance': __('Sample Variance'),
  'Sample Standard Deviation': __('Sample Standard Deviation'),
  'Minimum': __('Minimum'),
  'Maximum': __('Maximum'),
  'First': __('First value'),
  'Last': __('Last value'),
  'Sum as Fraction of Total': __('Sum as Fraction of Total'),
  'Sum as Fraction of Rows': __('Sum as Fraction of Rows'),
  'Sum as Fraction of Columns': __('Sum as Fraction of Columns'),
  'Count as Fraction of Total': __('Count as Fraction of Total'),
  'Count as Fraction of Rows': __('Count as Fraction of Rows'),
  'Count as Fraction of Columns': __('Count as Fraction of Columns'),
}
...
def pivot_df(  # pylint: disable=too-many-locals, too-many-arguments, too-many-statements, too-many-branches
    ...
    metric_name = f"Общий итог ({pivot_v2_aggfunc_localization_map.get(aggfunc, aggfunc)})"
    ...
    try:
        ...
        logger.error(f"Cannot add subtotal and overall total (index): {e}")
...
def apply_post_process(
    ...
    for query in result["queries"]:
        if query["result_format"] == ChartDataResultFormat.JSON:
            ...
            df = pd.read_excel(BytesIO(query["data"]))
        else:
            raise Exception(f"Формат файла {query['result_format']} не поддерживается")
        ...
        # ]
        # Сетап ниже может не учитывать некоторые нюансы, которые возможно
        # учитывает сетап выше
        processed_df = processed_df.reset_index()
        processed_df = processed_df.reset_index().rename(columns={'level_0': 'Мера'})

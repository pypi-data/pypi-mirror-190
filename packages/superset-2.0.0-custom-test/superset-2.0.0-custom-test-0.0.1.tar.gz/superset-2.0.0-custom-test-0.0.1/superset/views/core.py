...
from superset.datasets.commands.exceptions import DatasetCreateFailedError, DatasetInvalidError, DatasetNotFoundError
...
class Superset(BaseSupersetView):  # pylint: disable=too-many-public-methods
    ...
    @has_access_api
    @event_logger.log_this
    @expose("/copy_dash_slices_info/<int:dashboard_id>/", methods=["GET", "POST"])
    def copy_dash_slices_info(  # pylint: disable=no-self-use
        ...
        if data["duplicate_slices_to_new_database"]:
            # Get existing datasets in new database
            new_database = db.session.query(Database).filter_by(id=data["new_database"]["id"]).one_or_none()

            logger.info(f"Новая база данных {new_database}")
            existing_datasets_in_new_database = [{
                "table_name": dataset["table_name"],
                "database": {"id": dataset["database"]["id"]},
                "schema": dataset["schema"],
                "id": dataset["id"],
            } for dataset in data["all_datasets"] if dataset["database"]["id"] == data["new_database"]["id"]]

            # Create datasets and retrieve new datasets' ids
            datasets = []
            for slc in original_dash.slices:
                datasets.append({"id": slc.datasource_id, "table_name": slc.datasource_name})
            datasets = [dict(t) for t in {tuple(d.items()) for d in datasets}]
            logger.info(f"Датасетов использовано в дашборде: {len(datasets)}. Список: {datasets}")

            new_datasets_ids = {}
            for dataset in datasets:
                dataset_name = dataset["table_name"].split('.', 1)[-1]
                # dataset = {id: 1, database: {name: 'PostgreSQL', id: 1}, schema: 'public', table_name: 'logs'}
                # Проверка, что такой датасет уже содержится в другой базе данных
                # если содержится, то новый датасет не создается.
                if dataset_name in [dataset["table_name"] for dataset in existing_datasets_in_new_database if dataset["database"]["id"] == data["new_database"]["id"]]:
                    new_datasets_ids[dataset["id"]] = [
                        new_dataset["id"] for new_dataset in existing_datasets_in_new_database 
                        if dataset_name == new_dataset["table_name"] and new_dataset["database"]["id"] == data["new_database"]["id"]
                        ][0]
                    logger.info(f"Уже существующий датасет {dataset['table_name']} не создается")
                else:
                    try:
                        item = DatasetRestApi.add_model_schema.load({
                            "database": data["new_database"]["id"],
                            "schema": data["new_database"]["schema"],
                            "table_name": dataset_name,
                        })
                    except ValidationError as error:
                        return self.response_400(message=error.messages)
                    try:
                        logger.info(f"Пытаемся создать новый датасет: {item}")
                        new_model = CreateDatasetCommand(g.user, item).run()
                        logger.info(f"Создан датасет: {item}. Его ID: {new_model.id}")
                        # mapping {old_dataset_id: new_dataset_id}
                        new_datasets_ids[dataset["id"]] = new_model.id
                    except DatasetInvalidError as ex:
                        session.rollback()
                        error_message = f"Не удалось создать датасет \"{dataset_name}\" для новой базы данных. Скорее всего, такая таблица отсутствует в новой базе данных."
                        ...
                        error_message = f"Не удалось создать датасет \"{dataset_name}\" для новой базы данных. Скорее всего, такая таблица отсутствует в новой базе данных."
                        logger.error(
                            "Ошибка при создании датасета %s: %s",
            ...
            logger.info(f"Маппинг датасетов (старый: новый): {new_datasets_ids}")

        old_to_new_slice_ids: Dict[int, int] = {}
        if data["duplicate_slices"]:
            # Create new charts with new datasets
            for slc in original_dash.slices:
                if data["duplicate_slices_to_new_database"]:
                    new_slice = slc.clone_to_new_dataset(
                        datasource_id=new_datasets_ids.get(slc.datasource_id, 0),
                    )
                else:
                    new_slice = slc.clone()
                ...
                logger.info(f"Создан график: {new_slice.id}")

            logger.info(f"Маппинг графиков (старый: новый): {old_to_new_slice_ids}")
            ...
            for value in data["positions"].values():
                if isinstance(value, dict) and value.get("meta", {}).get("chartId"):
                    ...
                    logger.info(f"old_id: {old_id}, new_id: {new_id}")

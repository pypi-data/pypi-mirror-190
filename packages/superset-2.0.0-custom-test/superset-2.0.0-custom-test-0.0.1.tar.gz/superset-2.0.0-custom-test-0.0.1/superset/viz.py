...
from superset.utils import core as utils, csv, xlsx
    ...
    def run_extra_queries(self) -> None:
        ...
        for option in time_compare:
            ...
            if not query_object["from_dttm"] or not query_object["to_dttm"]:
                raise QueryObjectValidationError(
                    "При использовании сравнения времени необходимо указать закрытый " 
                    "временной диапазон (как начало, так и конец). Его можно задать "
                    "через фильтрацию по времени."
                )

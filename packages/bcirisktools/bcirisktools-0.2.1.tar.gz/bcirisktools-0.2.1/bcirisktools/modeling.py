...
class ModelTools:
    ...
    def analyze_imbalance(X, y, model, weights, metric_score='roc_auc'):
        ...
        grid = GridSearchCV(estimator=model, param_grid=param_grid, n_jobs=-1, cv=cv, scoring=metric_score)
        # Entrenar la grid search
        grid_result = grid.fit(X, y)
        # Salida
        print(f"Mejor score: {grid_result.best_score_} usando {grid_result.best_params_}" )
    ...
    def select_variables_importance(X, y, min_features):
        ...
        list_variables = []
        # Separamos los datos
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33, random_state=31415, stratify=y)
        # Ajustamos el modelo
        model = XGBClassifier()
        ...
        for thresh in thresholds:
            ...
            if select_X_train.shape[1] >= min_features:
                list_variables.append((thresh, select_X_train.shape[1], [selection.get_support()], recall*100.0))
                sys.stdout.write("\r Running... Thresh=%.5f, n=%d, Recall: %.2f%%" % (thresh, select_X_train.shape[1], recall*100.0))
            else:
                break
    ...
    def select_variables_shap(X, y, min_features):
        ...
        df_shap = pd.DataFrame(zip(var_names, var_shap), 
                                    columns=['Features','Mean Shapley'])
        df_shap = df_shap.sort_values(by='Mean Shapley', 
                                      ascending=False)\
                                      .reset_index(drop=True)
        
        list_variables.append((0, X_train.shape[1], df_shap["Features"].values, recall*100))
        ...
        while n_vars > min_features:
            ...
            list_variables.append((threshold_shap, n_vars, df_shap["Features"].values, recall*100.0))
            sys.stdout.write("\r Running... Thresh=%.5f, n=%d, Recall: %.2f%%" % (threshold_shap, 
                                                                                  n_vars,
                                                                                  recall*100.0))
            threshold_shap = df_shap['Mean Shapley'].min()

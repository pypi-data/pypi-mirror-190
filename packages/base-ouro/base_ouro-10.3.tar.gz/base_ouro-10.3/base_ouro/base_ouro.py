...
def encontra_periodo(cod_turma):
    if cod_turma.__contains__('LIBRAS'):
        if cod_turma.__contains__('CSSC'):
            return '8'
        else:
            return '10'
    if cod_turma.__contains__('LEA'):
        if cod_turma.__contains__('LDN'):
            return '3'
        else:
            return '2'
    if cod_turma.__contains__('CIDE'):
        return '1'
...
def encontra_cr(turma, dict_sigla_cr):
    if not turma.__contains__('-'):
        ...
        if turma.__contains__('LEA') or turma.__contains__('LIBRAS') or turma.__contains__('EAD'):
...
def trata_turma(arquivo, coluna):
    for i in range(len(arquivo)):
        if arquivo[i].__contains__('INTERCAMBIO'):
...
def sigla_escola(df_curso, df_escola):
    ...
    for index, row in curso.iterrows():
        ...
        elif valida_curso == 'Humanidades' or curso['Turma'][index].__contains__('LEA'):
...
def recebe_arquivos(alunos, disciplinas, ch):
...
if __name__ == '__main__':
    ...
    recebe_arquivos(relacao_alunos, relatorio_disciplinas, relatorio_ch)


# Identidade dica sempre no 1º período > Pode ser q mude ao longo dos semestres

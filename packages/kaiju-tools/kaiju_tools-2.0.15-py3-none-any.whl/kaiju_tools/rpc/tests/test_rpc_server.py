...
async def test_rpc_server_methods(rpc_interface, rpc_compatible_service, logger):
    ...
    async with rpc_interface as rpc:
        ...
        session_1, session_2 = {'test': True, 'permissions': frozenset()}, {'test': False, 'permissions': frozenset()}
        session_1, session_2 = SimpleNamespace(**session_1), SimpleNamespace(**session_2)
        ...
        assert r1[1]['result'] == session_1
        assert r2[1]['result'] == session_2

        logger.info('Testing permissions handling.')
        session = SimpleNamespace(permissions=set())

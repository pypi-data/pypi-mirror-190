...
from locater.functions import is_large_file, pprint, shrink_stock_ref, fix_nan, make_filename_valid, \
    ...
    add_top_level_parent_directory, is_image_product, replace_strange_folder_paths, get_alt_images, get_alternate_files, \
    show_image_window, show_folder_window
...
class Locator:
    ...
    def _copy_file_with_checks(self, src_file: str, dst_file: str, ref: str, *args, **kwargs) -> None:
        ...
        try:
            ...
            if os.path.exists(dst_file):
                dst_file = self._deal_with_matches_etc(dst_file, src_file)
            else:
    ...
    def _build_file_lists(self, frm="partials"):
        ...
        for fl in self.paths[frm]:
            ref, path = fl
            base_paths.append([ref, self._get_desc(ref), get_alternate_files(path)])
        return base_paths
    ...
    def check_move_partials(self):
        ...
        paths_to_check_move = []
        for ref, desc, files in files:
            for path in files:
                out_path = show_image_window(path, ref, desc, self._copy_file_with_checks)
                if out_path:

    ...
    while counter < 10:
        ...
        try:
            ...
            if resp.status_code > 204:
                raise Exception("failed to insert") #f"failed on row {i}, column {k}")
            break
                ...
                for i, row in tqdm(df.iterrows(), total=len(df), unit='values', unit_scale=len(df.columns) - 1):
                    ...
                    try:
                        if 'end' in row and origin is not None:
                            ...
                            print (df['start'].iloc[i+1], row['start'])
                            dur = (df['start'].iloc[i+1] - row['start']) // np.timedelta64(1, 'ns')
                    except IndexError:
                        dur = 1
...
class Stream(API):
    ...
    def insert(self, content):
        ...
        for cname in df.columns:
            if cname in ('start', 'end'):
                continue
            nd = self._parent._get('paths', *self._path, cname).json()['node']
            if df[cname].dtype == np.dtype('float32'):
                tmap[cname] = 'float'
            elif df[cname].dtype == np.dtype('float64'):
                tmap[cname] = 'float'
                ...
                tmap[cname] = 'text'
            if self._parent._get('nodes', nd, 'types').status_code == 404:
                self._put('nodes', nd, 'types', tmap[cname])
            cmap[cname] = nd
            dmap[cname] = []
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            for i, row in df.iterrows():
                ...
                if len(dmap['start']) >= 4096:
                    list(res) # force result
                    res = pool.map(index_data, [(self._parent, cmap[k], tmap[k], dmap[k]) for k, v in dmap.items()])
                    for k, v in dmap.items():
                        dmap[k] = []
            if len(dmap['start']) > 0:
                res = pool.map(index_data, [(self._parent, cmap[k], tmap[k], dmap[k]) for k, v in dmap.items()])

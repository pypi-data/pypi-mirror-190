...
async def test_get_or_create():
    await get_or_create_tracker_store(InMemoryTrackerStore(domain))


# noinspection PyPep8Naming
@mock_dynamodb2
async def test_dynamo_get_or_create():
    await get_or_create_tracker_store(DynamoTrackerStore(domain))
...
async def test_tracker_serialisation():
    ...
    assert tracker == await store.deserialise_tracker(
...
async def test_fail_safe_tracker_store_if_no_errors():
    ...
    # test retrieve
    expected = [1]
    mocked_tracker_store.retrieve = CoroutineMock(return_value=expected)
    sender_id = "10"
    ...
    mocked_tracker_store.keys = CoroutineMock(return_value=expected)
...
async def test_fail_safe_tracker_store_with_keys_error():
    mocked_tracker_store = Mock()
    mocked_tracker_store.keys = CoroutineMock(side_effect=Exception())

...
from rasa.constants import ENV_AWS_ENDPOINT_URL, ENV_AZURE_ACCOUNT_KEY, ENV_AZURE_ACCOUNT_NAME, ENV_AZURE_CONTAINER, ENV_BUCKET_NAME
...
def get_persistor(name: Text) -> Optional["Persistor"]:
    ...
    if name == "aws":
        return AWSPersistor(
            ENV_BUCKET_NAME, ENV_AWS_ENDPOINT_URL
        )
    if name == "gcs":
        return GCSPersistor(ENV_BUCKET_NAME)

...
file_map = {'english': 'data_english', 'spanish': 'data_spanish'}
...
def create_app(
    ...
    @app.get("/model/version/<language>/<fileName>")
    async def model_version(request: Request, language, fileName):
        try:
            lang_dir = file_map.get(language, 'data_english')
            with open(lang_dir + '/' + fileName) as f:
                searchTitle = re.search(r'me:\s+(.*?)\s+--->', f.readline())
    ...
    @app.route('/model/download/<language>/<fileName>')
    async def download_file(request: Request, language, fileName):
        try:
            lang_dir = file_map.get(language, 'data_english')
            return await response.file(lang_dir + "/" + fileName, status=200)
        except Exception as err:
    ...
    @app.get("/status/<language>")
    @requires_auth(app, auth_token)
    @ensure_loaded_agent(app)
    async def status(request: Request, language):
        """Respond with the model name and the fingerprint of that model."""
        model_id = model_map.get(language, 0)

        return response.json(
            {
                "model_file": app.agent[model_id].path_to_model_archive
                or app.agent[model_id].model_directory,
    ...
    @app.post("/model/<language>/predict")
    @requires_auth(app, auth_token)
    @ensure_loaded_agent(app, require_core_is_ready=True)
    async def tracker_predict(request: Request, language):
        ...
        try:
            tracker = DialogueStateTracker.from_dict(
                sender_id, request_params, app.agent[model_id].domain.slots
            )
        except Exception as e:
            logger.debug(traceback.format_exc())
            raise ErrorResponse(
        ...
        try:
            policy_ensemble = app.agent[model_id].policy_ensemble
            probabilities, policy = policy_ensemble.probabilities_using_best_policy(
                tracker, app.agent[model_id].domain
            )

            scores = [
                {"action": a, "score": p}
                for a, p in zip(app.agent[model_id].domain.action_names, probabilities)
            ]
    ...
    async def parse(request: Request, model_name: Text):
        ...
        try:
            ...
            try:
                _broker = app.agent[model_id].tracker_store.event_broker
                if _broker:
                    event = {
                        'event': 'nlu',
                        'timestamp': time.time(),
    ...
    @app.post("/model/parseList/<language>")
    @requires_auth(app, auth_token)
    async def parseList(request: Request, language):
        validate_request_body(
            request,
            "No text message defined in request_body. Add text message to request body "
            "in order to obtain the intent and extracted entities.",
    ...
    @app.put("/model/<language>")
    @requires_auth(app, auth_token)
    async def load_model(request: Request, language):
        ...
        app.agent = await _load_agent(
            model_path, model_server, remote_storage, endpoints, app.agent[model_id].lock_store
        )

        logger.debug(f"Successfully loaded model '{model_path}'.")
        return response.json(None, status=204)

    @app.delete("/model/<language>")
    @requires_auth(app, auth_token)
    async def unload_model(request: Request, language):
        model_id = model_map.get(language, 0)
        model_file = app.agent[model_id].model_directory

        app.agent = Agent(lock_store=app.agent[model_id].lock_store)

        logger.debug(f"Successfully unloaded model '{model_file}'.")
        return response.json(None, status=204)

    @app.get("/domain/<language>")
    @requires_auth(app, auth_token)
    @ensure_loaded_agent(app)
    async def get_domain(request: Request, language):
        """Get current domain in yaml or json format."""
        model_id = model_map.get(language, 0)

        accepts = request.headers.get("Accept", default="application/yaml")
        if accepts.endswith("json"):
            domain = app.agent[model_id].domain.as_dict()
            return response.json(domain)
        elif accepts.endswith("yml") or accepts.endswith("yaml"):
            domain_yaml = app.agent[model_id].domain.as_yaml()
            return response.text(
                domain_yaml, status=200, content_type="application/yaml"
            )
        else:
            raise ErrorResponse(
                406,
    ...
    @app.post("/agent/response/<language>")
    #@requires_auth(app, auth_token)
    async def process_response(request: Request, language):
        ...
        try:
            ...
            if tracker.get_slot("handoff_active"):
                ...
                await app.agent[model_id].execute_action(

...
from rasa.constants import DEFAULT_SANIC_WORKERS, SANIC_WORKERS
...
def number_of_sanic_workers(lock_store: Union[EndpointConfig, LockStore, None]) -> int:
    ...
    try:
        env_value = int(os.environ.get(SANIC_WORKERS, DEFAULT_SANIC_WORKERS))
    except ValueError:
        logger.error(
            f"Cannot convert environment variable `ENV_SANIC_WORKERS` "
            f"to int ('{os.environ[SANIC_WORKERS]}')."
        )

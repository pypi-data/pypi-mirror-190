...
class UTrainer(ABC):
    ...
    def model(self, model):
        ...
        if self.attack_cls:
            ...
            logger.info(f'use attack {self.attack_ins}')
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            logger.info(f'[Epoch: {epoch}] train_metric_score: {train_metric_score} train_loss: {train_loss}')
            if train_metric_report:
                self.tb_writer.add_scalars('train_metric_report', train_metric_report, epoch)
                logger.info(f'[Epoch: {epoch}] train_metric_report: {train_metric_report}')
            ...
            logger.info(f'[Epoch: {epoch}] dev_metric_score: {dev_metric_score}')

            if dev_metric_report:
                self.tb_writer.add_scalars('dev_metric_report', dev_metric_report, epoch)
                logger.info(f'[Epoch: {epoch}] dev_metric_report: {dev_metric_report}')

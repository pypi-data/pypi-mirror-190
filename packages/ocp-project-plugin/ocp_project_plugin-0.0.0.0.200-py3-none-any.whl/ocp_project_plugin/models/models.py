...
class OCPProject(NetBoxModel):
    ...
    def save(self, *args, **kwargs):
        ...
        print()
        for ct in ContentType.objects.filter(app_label='ocp_project_plugin', model='ocpproject').all():
            print(ct)


        #ocp_project_type = ContentTypeManager.get_by_natural_key(self, app_label='ocp_project_plugin', model='ocp project')

        ocp_project_type = ContentType.objects.get(app_label='ocp_project_plugin', model='ocpproject')
        contact_assignment = ContactAssignment(object_id=self.pk, contact_id=self.contact.pk,
                                               content_type_id=ocp_project_type.pk, role_id=1)

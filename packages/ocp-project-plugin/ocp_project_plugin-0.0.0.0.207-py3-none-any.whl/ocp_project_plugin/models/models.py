...
project_name_validator = RegexValidator(r"[1-9][0-9]*(Mi|Gi)$", "The input should contain only positive Number, which "
                                                                "ends with Mi (Megabyte) or Gi (Gigabyte)")
...
class OCPProject(NetBoxModel):
    ...
    clone_fields = ["name", "description", "display_name", "owner", "customer", "docu_url", "workload",
                    "request"]

    class Meta:
        verbose_name = "OCP Project"
        ordering = ["name", "description", "display_name", "owner", "customer", "docu_url", "workload",
                    "request"]
